  ...
  if (v5)
  {
    if (!v4)
    {
      ...
      if (CRLAssertCat_init_token != -1)
        dispatch_once(&CRLAssertCat_init_token, &stru_1000719A0);
      if (os_log_type_enabled((os_log_t)CRLAssertCat_log_t, OS_LOG_TYPE_ERROR))
        sub_10004E2B0();
      if (CRLAssertCat_init_token != -1)
        dispatch_once(&CRLAssertCat_init_token, &stru_1000719C0);
...
- (BOOL)supportsGPUFamily:(int64_t)a3
{
  ...
  +[CRLAssertionHandler _atomicIncrementAssertCount](CRLAssertionHandler, "_atomicIncrementAssertCount");
  if (CRLAssertCat_init_token != -1)
    dispatch_once(&CRLAssertCat_init_token, &stru_1000719E0);
  if (os_log_type_enabled((os_log_t)CRLAssertCat_log_t, OS_LOG_TYPE_ERROR))
    sub_10004E35C();
  if (CRLAssertCat_init_token != -1)
    dispatch_once(&CRLAssertCat_init_token, &stru_100071A00);
