@implementation PBKaleidoscopeFilter

- (BOOL)allowAbsoluteGestures
{
  return 0;
}

- (void)setDefaults
{
  double v3;
  double v4;
  objc_super v5;

  -[PBKaleidoscopeFilter setInputPoint:](self, "setInputPoint:", 0.5, 0.5);
  LODWORD(v3) = -1095253570;
  -[PBKaleidoscopeFilter setInputRotation:](self, "setInputRotation:", v3);
  LODWORD(v4) = 1060320051;
  -[PBKaleidoscopeFilter setInputAmount:](self, "setInputAmount:", v4);
  self->firstTime = 1;
  -[CIFilter setDefaults](self->super._ciFilter, "setDefaults");
  v5.receiver = self;
  v5.super_class = (Class)PBKaleidoscopeFilter;
  -[PBFilter setDefaults](&v5, "setDefaults");
}

- (float)inputAmount
{
  return self->_inputAmount;
}

- (void)setInputAmount:(float)a3
{
  float v3;

  v3 = fmin(fmax(a3, 0.1), 1.0);
  self->_inputAmount = v3;
}

- (void)applyParametersToCIFilter:(BOOL)a3 extent:(CGRect)a4
{
  double height;
  double width;
  _BOOL8 v6;
  double v8;
  double v9;
  double v10;
  double v11;
  float v12;
  float v13;
  float v14;
  double v15;
  float v16;
  _BOOL4 v17;
  float inputAmount;
  double v19;
  double v20;
  float v21;
  float v22;

  height = a4.size.height;
  width = a4.size.width;
  v6 = a3;
  -[PBKaleidoscopeFilter inputPoint](self, "inputPoint", a4.origin.x, a4.origin.y);
  -[PBFilter adjustedInputPoint:mirrored:](self, "adjustedInputPoint:mirrored:", v6);
  v9 = v8;
  v11 = v10;
  -[PBKaleidoscopeFilter inputRotation](self, "inputRotation");
  -[PBFilter adjustedInputRotation:mirrored:](self, "adjustedInputRotation:mirrored:", v6);
  v13 = v12;
  v14 = width * v9;
  v15 = height * v11;
  v16 = height * v11;
  if ((!self->firstTime && self->lastInputRotation == v13
     || (*(float *)&v15 = v13,
         -[CIFilter setValue:forKey:](self->super._ciFilter, "setValue:forKey:", +[NSNumber numberWithFloat:](NSNumber, "numberWithFloat:", v15), CFSTR("inputRotation")), self->lastInputRotation = v13, !self->firstTime))&& self->lastInputX == v14&& self->lastInputY == v16)
  {
    v17 = 1;
  }
  else
  {
    -[CIFilter setValue:forKey:](self->super._ciFilter, "setValue:forKey:", +[CIVector vectorWithX:Y:](CIVector, "vectorWithX:Y:", v14, v16), CFSTR("inputPoint"));
    self->lastInputX = v14;
    self->lastInputY = v16;
    v17 = !self->firstTime;
  }
  inputAmount = self->_inputAmount;
  if (width >= height)
    v19 = height;
  else
    v19 = width;
  v20 = v19 * inputAmount * 1.73205081;
  v21 = v20 * 0.5;
  if (v17 && self->lastInputSize == v21)
  {
    v22 = (float)(inputAmount * -0.1) + 0.95;
  }
  else
  {
    *(float *)&v15 = v20 * 0.5;
    -[CIFilter setValue:forKey:](self->super._ciFilter, "setValue:forKey:", +[NSNumber numberWithFloat:](NSNumber, "numberWithFloat:", v15), CFSTR("inputSize"));
    self->lastInputSize = v21;
    v22 = (float)(self->_inputAmount * -0.1) + 0.95;
    if (self->firstTime)
    {
LABEL_16:
      *(float *)&v15 = v22;
      -[CIFilter setValue:forKey:](self->super._ciFilter, "setValue:forKey:", +[NSNumber numberWithFloat:](NSNumber, "numberWithFloat:", v15), CFSTR("inputDecay"));
      self->lastInputDecay = v22;
      goto LABEL_17;
    }
  }
  if (self->lastInputDecay != v22)
    goto LABEL_16;
LABEL_17:
  self->firstTime = 0;
}

- (id)ciFilterName
{
  return CFSTR("CITriangleKaleidoscope");
}

- (BOOL)needsWrapMirror
{
  return 1;
}

- (CGPoint)inputPoint
{
  double v2;
  double v3;
  _QWORD v4[2];
  CGPoint result;

  objc_copyStruct(v4, &self->inputPoint, 16, 1, 0);
  v2 = *(double *)v4;
  v3 = *(double *)&v4[1];
  result.y = v3;
  result.x = v2;
  return result;
}

- (void)setInputPoint:(CGPoint)a3
{
  CGPoint v3;

  v3 = a3;
  objc_copyStruct(&self->inputPoint, &v3, 16, 1, 0);
}

- (float)inputRotation
{
  return self->inputRotation;
}

- (void)setInputRotation:(float)a3
{
  self->inputRotation = a3;
}

@end
