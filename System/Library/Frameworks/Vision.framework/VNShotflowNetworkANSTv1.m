...
+ (id)modelName
{
  return CFSTR("anst_v1.5.espresso");
}
...
+ (const)importantClasses
{
  if (+[VNShotflowNetworkANSTv1 importantClasses]::onceToken != -1)
    dispatch_once(&+[VNShotflowNetworkANSTv1 importantClasses]::onceToken, &__block_literal_global_195);
...
double __43__VNShotflowNetworkANSTv1_importantClasses__block_invoke()
{
  _QWORD *v0;
  _QWORD *v1;
  double result;
  ...
  *v0 = v1;
  +[VNShotflowNetworkANSTv1 importantClasses]::importantClasses = (uint64_t)v0;
  return result;
}
...
- (int)setInputShape:(unint64_t)a3 height:(unint64_t)a4
{
  ...
  if (!result)
  {
    ...
    if (!result)
    {
      ...
      if (!result)
      {
        -[VNShotflowNetworkANSTv1 initializeBuffers](self, "initializeBuffers");
        return 0;
      }
    }
  }
...
- (id)processVImage:(vImage_Buffer *)a3 inputIsBGR:(BOOL)a4
{
  ...
  height = a3->height;
  width = a3->width;
  if (-[VNShotflowNetworkANSTv1 setInputShape:height:](self, "setInputShape:height:", width, height))
  {
    exception = __cxa_allocate_exception(8uLL);
    *exception = 6014;
    __cxa_throw(exception, MEMORY[0x1E0DE4F28], 0);
