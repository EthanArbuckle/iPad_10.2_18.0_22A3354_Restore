...
uint64_t __FPSupport_GetAudioSpatializationPreferencesMonitor_block_invoke()
{
  ...
  if ((in_audio_mx_server_process() & 1) != 0
    || (result = FigAudioSessionCreateUsingAuxiliaryAVAudioSession(*MEMORY[0x1E0C9AE00], 0, (uint64_t)CFSTR("AudioSpatializationPreferencesMonitor"), &FPSupport_GetAudioSpatializationPreferencesMonitor_audioSession), !(_DWORD)result))
  {
    CMNotificationCenterGetDefaultLocalCenter();
