...
- (CMBikeSensorInternal)init
{
  ...
  v7.receiver = self;
  v7.super_class = (Class)CMBikeSensorInternal;
  v2 = -[CMBikeSensorInternal init](&v7, sel_init);
  if (v2)
  {
    *((_QWORD *)v2 + 2) = dispatch_queue_create("com.apple.CoreMotion.CMBikeSensor", 0);
    v3 = operator new();
  ...
  return (CMBikeSensorInternal *)v2;
}
...
- (void)dealloc
{
  ...
  -[CMBikeSensorInternal dealloc](&v3, sel_dealloc);
}
...
- (void)_feedBikeSensorData:(id)a3
{
  ...
  if (!a3)
  {
    ...
    objc_msgSend_handleFailureInMethod_object_file_lineNumber_description_(v9, v10, (uint64_t)a2, (uint64_t)self, (uint64_t)CFSTR("CMBikeSensor.mm"), 78, CFSTR("Invalid parameter not satisfying: %@"), CFSTR("bikeSensorData"));
  }
  fInternalQueue = self->fInternalQueue;
  block[0] = MEMORY[0x1E0C809B0];
  block[1] = 3221225472;
  block[2] = sub_18F427ABC;
