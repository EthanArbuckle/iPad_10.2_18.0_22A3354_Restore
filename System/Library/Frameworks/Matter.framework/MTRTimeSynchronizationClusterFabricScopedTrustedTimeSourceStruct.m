@implementation MTRTimeSynchronizationClusterFabricScopedTrustedTimeSourceStruct

- (MTRTimeSynchronizationClusterFabricScopedTrustedTimeSourceStruct)init
{
  MTRTimeSynchronizationClusterFabricScopedTrustedTimeSourceStruct *v2;
  MTRTimeSynchronizationClusterFabricScopedTrustedTimeSourceStruct *v3;
  NSNumber *nodeID;
  NSNumber *endpoint;
  objc_super v7;

  v7.receiver = self;
  v7.super_class = (Class)MTRTimeSynchronizationClusterFabricScopedTrustedTimeSourceStruct;
  v2 = -[MTRTimeSynchronizationClusterFabricScopedTrustedTimeSourceStruct init](&v7, sel_init);
  v3 = v2;
  if (v2)
  {
    nodeID = v2->_nodeID;
    v2->_nodeID = (NSNumber *)&unk_250591B18;

    endpoint = v3->_endpoint;
    v3->_endpoint = (NSNumber *)&unk_250591B18;

  }
  return v3;
}

- (id)copyWithZone:(_NSZone *)a3
{
  MTRTimeSynchronizationClusterFabricScopedTrustedTimeSourceStruct *v4;
  const char *v5;
  uint64_t v6;
  void *v7;
  const char *v8;
  const char *v9;
  uint64_t v10;
  void *v11;
  const char *v12;

  v4 = objc_alloc_init(MTRTimeSynchronizationClusterFabricScopedTrustedTimeSourceStruct);
  objc_msgSend_nodeID(self, v5, v6);
  v7 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_setNodeID_(v4, v8, (uint64_t)v7);

  objc_msgSend_endpoint(self, v9, v10);
  v11 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_setEndpoint_(v4, v12, (uint64_t)v11);

  return v4;
}

- (id)description
{
  void *v3;
  objc_class *v4;
  void *v5;
  const char *v6;
  void *v7;

  v3 = (void *)MEMORY[0x24BDD17C8];
  v4 = (objc_class *)objc_opt_class();
  NSStringFromClass(v4);
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_stringWithFormat_(v3, v6, (uint64_t)CFSTR("<%@: nodeID:%@; endpoint:%@; >"),
    v5,
    self->_nodeID,
    self->_endpoint);
  v7 = (void *)objc_claimAutoreleasedReturnValue();

  return v7;
}

- (NSNumber)nodeID
{
  return self->_nodeID;
}

- (void)setNodeID:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 8);
}

- (NSNumber)endpoint
{
  return self->_endpoint;
}

- (void)setEndpoint:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 16);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_endpoint, 0);
  objc_storeStrong((id *)&self->_nodeID, 0);
}

@end
