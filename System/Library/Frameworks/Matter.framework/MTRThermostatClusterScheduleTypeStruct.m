@implementation MTRThermostatClusterScheduleTypeStruct

- (MTRThermostatClusterScheduleTypeStruct)init
{
  MTRThermostatClusterScheduleTypeStruct *v2;
  MTRThermostatClusterScheduleTypeStruct *v3;
  NSNumber *systemMode;
  NSNumber *numberOfSchedules;
  NSNumber *scheduleTypeFeatures;
  objc_super v8;

  v8.receiver = self;
  v8.super_class = (Class)MTRThermostatClusterScheduleTypeStruct;
  v2 = -[MTRThermostatClusterScheduleTypeStruct init](&v8, sel_init);
  v3 = v2;
  if (v2)
  {
    systemMode = v2->_systemMode;
    v2->_systemMode = (NSNumber *)&unk_250591B18;

    numberOfSchedules = v3->_numberOfSchedules;
    v3->_numberOfSchedules = (NSNumber *)&unk_250591B18;

    scheduleTypeFeatures = v3->_scheduleTypeFeatures;
    v3->_scheduleTypeFeatures = (NSNumber *)&unk_250591B18;

  }
  return v3;
}

- (id)copyWithZone:(_NSZone *)a3
{
  MTRThermostatClusterScheduleTypeStruct *v4;
  const char *v5;
  uint64_t v6;
  void *v7;
  const char *v8;
  const char *v9;
  uint64_t v10;
  void *v11;
  const char *v12;
  const char *v13;
  uint64_t v14;
  void *v15;
  const char *v16;

  v4 = objc_alloc_init(MTRThermostatClusterScheduleTypeStruct);
  objc_msgSend_systemMode(self, v5, v6);
  v7 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_setSystemMode_(v4, v8, (uint64_t)v7);

  objc_msgSend_numberOfSchedules(self, v9, v10);
  v11 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_setNumberOfSchedules_(v4, v12, (uint64_t)v11);

  objc_msgSend_scheduleTypeFeatures(self, v13, v14);
  v15 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_setScheduleTypeFeatures_(v4, v16, (uint64_t)v15);

  return v4;
}

- (id)description
{
  void *v3;
  objc_class *v4;
  void *v5;
  const char *v6;
  void *v7;

  v3 = (void *)MEMORY[0x24BDD17C8];
  v4 = (objc_class *)objc_opt_class();
  NSStringFromClass(v4);
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_stringWithFormat_(v3, v6, (uint64_t)CFSTR("<%@: systemMode:%@; numberOfSchedules:%@; scheduleTypeFeatures:%@; >"),
    v5,
    self->_systemMode,
    self->_numberOfSchedules,
    self->_scheduleTypeFeatures);
  v7 = (void *)objc_claimAutoreleasedReturnValue();

  return v7;
}

- (NSNumber)systemMode
{
  return self->_systemMode;
}

- (void)setSystemMode:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 8);
}

- (NSNumber)numberOfSchedules
{
  return self->_numberOfSchedules;
}

- (void)setNumberOfSchedules:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 16);
}

- (NSNumber)scheduleTypeFeatures
{
  return self->_scheduleTypeFeatures;
}

- (void)setScheduleTypeFeatures:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 24);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_scheduleTypeFeatures, 0);
  objc_storeStrong((id *)&self->_numberOfSchedules, 0);
  objc_storeStrong((id *)&self->_systemMode, 0);
}

@end
