@implementation MTRChannelClusterSeriesInfoStruct

- (MTRChannelClusterSeriesInfoStruct)init
{
  MTRChannelClusterSeriesInfoStruct *v2;
  MTRChannelClusterSeriesInfoStruct *v3;
  NSString *season;
  NSString *episode;
  objc_super v7;

  v7.receiver = self;
  v7.super_class = (Class)MTRChannelClusterSeriesInfoStruct;
  v2 = -[MTRChannelClusterSeriesInfoStruct init](&v7, sel_init);
  v3 = v2;
  if (v2)
  {
    season = v2->_season;
    v2->_season = (NSString *)&stru_2505249E8;

    episode = v3->_episode;
    v3->_episode = (NSString *)&stru_2505249E8;

  }
  return v3;
}

- (id)copyWithZone:(_NSZone *)a3
{
  MTRChannelClusterSeriesInfoStruct *v4;
  const char *v5;
  uint64_t v6;
  void *v7;
  const char *v8;
  const char *v9;
  uint64_t v10;
  void *v11;
  const char *v12;

  v4 = objc_alloc_init(MTRChannelClusterSeriesInfoStruct);
  objc_msgSend_season(self, v5, v6);
  v7 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_setSeason_(v4, v8, (uint64_t)v7);

  objc_msgSend_episode(self, v9, v10);
  v11 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_setEpisode_(v4, v12, (uint64_t)v11);

  return v4;
}

- (id)description
{
  void *v3;
  objc_class *v4;
  void *v5;
  const char *v6;
  void *v7;

  v3 = (void *)MEMORY[0x24BDD17C8];
  v4 = (objc_class *)objc_opt_class();
  NSStringFromClass(v4);
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_stringWithFormat_(v3, v6, (uint64_t)CFSTR("<%@: season:%@; episode:%@; >"),
    v5,
    self->_season,
    self->_episode);
  v7 = (void *)objc_claimAutoreleasedReturnValue();

  return v7;
}

- (NSString)season
{
  return self->_season;
}

- (void)setSeason:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 8);
}

- (NSString)episode
{
  return self->_episode;
}

- (void)setEpisode:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 16);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_episode, 0);
  objc_storeStrong((id *)&self->_season, 0);
}

@end
