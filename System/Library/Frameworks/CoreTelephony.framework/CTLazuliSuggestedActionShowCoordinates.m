@implementation CTLazuliSuggestedActionShowCoordinates

- (id)description
{
  void *v3;
  void *v4;
  void *v5;

  objc_msgSend(MEMORY[0x1E0CB37A0], "stringWithFormat:", CFSTR("<%@"), objc_opt_class());
  v3 = (void *)objc_claimAutoreleasedReturnValue();
  -[CTLazuliSuggestedActionShowCoordinates latitude](self, "latitude");
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v3, "appendFormat:", CFSTR(", latitude = %@"), v4);

  -[CTLazuliSuggestedActionShowCoordinates longitude](self, "longitude");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v3, "appendFormat:", CFSTR(", longitude = %@"), v5);

  objc_msgSend(v3, "appendString:", CFSTR(">"));
  return v3;
}

- (BOOL)isEqualToCTLazuliSuggestedActionShowCoordinates:(id)a3
{
  id v4;
  void *v5;
  double v6;
  double v7;
  void *v8;
  double v9;
  void *v10;
  double v11;
  double v12;
  void *v13;
  double v14;
  BOOL v15;

  v4 = a3;
  -[CTLazuliSuggestedActionShowCoordinates latitude](self, "latitude");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v5, "doubleValue");
  v7 = v6;
  objc_msgSend(v4, "latitude");
  v8 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v8, "doubleValue");
  if (v7 == v9)
  {
    -[CTLazuliSuggestedActionShowCoordinates longitude](self, "longitude");
    v10 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v10, "doubleValue");
    v12 = v11;
    objc_msgSend(v4, "longitude");
    v13 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v13, "doubleValue");
    v15 = v12 == v14;

  }
  else
  {
    v15 = 0;
  }

  return v15;
}

- (BOOL)isEqual:(id)a3
{
  CTLazuliSuggestedActionShowCoordinates *v4;
  CTLazuliSuggestedActionShowCoordinates *v5;
  BOOL v6;

  v4 = (CTLazuliSuggestedActionShowCoordinates *)a3;
  v5 = v4;
  if (v4 == self)
    v6 = 1;
  else
    v6 = v4
      && (objc_opt_class(), (objc_opt_isKindOfClass() & 1) != 0)
      && -[CTLazuliSuggestedActionShowCoordinates isEqualToCTLazuliSuggestedActionShowCoordinates:](self, "isEqualToCTLazuliSuggestedActionShowCoordinates:", v5);

  return v6;
}

- (id)copyWithZone:(_NSZone *)a3
{
  CTLazuliSuggestedActionShowCoordinates *v4;

  v4 = +[CTLazuliSuggestedActionShowCoordinates allocWithZone:](CTLazuliSuggestedActionShowCoordinates, "allocWithZone:", a3);
  -[CTLazuliSuggestedActionShowCoordinates setLatitude:](v4, "setLatitude:", self->_latitude);
  -[CTLazuliSuggestedActionShowCoordinates setLongitude:](v4, "setLongitude:", self->_longitude);
  return v4;
}

- (void)encodeWithCoder:(id)a3
{
  id v4;

  v4 = a3;
  objc_msgSend(v4, "encodeObject:forKey:", self->_latitude, CFSTR("kLatitudeKey"));
  objc_msgSend(v4, "encodeObject:forKey:", self->_longitude, CFSTR("kLongitudeKey"));

}

- (CTLazuliSuggestedActionShowCoordinates)initWithCoder:(id)a3
{
  id v4;
  CTLazuliSuggestedActionShowCoordinates *v5;
  uint64_t v6;
  NSNumber *latitude;
  uint64_t v8;
  NSNumber *longitude;
  objc_super v11;

  v4 = a3;
  v11.receiver = self;
  v11.super_class = (Class)CTLazuliSuggestedActionShowCoordinates;
  v5 = -[CTLazuliSuggestedActionShowCoordinates init](&v11, sel_init);
  if (v5)
  {
    objc_msgSend(v4, "decodeObjectOfClass:forKey:", objc_opt_class(), CFSTR("kLatitudeKey"));
    v6 = objc_claimAutoreleasedReturnValue();
    latitude = v5->_latitude;
    v5->_latitude = (NSNumber *)v6;

    objc_msgSend(v4, "decodeObjectOfClass:forKey:", objc_opt_class(), CFSTR("kLongitudeKey"));
    v8 = objc_claimAutoreleasedReturnValue();
    longitude = v5->_longitude;
    v5->_longitude = (NSNumber *)v8;

  }
  return v5;
}

- (CTLazuliSuggestedActionShowCoordinates)initWithReflection:(const SuggestedActionShowCoordinates *)a3
{
  CTLazuliSuggestedActionShowCoordinates *v4;
  uint64_t v5;
  NSNumber *latitude;
  uint64_t v7;
  NSNumber *longitude;
  objc_super v10;

  v10.receiver = self;
  v10.super_class = (Class)CTLazuliSuggestedActionShowCoordinates;
  v4 = -[CTLazuliSuggestedActionShowCoordinates init](&v10, sel_init);
  if (v4)
  {
    objc_msgSend(MEMORY[0x1E0CB37E8], "numberWithDouble:", a3->var0);
    v5 = objc_claimAutoreleasedReturnValue();
    latitude = v4->_latitude;
    v4->_latitude = (NSNumber *)v5;

    objc_msgSend(MEMORY[0x1E0CB37E8], "numberWithDouble:", a3->var1);
    v7 = objc_claimAutoreleasedReturnValue();
    longitude = v4->_longitude;
    v4->_longitude = (NSNumber *)v7;

  }
  return v4;
}

+ (BOOL)supportsSecureCoding
{
  return 1;
}

- (NSNumber)latitude
{
  return self->_latitude;
}

- (void)setLatitude:(id)a3
{
  objc_storeStrong((id *)&self->_latitude, a3);
}

- (NSNumber)longitude
{
  return self->_longitude;
}

- (void)setLongitude:(id)a3
{
  objc_storeStrong((id *)&self->_longitude, a3);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_longitude, 0);
  objc_storeStrong((id *)&self->_latitude, 0);
}

@end
