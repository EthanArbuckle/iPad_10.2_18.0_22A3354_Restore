@implementation MPMediaKitInitPropertyRadioStationMap

id ___MPMediaKitInitPropertyRadioStationMap_block_invoke_6(uint64_t a1, void *a2)
{
  void *v2;
  void *v3;

  objc_msgSend(a2, "valueForKeyPath:", CFSTR("attributes.airTime.end"));
  v2 = (void *)objc_claimAutoreleasedReturnValue();
  _MPMediaKitDateAndTimeFromString(v2);
  v3 = (void *)objc_claimAutoreleasedReturnValue();

  return v3;
}

id ___MPMediaKitInitPropertyRadioStationMap_block_invoke_5(uint64_t a1, void *a2)
{
  void *v2;
  void *v3;

  objc_msgSend(a2, "valueForKeyPath:", CFSTR("attributes.airTime.start"));
  v2 = (void *)objc_claimAutoreleasedReturnValue();
  _MPMediaKitDateAndTimeFromString(v2);
  v3 = (void *)objc_claimAutoreleasedReturnValue();

  return v3;
}

id ___MPMediaKitInitPropertyRadioStationMap_block_invoke_4(uint64_t a1, void *a2)
{
  void *v2;
  uint64_t v3;
  void *v4;

  objc_msgSend(a2, "valueForKeyPath:", CFSTR("attributes.streamingRadioSubType"));
  v2 = (void *)objc_claimAutoreleasedReturnValue();
  if ((objc_msgSend(v2, "isEqualToString:", CFSTR("Episode")) & 1) != 0)
  {
    v3 = 1;
  }
  else if (objc_msgSend(v2, "isEqualToString:", CFSTR("Shoutcast")))
  {
    v3 = 2;
  }
  else
  {
    v3 = 0;
  }
  objc_msgSend(MEMORY[0x1E0CB37E8], "numberWithInteger:", v3);
  v4 = (void *)objc_claimAutoreleasedReturnValue();

  return v4;
}

void ___MPMediaKitInitPropertyRadioStationMap_block_invoke(uint64_t a1, void *a2, void *a3, void *a4)
{
  id v6;
  id v7;
  id v8;
  void *v9;
  id v10;
  _QWORD v11[4];
  id v12;

  v6 = a2;
  v11[0] = MEMORY[0x1E0C809B0];
  v11[1] = 3221225472;
  v11[2] = ___MPMediaKitInitPropertyRadioStationMap_block_invoke_2;
  v11[3] = &unk_1E3162BD0;
  v12 = v6;
  v10 = v6;
  v7 = a4;
  v8 = a3;
  objc_msgSend(v7, "setRadioIdentifiersWithBlock:", v11);
  objc_msgSend(v8, "personID");
  v9 = (void *)objc_claimAutoreleasedReturnValue();

  objc_msgSend(v7, "setPersonalStoreIdentifiersWithPersonID:block:", v9, &__block_literal_global_310_25706);
}

void ___MPMediaKitInitPropertyRadioStationMap_block_invoke_2(uint64_t a1, void *a2)
{
  void *v2;
  id v3;
  id v4;

  v2 = *(void **)(a1 + 32);
  v3 = a2;
  objc_msgSend(v2, "objectForKeyedSubscript:", CFSTR("id"));
  v4 = (id)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v3, "setStationStringID:", v4);

}

@end
