@implementation IIODisableMetadataParsing
...
uint64_t __IIODisableMetadataParsing_block_invoke(uint64_t result)
{
  ...
  if ((gIIODebugFlags & 0x4000000000000) != 0)
  {
    ...
    if ((_DWORD)result)
      result = (uint64_t)ImageIOLog("••• Ⓜ️  CGImageSourceDisableMetadataParsing - image metadata will no longer be handled in this process\n");
  }
  gPermissions &= ~4uLL;
