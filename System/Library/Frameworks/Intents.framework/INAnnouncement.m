@implementation INAnnouncement

- (INAnnouncement)initWithSpeechDataURL:(id)a3 startTime:(id)a4 endTime:(id)a5 identifier:(id)a6
{
  return -[INAnnouncement initWithSpeechDataURL:identifier:](self, "initWithSpeechDataURL:identifier:", a3, a6, a5);
}

- (NSNumber)startTime
{
  return 0;
}

- (NSNumber)endTime
{
  return 0;
}

- (INAnnouncement)initWithSpeechDataURL:(id)a3 identifier:(id)a4 speechDataTranscription:(id)a5
{
  id v8;
  id v9;
  id v10;
  INAnnouncement *v11;
  uint64_t v12;
  NSURL *speechDataURL;
  uint64_t v14;
  NSString *identifier;
  uint64_t v16;
  NSString *speechDataTranscription;
  objc_super v19;

  v8 = a3;
  v9 = a4;
  v10 = a5;
  v19.receiver = self;
  v19.super_class = (Class)INAnnouncement;
  v11 = -[INAnnouncement init](&v19, sel_init);
  if (v11)
  {
    v12 = objc_msgSend(v8, "copy");
    speechDataURL = v11->_speechDataURL;
    v11->_speechDataURL = (NSURL *)v12;

    v14 = objc_msgSend(v9, "copy");
    identifier = v11->_identifier;
    v11->_identifier = (NSString *)v14;

    v16 = objc_msgSend(v10, "copy");
    speechDataTranscription = v11->_speechDataTranscription;
    v11->_speechDataTranscription = (NSString *)v16;

  }
  return v11;
}

- (INAnnouncement)initWithSpeechDataURL:(id)a3 identifier:(id)a4
{
  return -[INAnnouncement initWithSpeechDataURL:identifier:speechDataTranscription:](self, "initWithSpeechDataURL:identifier:speechDataTranscription:", a3, a4, 0);
}

- (unint64_t)hash
{
  uint64_t v3;
  NSUInteger v4;

  v3 = -[NSURL hash](self->_speechDataURL, "hash");
  v4 = -[NSString hash](self->_identifier, "hash") ^ v3;
  return v4 ^ -[NSString hash](self->_speechDataTranscription, "hash");
}

- (BOOL)isEqual:(id)a3
{
  INAnnouncement *v4;
  INAnnouncement *v5;
  NSURL *speechDataURL;
  NSString *identifier;
  NSString *speechDataTranscription;
  BOOL v9;

  v4 = (INAnnouncement *)a3;
  if (v4 == self)
  {
    v9 = 1;
  }
  else
  {
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
    {
      v5 = v4;
      speechDataURL = self->_speechDataURL;
      v9 = 0;
      if (speechDataURL == v5->_speechDataURL || -[NSURL isEqual:](speechDataURL, "isEqual:"))
      {
        identifier = self->_identifier;
        if (identifier == v5->_identifier || -[NSString isEqual:](identifier, "isEqual:"))
        {
          speechDataTranscription = self->_speechDataTranscription;
          if (speechDataTranscription == v5->_speechDataTranscription
            || -[NSString isEqual:](speechDataTranscription, "isEqual:"))
          {
            v9 = 1;
          }
        }
      }

    }
    else
    {
      v9 = 0;
    }
  }

  return v9;
}

- (INAnnouncement)initWithCoder:(id)a3
{
  id v4;
  void *v5;
  void *v6;
  uint64_t v7;
  void *v8;
  void *v9;
  void *v10;
  uint64_t v11;
  void *v12;
  void *v13;
  INAnnouncement *v14;

  v4 = a3;
  objc_msgSend(v4, "decodeObjectOfClass:forKey:", objc_opt_class(), CFSTR("speechDataURL"));
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  v6 = (void *)MEMORY[0x1E0C99E60];
  v7 = objc_opt_class();
  objc_msgSend(v6, "setWithObjects:", v7, objc_opt_class(), 0);
  v8 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "decodeObjectOfClasses:forKey:", v8, CFSTR("identifier"));
  v9 = (void *)objc_claimAutoreleasedReturnValue();

  v10 = (void *)MEMORY[0x1E0C99E60];
  v11 = objc_opt_class();
  objc_msgSend(v10, "setWithObjects:", v11, objc_opt_class(), 0);
  v12 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "decodeObjectOfClasses:forKey:", v12, CFSTR("speechDataTranscription"));
  v13 = (void *)objc_claimAutoreleasedReturnValue();

  v14 = -[INAnnouncement initWithSpeechDataURL:identifier:speechDataTranscription:](self, "initWithSpeechDataURL:identifier:speechDataTranscription:", v5, v9, v13);
  return v14;
}

- (void)encodeWithCoder:(id)a3
{
  NSURL *speechDataURL;
  id v5;

  speechDataURL = self->_speechDataURL;
  v5 = a3;
  objc_msgSend(v5, "encodeObject:forKey:", speechDataURL, CFSTR("speechDataURL"));
  objc_msgSend(v5, "encodeObject:forKey:", self->_identifier, CFSTR("identifier"));
  objc_msgSend(v5, "encodeObject:forKey:", self->_speechDataTranscription, CFSTR("speechDataTranscription"));

}

- (id)_intents_encodeWithJSONEncoder:(id)a3 codableDescription:(id)a4
{
  void *v5;
  id v6;
  void *v7;
  void *v8;
  void *v9;
  void *v10;

  v5 = (void *)MEMORY[0x1E0C99E08];
  v6 = a3;
  objc_msgSend(v5, "dictionary");
  v7 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v6, "encodeObject:", self->_speechDataURL);
  v8 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v7, "if_setObjectIfNonNil:forKey:", v8, CFSTR("speechDataURL"));

  objc_msgSend(v6, "encodeObject:", self->_identifier);
  v9 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v7, "if_setObjectIfNonNil:forKey:", v9, CFSTR("identifier"));

  objc_msgSend(v6, "encodeObject:", self->_speechDataTranscription);
  v10 = (void *)objc_claimAutoreleasedReturnValue();

  objc_msgSend(v7, "if_setObjectIfNonNil:forKey:", v10, CFSTR("speechDataTranscription"));
  return v7;
}

- (NSString)description
{
  return (NSString *)-[INAnnouncement descriptionAtIndent:](self, "descriptionAtIndent:", 0);
}

- (id)descriptionAtIndent:(unint64_t)a3
{
  void *v5;
  void *v6;
  void *v7;
  void *v8;
  void *v9;
  objc_super v11;

  v5 = (void *)MEMORY[0x1E0CB3940];
  v11.receiver = self;
  v11.super_class = (Class)INAnnouncement;
  -[INAnnouncement description](&v11, sel_description);
  v6 = (void *)objc_claimAutoreleasedReturnValue();
  -[INAnnouncement _dictionaryRepresentation](self, "_dictionaryRepresentation");
  v7 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v7, "descriptionAtIndent:", a3);
  v8 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v5, "stringWithFormat:", CFSTR("%@ %@"), v6, v8);
  v9 = (void *)objc_claimAutoreleasedReturnValue();

  return v9;
}

- (id)_dictionaryRepresentation
{
  NSURL *speechDataURL;
  void *v4;
  NSString *identifier;
  void *v6;
  NSString *speechDataTranscription;
  void *v8;
  void *v9;
  _QWORD v11[3];
  _QWORD v12[4];

  v12[3] = *MEMORY[0x1E0C80C00];
  v11[0] = CFSTR("speechDataURL");
  speechDataURL = self->_speechDataURL;
  v4 = speechDataURL;
  if (!speechDataURL)
  {
    objc_msgSend(MEMORY[0x1E0C99E38], "null");
    v4 = (void *)objc_claimAutoreleasedReturnValue();
  }
  v12[0] = v4;
  v11[1] = CFSTR("identifier");
  identifier = self->_identifier;
  v6 = identifier;
  if (!identifier)
  {
    objc_msgSend(MEMORY[0x1E0C99E38], "null");
    v6 = (void *)objc_claimAutoreleasedReturnValue();
  }
  v12[1] = v6;
  v11[2] = CFSTR("speechDataTranscription");
  speechDataTranscription = self->_speechDataTranscription;
  v8 = speechDataTranscription;
  if (!speechDataTranscription)
  {
    objc_msgSend(MEMORY[0x1E0C99E38], "null");
    v8 = (void *)objc_claimAutoreleasedReturnValue();
  }
  v12[2] = v8;
  objc_msgSend(MEMORY[0x1E0C99D80], "dictionaryWithObjects:forKeys:count:", v12, v11, 3);
  v9 = (void *)objc_claimAutoreleasedReturnValue();
  if (speechDataTranscription)
  {
    if (identifier)
      goto LABEL_9;
LABEL_12:

    if (speechDataURL)
      return v9;
LABEL_13:

    return v9;
  }

  if (!identifier)
    goto LABEL_12;
LABEL_9:
  if (!speechDataURL)
    goto LABEL_13;
  return v9;
}

- (NSURL)speechDataURL
{
  return self->_speechDataURL;
}

- (NSString)identifier
{
  return self->_identifier;
}

- (NSString)speechDataTranscription
{
  return self->_speechDataTranscription;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_speechDataTranscription, 0);
  objc_storeStrong((id *)&self->_identifier, 0);
  objc_storeStrong((id *)&self->_speechDataURL, 0);
}

+ (BOOL)supportsSecureCoding
{
  return 1;
}

+ (id)_intents_decodeWithJSONDecoder:(id)a3 codableDescription:(id)a4 from:(id)a5
{
  id v7;
  id v8;
  uint64_t v9;
  void *v10;
  void *v11;
  void *v12;
  void *v13;
  void *v14;

  v7 = a3;
  v8 = a5;
  objc_opt_class();
  if ((objc_opt_isKindOfClass() & 1) != 0)
  {
    v9 = objc_opt_class();
    objc_msgSend(v8, "objectForKeyedSubscript:", CFSTR("speechDataURL"));
    v10 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v7, "decodeObjectOfClass:from:", v9, v10);
    v11 = (void *)objc_claimAutoreleasedReturnValue();

    objc_msgSend(v8, "objectForKeyedSubscript:", CFSTR("identifier"));
    v12 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v8, "objectForKeyedSubscript:", CFSTR("speechDataTranscription"));
    v13 = (void *)objc_claimAutoreleasedReturnValue();
    v14 = (void *)objc_msgSend(objc_alloc((Class)a1), "initWithSpeechDataURL:identifier:speechDataTranscription:", v11, v12, v13);

  }
  else
  {
    v14 = 0;
  }

  return v14;
}

@end
