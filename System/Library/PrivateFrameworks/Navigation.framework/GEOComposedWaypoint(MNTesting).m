@implementation GEOComposedWaypoint(MNTesting)

+ (id)appleParkWaypointFromMuid
{
  id v0;
  void *v1;
  void *v2;

  v0 = objc_alloc(MEMORY[0x1E0D26FF8]);
  v1 = (void *)objc_msgSend(objc_alloc(MEMORY[0x1E0C99D50]), "initWithBase64EncodedString:options:", &cfstr_Cquccaisngiiyu, 0);
  v2 = (void *)objc_msgSend(v0, "initWithData:", v1);

  return v2;
}

+ (id)appleParkWaypointFromAddress
{
  id v0;
  void *v1;
  void *v2;

  v0 = objc_alloc(MEMORY[0x1E0D26FF8]);
  v1 = (void *)objc_msgSend(objc_alloc(MEMORY[0x1E0C99D50]), "initWithBase64EncodedString:options:", CFSTR(""), 0);
  v2 = (void *)objc_msgSend(v0, "initWithData:", v1);

  return v2;
}

+ (id)appleParkWaypointFromLatLng
{
  void *v0;
  void *v1;

  v0 = (void *)objc_msgSend(objc_alloc(MEMORY[0x1E0D27168]), "initWithLatitude:longitude:", 37.3348869, -122.008996);
  v1 = (void *)objc_msgSend(objc_alloc(MEMORY[0x1E0D26FF8]), "initWithLocation:isCurrentLocation:", v0, 0);

  return v1;
}

@end
