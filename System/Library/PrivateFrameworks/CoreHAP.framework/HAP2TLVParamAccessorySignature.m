@implementation HAP2TLVParamAccessorySignature

- (HAP2TLVParamAccessorySignature)init
{
  objc_super v3;

  v3.receiver = self;
  v3.super_class = (Class)HAP2TLVParamAccessorySignature;
  return -[HAP2TLVParamAccessorySignature init](&v3, sel_init);
}

- (HAP2TLVParamAccessorySignature)initWithAccessoryID:(id)a3 serviceList:(id)a4
{
  id v7;
  id v8;
  HAP2TLVParamAccessorySignature *v9;
  HAP2TLVParamAccessorySignature *v10;
  objc_super v12;

  v7 = a3;
  v8 = a4;
  v12.receiver = self;
  v12.super_class = (Class)HAP2TLVParamAccessorySignature;
  v9 = -[HAP2TLVParamAccessorySignature init](&v12, sel_init);
  v10 = v9;
  if (v9)
  {
    objc_storeStrong((id *)&v9->_accessoryID, a3);
    objc_storeStrong((id *)&v10->_serviceList, a4);
  }

  return v10;
}

- (BOOL)parseFromData:(id)a3 error:(id *)a4
{
  id v6;
  unint64_t v7;
  uint64_t v8;
  void *v9;
  void *v10;
  void *v11;
  unint64_t v12;
  uint64_t Next;
  void *v14;
  uint64_t v15;
  id v16;
  void *v17;
  uint64_t v18;
  BOOL v19;
  id *v20;
  id v21;
  uint64_t v22;
  id *v24;
  id v25;
  id v26;
  id v27;
  unint64_t v28[2];
  uint64_t v29;
  char v30;

  v6 = objc_retainAutorelease(a3);
  v7 = objc_msgSend(v6, "bytes");
  v8 = objc_msgSend(v6, "length");
  if (v8 < 1)
  {
    v9 = 0;
    v10 = 0;
LABEL_16:
    -[HAP2TLVParamAccessorySignature setAccessoryID:](self, "setAccessoryID:", v10, v24);
    -[HAP2TLVParamAccessorySignature setServiceList:](self, "setServiceList:", v9);
    v11 = 0;
    v19 = 1;
    goto LABEL_24;
  }
  v24 = a4;
  v9 = 0;
  v10 = 0;
  v11 = 0;
  v12 = v7 + v8;
  while (1)
  {
    v30 = 0;
    v28[1] = 0;
    v29 = 0;
    v28[0] = 0;
    Next = TLV8GetNext();
    if ((_DWORD)Next)
      break;
    if (!v29)
    {
      objc_msgSend(MEMORY[0x1E0CB35C8], "errorWithDomain:code:userInfo:", *MEMORY[0x1E0D27FC8], 3, 0);
      v22 = objc_claimAutoreleasedReturnValue();

      v11 = (void *)v22;
      if (!v22)
        goto LABEL_16;
LABEL_20:
      v20 = v24;
      if (v24)
      {
        v21 = objc_retainAutorelease(v11);
        v11 = v21;
        goto LABEL_22;
      }
      goto LABEL_23;
    }
    if (v30 == 22)
    {
      v26 = v11;
      HAPTLVParseContiguousTlvs(22, v7, v12, v28, &v26);
      v14 = (void *)objc_claimAutoreleasedReturnValue();
      v16 = v26;

      if (!v16)
      {
        v25 = 0;
        +[HAP2TLVParamServiceList parsedFromData:error:](HAP2TLVParamServiceList, "parsedFromData:error:", v14, &v25);
        v18 = objc_claimAutoreleasedReturnValue();
        v16 = v25;
        v17 = v9;
        v9 = (void *)v18;
LABEL_10:

      }
      v11 = v16;
      goto LABEL_12;
    }
    if (v30 == 26)
    {
      objc_msgSend(MEMORY[0x1E0C99D50], "dataWithBytesNoCopy:length:freeWhenDone:");
      v14 = (void *)objc_claimAutoreleasedReturnValue();
      v27 = v11;
      +[HAPTLVUnsignedNumberValue parsedFromData:error:](HAPTLVUnsignedNumberValue, "parsedFromData:error:", v14, &v27);
      v15 = objc_claimAutoreleasedReturnValue();
      v16 = v27;

      v17 = v10;
      v10 = (void *)v15;
      goto LABEL_10;
    }
LABEL_12:
    v7 = v28[0];
    if (v28[0] >= v12)
    {
      if (!v11)
        goto LABEL_16;
      goto LABEL_20;
    }
  }
  v20 = v24;
  if (v24)
  {
    HMErrorFromOSStatus(Next);
    v21 = (id)objc_claimAutoreleasedReturnValue();
LABEL_22:
    v19 = 0;
    *v20 = v21;
    goto LABEL_24;
  }
LABEL_23:
  v19 = 0;
LABEL_24:

  return v19;
}

- (id)serializeWithError:(id *)a3
{
  void *v5;
  void *v6;
  void *v7;
  id v8;
  id v9;
  uint64_t v10;
  void *v11;
  void *v12;
  id v13;
  id v14;
  unint64_t v15;
  unint64_t v16;
  uint64_t v17;
  uint64_t v18;
  uint64_t v19;
  BOOL v20;
  void *v21;
  id v23;
  id v24;
  __int128 v25;
  __int128 v26;
  __int128 v27;
  __int128 v28;
  __int128 v29;
  __int128 v30;
  __int128 v31;
  __int128 v32;
  __int128 v33;
  __int128 v34;
  __int128 v35;
  __int128 v36;
  __int128 v37;
  __int128 v38;
  __int128 v39;
  __int128 v40;
  __int128 v41;
  __int128 v42;
  __int128 v43;
  __int128 v44;
  __int128 v45;
  uint64_t v46;

  v46 = *MEMORY[0x1E0C80C00];
  v44 = 0u;
  v45 = 0u;
  v42 = 0u;
  v43 = 0u;
  v40 = 0u;
  v41 = 0u;
  v38 = 0u;
  v39 = 0u;
  v36 = 0u;
  v37 = 0u;
  v34 = 0u;
  v35 = 0u;
  v32 = 0u;
  v33 = 0u;
  v30 = 0u;
  v31 = 0u;
  v28 = 0u;
  v29 = 0u;
  v26 = 0u;
  v27 = 0u;
  v25 = 0u;
  TLV8BufferInit();
  -[HAP2TLVParamAccessorySignature accessoryID](self, "accessoryID");
  v5 = (void *)objc_claimAutoreleasedReturnValue();

  if (v5)
  {
    -[HAP2TLVParamAccessorySignature accessoryID](self, "accessoryID");
    v6 = (void *)objc_claimAutoreleasedReturnValue();
    v24 = 0;
    objc_msgSend(v6, "serializeWithError:", &v24);
    v7 = (void *)objc_claimAutoreleasedReturnValue();
    v8 = v24;

    if (v8)
      goto LABEL_6;
    v9 = objc_retainAutorelease(v7);
    objc_msgSend(v9, "bytes");
    objc_msgSend(v9, "length");
    v10 = TLV8BufferAppend();

    if ((_DWORD)v10)
    {
LABEL_21:
      if (a3)
      {
        HMErrorFromOSStatus(v10);
        v13 = (id)objc_claimAutoreleasedReturnValue();
        v8 = 0;
        goto LABEL_23;
      }
      v8 = 0;
LABEL_26:
      v21 = 0;
      goto LABEL_27;
    }
  }
  -[HAP2TLVParamAccessorySignature serviceList](self, "serviceList");
  v11 = (void *)objc_claimAutoreleasedReturnValue();

  if (!v11)
  {
LABEL_24:
    objc_msgSend(MEMORY[0x1E0C99D50], "dataWithBytes:length:", v25);
    v21 = (void *)objc_claimAutoreleasedReturnValue();
    v8 = 0;
    goto LABEL_27;
  }
  -[HAP2TLVParamAccessorySignature serviceList](self, "serviceList");
  v12 = (void *)objc_claimAutoreleasedReturnValue();
  v23 = 0;
  objc_msgSend(v12, "serializeWithError:", &v23);
  v7 = (void *)objc_claimAutoreleasedReturnValue();
  v8 = v23;

  if (!v8)
  {
    v14 = objc_retainAutorelease(v7);
    v15 = objc_msgSend(v14, "bytes");
    v16 = v15 + objc_msgSend(v14, "length");
    do
    {
      if ((uint64_t)(v16 - v15) >= 255)
        v17 = 255;
      else
        v17 = v16 - v15;
      v18 = TLV8BufferAppend();
      if ((_DWORD)v18)
        v19 = 0;
      else
        v19 = v17;
      v15 += v19;
      if ((_DWORD)v18)
        v20 = 1;
      else
        v20 = v15 >= v16;
    }
    while (!v20);
    v10 = v18;

    if ((_DWORD)v10)
      goto LABEL_21;
    goto LABEL_24;
  }
LABEL_6:

  if (!a3)
    goto LABEL_26;
  v13 = objc_retainAutorelease(v8);
  v8 = v13;
LABEL_23:
  v21 = 0;
  *a3 = v13;
LABEL_27:
  TLV8BufferFree();

  return v21;
}

- (id)copyWithZone:(_NSZone *)a3
{
  HAP2TLVParamAccessorySignature *v4;
  void *v5;
  void *v6;
  HAP2TLVParamAccessorySignature *v7;

  v4 = +[HAP2TLVParamAccessorySignature allocWithZone:](HAP2TLVParamAccessorySignature, "allocWithZone:", a3);
  -[HAP2TLVParamAccessorySignature accessoryID](self, "accessoryID");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  -[HAP2TLVParamAccessorySignature serviceList](self, "serviceList");
  v6 = (void *)objc_claimAutoreleasedReturnValue();
  v7 = -[HAP2TLVParamAccessorySignature initWithAccessoryID:serviceList:](v4, "initWithAccessoryID:serviceList:", v5, v6);

  return v7;
}

- (BOOL)isEqual:(id)a3
{
  void *v3;
  HAP2TLVParamAccessorySignature *v5;
  HAP2TLVParamAccessorySignature *v6;
  void *v7;
  void *v8;
  void *v9;
  char v10;
  void *v11;
  void *v12;
  void *v13;
  void *v14;
  void *v16;

  v5 = (HAP2TLVParamAccessorySignature *)a3;
  if (self == v5)
  {
    v10 = 1;
  }
  else
  {
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
    {
      v6 = v5;
      -[HAP2TLVParamAccessorySignature accessoryID](self, "accessoryID");
      v7 = (void *)objc_claimAutoreleasedReturnValue();
      -[HAP2TLVParamAccessorySignature accessoryID](v6, "accessoryID");
      v8 = (void *)objc_claimAutoreleasedReturnValue();
      if (v7 != v8)
      {
        -[HAP2TLVParamAccessorySignature accessoryID](self, "accessoryID");
        v9 = (void *)objc_claimAutoreleasedReturnValue();
        -[HAP2TLVParamAccessorySignature accessoryID](v6, "accessoryID");
        v3 = (void *)objc_claimAutoreleasedReturnValue();
        if (!objc_msgSend(v9, "isEqual:", v3))
        {
          v10 = 0;
LABEL_13:

LABEL_14:
          goto LABEL_15;
        }
        v16 = v9;
      }
      -[HAP2TLVParamAccessorySignature serviceList](self, "serviceList");
      v11 = (void *)objc_claimAutoreleasedReturnValue();
      -[HAP2TLVParamAccessorySignature serviceList](v6, "serviceList");
      v12 = (void *)objc_claimAutoreleasedReturnValue();
      if (v11 == v12)
      {
        v10 = 1;
      }
      else
      {
        -[HAP2TLVParamAccessorySignature serviceList](self, "serviceList");
        v13 = (void *)objc_claimAutoreleasedReturnValue();
        -[HAP2TLVParamAccessorySignature serviceList](v6, "serviceList");
        v14 = (void *)objc_claimAutoreleasedReturnValue();
        v10 = objc_msgSend(v13, "isEqual:", v14);

      }
      v9 = v16;
      if (v7 == v8)
        goto LABEL_14;
      goto LABEL_13;
    }
    v10 = 0;
  }
LABEL_15:

  return v10;
}

- (NSString)description
{
  void *v3;
  void *v4;
  void *v5;
  void *v6;

  v3 = (void *)MEMORY[0x1E0CB3940];
  -[HAP2TLVParamAccessorySignature accessoryID](self, "accessoryID");
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  -[HAP2TLVParamAccessorySignature serviceList](self, "serviceList");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v3, "stringWithFormat:", CFSTR("<HAP2TLVParamAccessorySignature accessoryID=%@, serviceList=%@>"), v4, v5);
  v6 = (void *)objc_claimAutoreleasedReturnValue();

  return (NSString *)v6;
}

- (HAPTLVUnsignedNumberValue)accessoryID
{
  return self->_accessoryID;
}

- (void)setAccessoryID:(id)a3
{
  objc_storeStrong((id *)&self->_accessoryID, a3);
}

- (HAP2TLVParamServiceList)serviceList
{
  return self->_serviceList;
}

- (void)setServiceList:(id)a3
{
  objc_storeStrong((id *)&self->_serviceList, a3);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_serviceList, 0);
  objc_storeStrong((id *)&self->_accessoryID, 0);
}

+ (id)parsedFromData:(id)a3 error:(id *)a4
{
  id v5;
  HAP2TLVParamAccessorySignature *v6;
  HAP2TLVParamAccessorySignature *v7;
  id v8;
  id v10;

  v5 = a3;
  v6 = objc_alloc_init(HAP2TLVParamAccessorySignature);
  v7 = v6;
  if (v6)
  {
    v10 = 0;
    -[HAP2TLVParamAccessorySignature parseFromData:error:](v6, "parseFromData:error:", v5, &v10);
    v8 = v10;
    if (v8)
    {

      v7 = 0;
      if (a4)
        *a4 = objc_retainAutorelease(v8);
    }

  }
  return v7;
}

@end
