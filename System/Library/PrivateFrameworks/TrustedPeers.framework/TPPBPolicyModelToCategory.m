@implementation TPPBPolicyModelToCategory

- (BOOL)hasPrefix
{
  return self->_prefix != 0;
}

- (BOOL)hasCategory
{
  return self->_category != 0;
}

- (id)description
{
  void *v3;
  void *v4;
  void *v5;
  void *v6;
  objc_super v8;

  v3 = (void *)MEMORY[0x1E0CB3940];
  v8.receiver = self;
  v8.super_class = (Class)TPPBPolicyModelToCategory;
  -[TPPBPolicyModelToCategory description](&v8, sel_description);
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  -[TPPBPolicyModelToCategory dictionaryRepresentation](self, "dictionaryRepresentation");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v3, "stringWithFormat:", CFSTR("%@ %@"), v4, v5);
  v6 = (void *)objc_claimAutoreleasedReturnValue();

  return v6;
}

- (id)dictionaryRepresentation
{
  void *v3;
  void *v4;
  NSString *prefix;
  NSString *category;

  objc_msgSend(MEMORY[0x1E0C99E08], "dictionary");
  v3 = (void *)objc_claimAutoreleasedReturnValue();
  v4 = v3;
  prefix = self->_prefix;
  if (prefix)
    objc_msgSend(v3, "setObject:forKey:", prefix, CFSTR("prefix"));
  category = self->_category;
  if (category)
    objc_msgSend(v4, "setObject:forKey:", category, CFSTR("category"));
  return v4;
}

- (BOOL)readFrom:(id)a3
{
  return TPPBPolicyModelToCategoryReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  id v4;
  id v5;

  v4 = a3;
  v5 = v4;
  if (self->_prefix)
  {
    PBDataWriterWriteStringField();
    v4 = v5;
  }
  if (self->_category)
  {
    PBDataWriterWriteStringField();
    v4 = v5;
  }

}

- (void)copyTo:(id)a3
{
  id v4;
  id v5;

  v4 = a3;
  v5 = v4;
  if (self->_prefix)
  {
    objc_msgSend(v4, "setPrefix:");
    v4 = v5;
  }
  if (self->_category)
  {
    objc_msgSend(v5, "setCategory:");
    v4 = v5;
  }

}

- (id)copyWithZone:(_NSZone *)a3
{
  _QWORD *v5;
  uint64_t v6;
  void *v7;
  uint64_t v8;
  void *v9;

  v5 = (_QWORD *)objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  v6 = -[NSString copyWithZone:](self->_prefix, "copyWithZone:", a3);
  v7 = (void *)v5[2];
  v5[2] = v6;

  v8 = -[NSString copyWithZone:](self->_category, "copyWithZone:", a3);
  v9 = (void *)v5[1];
  v5[1] = v8;

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  _QWORD *v4;
  NSString *prefix;
  NSString *category;
  char v7;

  v4 = a3;
  if (objc_msgSend(v4, "isMemberOfClass:", objc_opt_class())
    && ((prefix = self->_prefix, !((unint64_t)prefix | v4[2]))
     || -[NSString isEqual:](prefix, "isEqual:")))
  {
    category = self->_category;
    if ((unint64_t)category | v4[1])
      v7 = -[NSString isEqual:](category, "isEqual:");
    else
      v7 = 1;
  }
  else
  {
    v7 = 0;
  }

  return v7;
}

- (unint64_t)hash
{
  NSUInteger v3;

  v3 = -[NSString hash](self->_prefix, "hash");
  return -[NSString hash](self->_category, "hash") ^ v3;
}

- (void)mergeFrom:(id)a3
{
  _QWORD *v4;
  _QWORD *v5;

  v4 = a3;
  v5 = v4;
  if (v4[2])
  {
    -[TPPBPolicyModelToCategory setPrefix:](self, "setPrefix:");
    v4 = v5;
  }
  if (v4[1])
  {
    -[TPPBPolicyModelToCategory setCategory:](self, "setCategory:");
    v4 = v5;
  }

}

- (NSString)prefix
{
  return self->_prefix;
}

- (void)setPrefix:(id)a3
{
  objc_storeStrong((id *)&self->_prefix, a3);
}

- (NSString)category
{
  return self->_category;
}

- (void)setCategory:(id)a3
{
  objc_storeStrong((id *)&self->_category, a3);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_prefix, 0);
  objc_storeStrong((id *)&self->_category, 0);
}

+ (id)TPPBPolicyModelToCategoryWithPrefix:(id)a3 category:(id)a4
{
  id v5;
  id v6;
  TPPBPolicyModelToCategory *v7;

  v5 = a4;
  v6 = a3;
  v7 = objc_alloc_init(TPPBPolicyModelToCategory);
  -[TPPBPolicyModelToCategory setPrefix:](v7, "setPrefix:", v6);

  -[TPPBPolicyModelToCategory setCategory:](v7, "setCategory:", v5);
  return v7;
}

@end
