@implementation SKUISwooshPageComponent

- (SKUISwooshPageComponent)initWithCustomPageContext:(id)a3
{
  id v4;
  _BOOL8 v5;
  uint64_t v6;
  uint64_t v7;
  uint64_t v8;
  uint64_t v9;
  uint64_t v10;
  uint64_t v11;
  uint64_t v12;
  SKUISwooshPageComponent *v13;
  void *v14;
  void *v15;
  uint64_t v16;
  void *v17;
  void *v18;
  void *v19;
  __int128 v20;
  void *v21;
  uint64_t v22;
  NSString *title;
  void *v24;
  void *v25;
  void *v26;
  int64_t swooshType;
  uint64_t v28;
  uint64_t v29;
  void *v30;
  __int128 v32;
  unint64_t v33;
  objc_super v34;

  v4 = a3;
  if (os_variant_has_internal_content())
  {
    if (_os_feature_enabled_impl())
    {
      v5 = os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_FAULT);
      if (v5)
        -[SKUISwooshPageComponent initWithCustomPageContext:].cold.1(v5, v6, v7, v8, v9, v10, v11, v12);
    }
  }
  v34.receiver = self;
  v34.super_class = (Class)SKUISwooshPageComponent;
  v13 = -[SKUIPageComponent initWithCustomPageContext:](&v34, sel_initWithCustomPageContext_, v4);
  if (v13)
  {
    objc_msgSend(v4, "componentDictionary");
    v14 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v14, "objectForKey:", CFSTR("childType"));
    v15 = (void *)objc_claimAutoreleasedReturnValue();
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) == 0)
      goto LABEL_12;
    if ((objc_msgSend(v15, "isEqualToString:", CFSTR("brick")) & 1) != 0)
    {
      v16 = 1;
    }
    else
    {
      if (!objc_msgSend(v15, "isEqualToString:", CFSTR("media")))
        goto LABEL_12;
      v16 = 2;
    }
    v13->_swooshType = v16;
LABEL_12:
    objc_msgSend(v14, "objectForKey:", CFSTR("showBrickTitles"));
    v17 = (void *)objc_claimAutoreleasedReturnValue();

    if ((objc_opt_respondsToSelector() & 1) != 0)
    {
      v18 = v17;
    }
    else
    {
      objc_msgSend(v14, "objectForKey:", CFSTR("showItemTitles"));
      v18 = (void *)objc_claimAutoreleasedReturnValue();

      v17 = v18;
      if ((objc_opt_respondsToSelector() & 1) == 0)
        goto LABEL_16;
    }
    v13->_showsItemTitles = objc_msgSend(v17, "BOOLValue");
LABEL_16:
    objc_msgSend(v14, "objectForKey:", CFSTR("lockupStyle"));
    v19 = (void *)objc_claimAutoreleasedReturnValue();

    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
      SKUILockupStyleForDictionary(v19, v4, (uint64_t)&v32);
    else
      SKUILockupStyleDefault((uint64_t)&v32);
    v20 = v32;
    v13->_lockupStyle.visibleFields = v33;
    *(_OWORD *)&v13->_lockupStyle.artworkSize = v20;
    objc_msgSend(v14, "objectForKey:", CFSTR("title"));
    v21 = (void *)objc_claimAutoreleasedReturnValue();

    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
    {
      v22 = objc_msgSend(v21, "copy");
      title = v13->_title;
      v13->_title = (NSString *)v22;

    }
    objc_msgSend(v14, "objectForKey:", CFSTR("titleIsLink"));
    v24 = (void *)objc_claimAutoreleasedReturnValue();

    if ((objc_opt_respondsToSelector() & 1) != 0 && objc_msgSend(v24, "BOOLValue"))
      v13->_seeAllStyle = 1;
    objc_msgSend(v14, "objectForKey:", CFSTR("link"));
    v25 = (void *)objc_claimAutoreleasedReturnValue();
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
      -[SKUISwooshPageComponent _setSeeAllValuesWithLinkInfo:](v13, "_setSeeAllValuesWithLinkInfo:", v25);
    objc_msgSend(v14, "objectForKey:", CFSTR("children"));
    v26 = (void *)objc_claimAutoreleasedReturnValue();
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) == 0)
      goto LABEL_34;
    swooshType = v13->_swooshType;
    if (swooshType)
    {
      if (swooshType == 2)
      {
        -[SKUISwooshPageComponent _mediaComponentsWithChildren:context:](v13, "_mediaComponentsWithChildren:context:", v26, v4);
        v28 = objc_claimAutoreleasedReturnValue();
        v29 = 72;
      }
      else
      {
        if (swooshType != 1)
        {
LABEL_34:
          -[SKUISwooshPageComponent _reloadMissingDataCount](v13, "_reloadMissingDataCount");

          goto LABEL_35;
        }
        -[SKUISwooshPageComponent _brickItemsWithChildren:customPageContext:](v13, "_brickItemsWithChildren:customPageContext:", v26, v4);
        v28 = objc_claimAutoreleasedReturnValue();
        v29 = 24;
      }
    }
    else
    {
      -[SKUISwooshPageComponent _lockupsWithChildren:context:](v13, "_lockupsWithChildren:context:", v26, v4);
      v28 = objc_claimAutoreleasedReturnValue();
      v29 = 40;
    }
    v30 = *(Class *)((char *)&v13->super.super.isa + v29);
    *(Class *)((char *)&v13->super.super.isa + v29) = (Class)v28;

    goto LABEL_34;
  }
LABEL_35:

  return v13;
}

- (SKUISwooshPageComponent)initWithFeaturedContentContext:(id)a3 kind:(int64_t)a4
{
  id v6;
  _BOOL8 v7;
  uint64_t v8;
  uint64_t v9;
  uint64_t v10;
  uint64_t v11;
  uint64_t v12;
  uint64_t v13;
  uint64_t v14;
  SKUISwooshPageComponent *v15;
  void *v16;
  __int128 v17;
  void *v18;
  void *v19;
  void *v20;
  uint64_t v21;
  NSString *platformKeyProfile;
  void *v23;
  uint64_t v24;
  NSString *title;
  void *v26;
  void *v27;
  uint64_t v28;
  NSURL *seeAllURL;
  void *v30;
  void *v31;
  int64_t swooshType;
  uint64_t v33;
  uint64_t v34;
  void *v35;
  __int128 v37;
  unint64_t v38;
  objc_super v39;

  v6 = a3;
  if (os_variant_has_internal_content())
  {
    if (_os_feature_enabled_impl())
    {
      v7 = os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_FAULT);
      if (v7)
        -[SKUISwooshPageComponent initWithFeaturedContentContext:kind:].cold.1(v7, v8, v9, v10, v11, v12, v13, v14);
    }
  }
  v39.receiver = self;
  v39.super_class = (Class)SKUISwooshPageComponent;
  v15 = -[SKUIPageComponent initWithFeaturedContentContext:kind:](&v39, sel_initWithFeaturedContentContext_kind_, v6, a4);
  if (v15)
  {
    objc_msgSend(v6, "componentDictionary");
    v16 = (void *)objc_claimAutoreleasedReturnValue();
    if (a4 == 261)
      v15->_swooshType = 1;
    v15->_fcKind = a4;
    SKUILockupStyleDefault((uint64_t)&v37);
    v17 = v37;
    v15->_lockupStyle.visibleFields = v38;
    *(_OWORD *)&v15->_lockupStyle.artworkSize = v17;
    objc_msgSend(v6, "platformKeyProfileOverrides");
    v18 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(MEMORY[0x1E0CB3940], "stringWithFormat:", CFSTR("%ld"), v15->_fcKind);
    v19 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v18, "objectForKey:", v19);
    v20 = (void *)objc_claimAutoreleasedReturnValue();

    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
    {
      v21 = objc_msgSend(v20, "copy");
      platformKeyProfile = v15->_platformKeyProfile;
      v15->_platformKeyProfile = (NSString *)v21;

    }
    objc_msgSend(v16, "objectForKey:", CFSTR("name"));
    v23 = (void *)objc_claimAutoreleasedReturnValue();
    if (!v23)
    {
      objc_msgSend(v16, "objectForKey:", CFSTR("title"));
      v23 = (void *)objc_claimAutoreleasedReturnValue();
    }
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
    {
      v24 = objc_msgSend(v23, "copy");
      title = v15->_title;
      v15->_title = (NSString *)v24;

    }
    objc_msgSend(v16, "objectForKey:", CFSTR("titleIsLink"));
    v26 = (void *)objc_claimAutoreleasedReturnValue();

    if ((objc_opt_respondsToSelector() & 1) != 0 && objc_msgSend(v26, "BOOLValue"))
      v15->_seeAllStyle = 1;
    objc_msgSend(v16, "objectForKey:", CFSTR("seeAllUrl"));
    v27 = (void *)objc_claimAutoreleasedReturnValue();

    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
    {
      v28 = objc_msgSend(objc_alloc(MEMORY[0x1E0C99E98]), "initWithString:", v27);
      seeAllURL = v15->_seeAllURL;
      v15->_seeAllURL = (NSURL *)v28;

    }
    objc_msgSend(v16, "objectForKey:", CFSTR("showBrickTitles"));
    v30 = (void *)objc_claimAutoreleasedReturnValue();

    if ((objc_opt_respondsToSelector() & 1) != 0)
      v15->_showsItemTitles = objc_msgSend(v30, "BOOLValue");
    objc_msgSend(v16, "objectForKey:", CFSTR("children"));
    v31 = (void *)objc_claimAutoreleasedReturnValue();
    if (!v31)
    {
      objc_msgSend(v16, "objectForKey:", CFSTR("adamIds"));
      v31 = (void *)objc_claimAutoreleasedReturnValue();
      if (!v31)
      {
        objc_msgSend(v16, "objectForKey:", CFSTR("content"));
        v31 = (void *)objc_claimAutoreleasedReturnValue();
      }
    }
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) == 0)
      goto LABEL_30;
    swooshType = v15->_swooshType;
    if (swooshType)
    {
      if (swooshType != 1)
      {
LABEL_30:
        -[SKUISwooshPageComponent _reloadMissingDataCount](v15, "_reloadMissingDataCount");

        goto LABEL_31;
      }
      -[SKUISwooshPageComponent _brickItemsWithChildren:featuredPageContext:](v15, "_brickItemsWithChildren:featuredPageContext:", v31, v6);
      v33 = objc_claimAutoreleasedReturnValue();
      v34 = 24;
    }
    else
    {
      -[SKUISwooshPageComponent _lockupsWithChildren:featuredPageContext:](v15, "_lockupsWithChildren:featuredPageContext:", v31, v6);
      v33 = objc_claimAutoreleasedReturnValue();
      v34 = 40;
    }
    v35 = *(Class *)((char *)&v15->super.super.isa + v34);
    *(Class *)((char *)&v15->super.super.isa + v34) = (Class)v33;

    goto LABEL_30;
  }
LABEL_31:

  return v15;
}

- (SKUISwooshPageComponent)initWithItemList:(id)a3
{
  id v4;
  _BOOL8 v5;
  uint64_t v6;
  uint64_t v7;
  uint64_t v8;
  uint64_t v9;
  uint64_t v10;
  uint64_t v11;
  uint64_t v12;
  SKUISwooshPageComponent *v13;
  SKUISwooshPageComponent *v14;
  SKUILockupStyle *p_lockupStyle;
  __int128 v16;
  uint64_t v17;
  NSString *seeAllTitle;
  uint64_t v19;
  NSString *title;
  id v21;
  void *v22;
  uint64_t v23;
  uint64_t v24;
  uint64_t v25;
  uint64_t v26;
  uint64_t v27;
  __int128 v28;
  id v29;
  uint64_t v30;
  NSArray *lockups;
  void *v32;
  uint64_t v33;
  NSURL *seeAllURL;
  __int128 v36;
  __int128 v37;
  __int128 v38;
  __int128 v39;
  __int128 v40;
  unint64_t visibleFields;
  objc_super v42;
  _BYTE v43[128];
  uint64_t v44;

  v44 = *MEMORY[0x1E0C80C00];
  v4 = a3;
  if (os_variant_has_internal_content())
  {
    if (_os_feature_enabled_impl())
    {
      v5 = os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_FAULT);
      if (v5)
        -[SKUISwooshPageComponent initWithItemList:].cold.1(v5, v6, v7, v8, v9, v10, v11, v12);
    }
  }
  v42.receiver = self;
  v42.super_class = (Class)SKUISwooshPageComponent;
  v13 = -[SKUISwooshPageComponent init](&v42, sel_init);
  v14 = v13;
  if (v13)
  {
    v13->_swooshType = 0;
    p_lockupStyle = &v13->_lockupStyle;
    SKUILockupStyleDefault((uint64_t)&v40);
    v16 = v40;
    v14->_lockupStyle.visibleFields = visibleFields;
    *(_OWORD *)&p_lockupStyle->artworkSize = v16;
    objc_msgSend(v4, "seeAllTitle");
    v17 = objc_claimAutoreleasedReturnValue();
    seeAllTitle = v14->_seeAllTitle;
    v14->_seeAllTitle = (NSString *)v17;

    objc_msgSend(v4, "title");
    v19 = objc_claimAutoreleasedReturnValue();
    title = v14->_title;
    v14->_title = (NSString *)v19;

    v21 = objc_alloc_init(MEMORY[0x1E0C99DE8]);
    objc_msgSend(v4, "items");
    v22 = (void *)objc_claimAutoreleasedReturnValue();
    v36 = 0u;
    v37 = 0u;
    v38 = 0u;
    v39 = 0u;
    v23 = objc_msgSend(v22, "countByEnumeratingWithState:objects:count:", &v36, v43, 16);
    if (v23)
    {
      v24 = v23;
      v25 = *(_QWORD *)v37;
      do
      {
        v26 = 0;
        do
        {
          if (*(_QWORD *)v37 != v25)
            objc_enumerationMutation(v22);
          v27 = *(_QWORD *)(*((_QWORD *)&v36 + 1) + 8 * v26);
          v28 = *(_OWORD *)&p_lockupStyle->artworkSize;
          visibleFields = v14->_lockupStyle.visibleFields;
          v40 = v28;
          v29 = -[SKUISwooshPageComponent _newLockupComponentWithItem:defaultStyle:](v14, "_newLockupComponentWithItem:defaultStyle:", v27, &v40);
          objc_msgSend(v21, "addObject:", v29);

          ++v26;
        }
        while (v24 != v26);
        v24 = objc_msgSend(v22, "countByEnumeratingWithState:objects:count:", &v36, v43, 16);
      }
      while (v24);
    }
    v30 = objc_msgSend(v21, "copy");
    lockups = v14->_lockups;
    v14->_lockups = (NSArray *)v30;

    objc_msgSend(v4, "seeAllURLString");
    v32 = (void *)objc_claimAutoreleasedReturnValue();
    if (v32)
    {
      v33 = objc_msgSend(objc_alloc(MEMORY[0x1E0C99E98]), "initWithString:", v32);
      seeAllURL = v14->_seeAllURL;
      v14->_seeAllURL = (NSURL *)v33;

    }
    -[SKUISwooshPageComponent _reloadMissingDataCount](v14, "_reloadMissingDataCount");

  }
  return v14;
}

- (SKUISwooshPageComponent)initWithLockups:(id)a3 swooshType:(int64_t)a4 title:(id)a5
{
  id v8;
  id v9;
  _BOOL8 v10;
  uint64_t v11;
  uint64_t v12;
  uint64_t v13;
  uint64_t v14;
  uint64_t v15;
  uint64_t v16;
  uint64_t v17;
  SKUISwooshPageComponent *v18;
  uint64_t v19;
  NSArray *lockups;
  uint64_t v21;
  NSString *title;
  __int128 v24;
  unint64_t v25;
  objc_super v26;

  v8 = a3;
  v9 = a5;
  if (os_variant_has_internal_content())
  {
    if (_os_feature_enabled_impl())
    {
      v10 = os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_FAULT);
      if (v10)
        -[SKUISwooshPageComponent initWithLockups:swooshType:title:].cold.1(v10, v11, v12, v13, v14, v15, v16, v17);
    }
  }
  v26.receiver = self;
  v26.super_class = (Class)SKUISwooshPageComponent;
  v18 = -[SKUISwooshPageComponent init](&v26, sel_init);
  if (v18)
  {
    v19 = objc_msgSend(v8, "copy");
    lockups = v18->_lockups;
    v18->_lockups = (NSArray *)v19;

    v18->_swooshType = a4;
    -[SKUISwooshPageComponent _lockupStyleWithLockups:](v18, "_lockupStyleWithLockups:", v18->_lockups);
    v18->_lockupStyle.visibleFields = v25;
    *(_OWORD *)&v18->_lockupStyle.artworkSize = v24;
    v21 = objc_msgSend(v9, "copy");
    title = v18->_title;
    v18->_title = (NSString *)v21;

    -[SKUISwooshPageComponent _reloadMissingDataCount](v18, "_reloadMissingDataCount");
  }

  return v18;
}

- (SKUISwooshPageComponent)initWithRelatedContentContext:(id)a3
{
  id v4;
  _BOOL8 v5;
  uint64_t v6;
  uint64_t v7;
  uint64_t v8;
  uint64_t v9;
  uint64_t v10;
  uint64_t v11;
  uint64_t v12;
  SKUISwooshPageComponent *v13;
  void *v14;
  __int128 v15;
  void *v16;
  uint64_t v17;
  NSString *title;
  void *v19;
  void *v20;
  void *v21;
  uint64_t v22;
  NSArray *lockups;
  __int128 v25;
  unint64_t v26;
  objc_super v27;

  v4 = a3;
  if (os_variant_has_internal_content())
  {
    if (_os_feature_enabled_impl())
    {
      v5 = os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_FAULT);
      if (v5)
        -[SKUISwooshPageComponent initWithRelatedContentContext:].cold.1(v5, v6, v7, v8, v9, v10, v11, v12);
    }
  }
  v27.receiver = self;
  v27.super_class = (Class)SKUISwooshPageComponent;
  v13 = -[SKUISwooshPageComponent init](&v27, sel_init);
  if (v13)
  {
    objc_msgSend(v4, "componentDictionary");
    v14 = (void *)objc_claimAutoreleasedReturnValue();
    v13->_swooshType = 0;
    SKUILockupStyleDefault((uint64_t)&v25);
    v15 = v25;
    v13->_lockupStyle.visibleFields = v26;
    *(_OWORD *)&v13->_lockupStyle.artworkSize = v15;
    objc_msgSend(v14, "objectForKey:", CFSTR("title"));
    v16 = (void *)objc_claimAutoreleasedReturnValue();
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
    {
      v17 = objc_msgSend(v16, "copy");
      title = v13->_title;
      v13->_title = (NSString *)v17;

    }
    objc_msgSend(v14, "objectForKey:", CFSTR("titleIsLink"));
    v19 = (void *)objc_claimAutoreleasedReturnValue();

    if ((objc_opt_respondsToSelector() & 1) != 0 && objc_msgSend(v19, "BOOLValue"))
      v13->_seeAllStyle = 1;
    objc_msgSend(v14, "objectForKey:", CFSTR("link"));
    v20 = (void *)objc_claimAutoreleasedReturnValue();
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
      -[SKUISwooshPageComponent _setSeeAllValuesWithLinkInfo:](v13, "_setSeeAllValuesWithLinkInfo:", v20);
    objc_msgSend(v14, "objectForKey:", CFSTR("childrenIds"));
    v21 = (void *)objc_claimAutoreleasedReturnValue();
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0 && objc_msgSend(v21, "count"))
    {
      -[SKUISwooshPageComponent _lockupsWithChildren:context:](v13, "_lockupsWithChildren:context:", v21, v4);
      v22 = objc_claimAutoreleasedReturnValue();
      lockups = v13->_lockups;
      v13->_lockups = (NSArray *)v22;

    }
    -[SKUISwooshPageComponent _reloadMissingDataCount](v13, "_reloadMissingDataCount");

  }
  return v13;
}

- (SKUISwooshPageComponent)initWithRoomContext:(id)a3
{
  id v4;
  _BOOL8 v5;
  uint64_t v6;
  uint64_t v7;
  uint64_t v8;
  uint64_t v9;
  uint64_t v10;
  uint64_t v11;
  uint64_t v12;
  SKUISwooshPageComponent *v13;
  void *v14;
  __int128 v15;
  void *v16;
  uint64_t v17;
  NSString *title;
  void *v19;
  void *v20;
  void *v21;
  uint64_t v22;
  NSArray *lockups;
  __int128 v25;
  unint64_t v26;
  objc_super v27;

  v4 = a3;
  if (os_variant_has_internal_content())
  {
    if (_os_feature_enabled_impl())
    {
      v5 = os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_FAULT);
      if (v5)
        -[SKUISwooshPageComponent initWithRoomContext:].cold.1(v5, v6, v7, v8, v9, v10, v11, v12);
    }
  }
  v27.receiver = self;
  v27.super_class = (Class)SKUISwooshPageComponent;
  v13 = -[SKUIPageComponent initWithFeaturedContentContext:kind:](&v27, sel_initWithFeaturedContentContext_kind_, v4, 260);
  if (v13)
  {
    objc_msgSend(v4, "componentDictionary");
    v14 = (void *)objc_claimAutoreleasedReturnValue();
    v13->_swooshType = 0;
    SKUILockupStyleDefault((uint64_t)&v25);
    v15 = v25;
    v13->_lockupStyle.visibleFields = v26;
    *(_OWORD *)&v13->_lockupStyle.artworkSize = v15;
    objc_msgSend(v14, "objectForKey:", CFSTR("title"));
    v16 = (void *)objc_claimAutoreleasedReturnValue();
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
    {
      v17 = objc_msgSend(v16, "copy");
      title = v13->_title;
      v13->_title = (NSString *)v17;

    }
    objc_msgSend(v14, "objectForKey:", CFSTR("titleIsLink"));
    v19 = (void *)objc_claimAutoreleasedReturnValue();

    if ((objc_opt_respondsToSelector() & 1) != 0 && objc_msgSend(v19, "BOOLValue"))
      v13->_seeAllStyle = 1;
    objc_msgSend(v14, "objectForKey:", CFSTR("seeAllUrl"));
    v20 = (void *)objc_claimAutoreleasedReturnValue();
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
      -[SKUISwooshPageComponent _setSeeAllValuesWithLinkInfo:](v13, "_setSeeAllValuesWithLinkInfo:", v20);
    objc_msgSend(v14, "objectForKey:", CFSTR("adamIds"));
    v21 = (void *)objc_claimAutoreleasedReturnValue();
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0 && objc_msgSend(v21, "count"))
    {
      -[SKUISwooshPageComponent _lockupsWithChildren:context:](v13, "_lockupsWithChildren:context:", v21, v4);
      v22 = objc_claimAutoreleasedReturnValue();
      lockups = v13->_lockups;
      v13->_lockups = (NSArray *)v22;

    }
    -[SKUISwooshPageComponent _reloadMissingDataCount](v13, "_reloadMissingDataCount");

  }
  return v13;
}

- (BOOL)isMissingItemData
{
  return self->_missingDataCount > 0;
}

- (BOOL)showsBrickTitles
{
  return self->_showsItemTitles;
}

- (BOOL)showsMediaTitles
{
  return self->_showsItemTitles;
}

- (int64_t)componentType
{
  return 15;
}

- (void)enumerateMissingItemIdentifiersFromIndex:(int64_t)a3 usingBlock:(id)a4
{
  id v6;
  NSArray *lockups;
  NSArray *bricks;
  id v9;

  v6 = a4;
  lockups = self->_lockups;
  if (lockups)
  {
    v9 = v6;
    -[SKUIPageComponent _enumerateMissingItemIdentifiersFromLockups:startIndex:usingBlock:](self, "_enumerateMissingItemIdentifiersFromLockups:startIndex:usingBlock:", lockups, a3, v6);
LABEL_5:
    v6 = v9;
    goto LABEL_6;
  }
  bricks = self->_bricks;
  if (bricks)
  {
    v9 = v6;
    -[SKUIPageComponent _enumerateMissingItemIdentifiersFromBricks:startIndex:usingBlock:](self, "_enumerateMissingItemIdentifiersFromBricks:startIndex:usingBlock:", bricks, a3, v6);
    goto LABEL_5;
  }
LABEL_6:

}

- (id)metricsElementName
{
  return CFSTR("swoosh");
}

- (id)valueForMetricsField:(id)a3
{
  id v4;
  uint64_t v5;
  void *v6;
  objc_super v8;

  v4 = a3;
  if ((objc_msgSend(v4, "isEqualToString:", *MEMORY[0x1E0DAFC60]) & 1) != 0
    || objc_msgSend(v4, "isEqualToString:", *MEMORY[0x1E0DAFC68]))
  {
    -[SKUISwooshPageComponent title](self, "title");
    v5 = objc_claimAutoreleasedReturnValue();
  }
  else
  {
    if (!objc_msgSend(v4, "isEqualToString:", *MEMORY[0x1E0DAFC58]))
    {
LABEL_8:
      v8.receiver = self;
      v8.super_class = (Class)SKUISwooshPageComponent;
      -[SKUIPageComponent valueForMetricsField:](&v8, sel_valueForMetricsField_, v4);
      v6 = (void *)objc_claimAutoreleasedReturnValue();
      goto LABEL_9;
    }
    objc_msgSend(MEMORY[0x1E0CB37E8], "numberWithInteger:", self->_fcKind);
    v5 = objc_claimAutoreleasedReturnValue();
  }
  v6 = (void *)v5;
  if (!v5)
    goto LABEL_8;
LABEL_9:

  return v6;
}

- (void)_updateBricksWithItems:(id)a3
{
  id v4;
  NSArray *v5;
  uint64_t v6;
  uint64_t v7;
  uint64_t v8;
  uint64_t i;
  void *v10;
  void *v11;
  void *v12;
  uint64_t v13;
  void *v14;
  __int128 v15;
  __int128 v16;
  __int128 v17;
  __int128 v18;
  _BYTE v19[128];
  uint64_t v20;

  v20 = *MEMORY[0x1E0C80C00];
  v4 = a3;
  v15 = 0u;
  v16 = 0u;
  v17 = 0u;
  v18 = 0u;
  v5 = self->_bricks;
  v6 = -[NSArray countByEnumeratingWithState:objects:count:](v5, "countByEnumeratingWithState:objects:count:", &v15, v19, 16);
  if (v6)
  {
    v7 = v6;
    v8 = *(_QWORD *)v16;
    do
    {
      for (i = 0; i != v7; ++i)
      {
        if (*(_QWORD *)v16 != v8)
          objc_enumerationMutation(v5);
        v10 = *(void **)(*((_QWORD *)&v15 + 1) + 8 * i);
        objc_msgSend(v10, "link", (_QWORD)v15);
        v11 = (void *)objc_claimAutoreleasedReturnValue();
        if ((objc_msgSend(v11, "isActionable") & 1) == 0)
        {
          objc_msgSend(v11, "itemIdentifier");
          v12 = (void *)objc_claimAutoreleasedReturnValue();
          if (v12)
          {
            objc_msgSend(v4, "objectForKey:", v12);
            v13 = objc_claimAutoreleasedReturnValue();
            if (v13)
            {
              v14 = (void *)v13;
              objc_msgSend(v10, "_setLinkItem:", v13);
              --self->_missingDataCount;

            }
          }

        }
      }
      v7 = -[NSArray countByEnumeratingWithState:objects:count:](v5, "countByEnumeratingWithState:objects:count:", &v15, v19, 16);
    }
    while (v7);
  }

}

- (id)_updateLockupItemsWithItems:(id)a3
{
  id v4;
  NSArray *v5;
  uint64_t v6;
  uint64_t v7;
  uint64_t v8;
  uint64_t i;
  void *v10;
  void *v11;
  void *v12;
  id v14;
  __int128 v15;
  __int128 v16;
  __int128 v17;
  __int128 v18;
  _BYTE v19[128];
  uint64_t v20;

  v20 = *MEMORY[0x1E0C80C00];
  v4 = a3;
  objc_msgSend(MEMORY[0x1E0C99E20], "set");
  v14 = (id)objc_claimAutoreleasedReturnValue();
  v15 = 0u;
  v16 = 0u;
  v17 = 0u;
  v18 = 0u;
  v5 = self->_lockups;
  v6 = -[NSArray countByEnumeratingWithState:objects:count:](v5, "countByEnumeratingWithState:objects:count:", &v15, v19, 16);
  if (v6)
  {
    v7 = v6;
    v8 = *(_QWORD *)v16;
    do
    {
      for (i = 0; i != v7; ++i)
      {
        if (*(_QWORD *)v16 != v8)
          objc_enumerationMutation(v5);
        v10 = *(void **)(*((_QWORD *)&v15 + 1) + 8 * i);
        if (objc_msgSend(v10, "_needsItemData"))
        {
          v11 = (void *)objc_msgSend(objc_alloc(MEMORY[0x1E0CB37E8]), "initWithLongLong:", objc_msgSend(v10, "itemIdentifier"));
          objc_msgSend(v4, "objectForKey:", v11);
          v12 = (void *)objc_claimAutoreleasedReturnValue();
          if (v12)
          {
            -[SKUISwooshPageComponent _updateLockup:withItem:](self, "_updateLockup:withItem:", v10, v12);
            objc_msgSend(v14, "addObject:", v10);
            --self->_missingDataCount;
          }

        }
      }
      v7 = -[NSArray countByEnumeratingWithState:objects:count:](v5, "countByEnumeratingWithState:objects:count:", &v15, v19, 16);
    }
    while (v7);
  }

  return v14;
}

- (void)_updateLockupItemsWithLookupResponse:(id)a3
{
  NSArray *v4;
  uint64_t v5;
  uint64_t v6;
  uint64_t v7;
  uint64_t i;
  void *v9;
  void *v10;
  void *v11;
  SKUIItem *v12;
  void *v13;
  SKUIItem *v14;
  id v15;
  __int128 v16;
  __int128 v17;
  __int128 v18;
  __int128 v19;
  _BYTE v20[128];
  uint64_t v21;

  v21 = *MEMORY[0x1E0C80C00];
  v15 = a3;
  v16 = 0u;
  v17 = 0u;
  v18 = 0u;
  v19 = 0u;
  v4 = self->_lockups;
  v5 = -[NSArray countByEnumeratingWithState:objects:count:](v4, "countByEnumeratingWithState:objects:count:", &v16, v20, 16);
  if (v5)
  {
    v6 = v5;
    v7 = *(_QWORD *)v17;
    do
    {
      for (i = 0; i != v6; ++i)
      {
        if (*(_QWORD *)v17 != v7)
          objc_enumerationMutation(v4);
        v9 = *(void **)(*((_QWORD *)&v16 + 1) + 8 * i);
        if (objc_msgSend(v9, "_needsItemData"))
        {
          v10 = (void *)objc_msgSend(objc_alloc(MEMORY[0x1E0CB3940]), "initWithFormat:", CFSTR("%lld"), objc_msgSend(v9, "itemIdentifier"));
          objc_msgSend(v15, "itemForKey:", v10);
          v11 = (void *)objc_claimAutoreleasedReturnValue();
          if (v11)
          {
            v12 = [SKUIItem alloc];
            objc_msgSend(v11, "lookupDictionary");
            v13 = (void *)objc_claimAutoreleasedReturnValue();
            v14 = -[SKUIItem initWithLookupDictionary:](v12, "initWithLookupDictionary:", v13);

            -[SKUISwooshPageComponent _updateLockup:withItem:](self, "_updateLockup:withItem:", v9, v14);
            --self->_missingDataCount;

          }
        }
      }
      v6 = -[NSArray countByEnumeratingWithState:objects:count:](v4, "countByEnumeratingWithState:objects:count:", &v16, v20, 16);
    }
    while (v6);
  }

}

- (id)_brickItemsWithChildren:(id)a3 customPageContext:(id)a4
{
  id v6;
  id v7;
  void *v8;
  id v9;
  uint64_t v10;
  uint64_t v11;
  uint64_t v12;
  uint64_t v13;
  uint64_t v14;
  SKUIBrickItem *v15;
  id v17;
  __int128 v18;
  __int128 v19;
  __int128 v20;
  __int128 v21;
  _BYTE v22[128];
  uint64_t v23;

  v23 = *MEMORY[0x1E0C80C00];
  v6 = a3;
  v7 = a4;
  objc_msgSend(MEMORY[0x1E0C99DE8], "array");
  v17 = (id)objc_claimAutoreleasedReturnValue();
  v8 = (void *)objc_msgSend(v7, "copy");
  v18 = 0u;
  v19 = 0u;
  v20 = 0u;
  v21 = 0u;
  v9 = v6;
  v10 = objc_msgSend(v9, "countByEnumeratingWithState:objects:count:", &v18, v22, 16);
  if (v10)
  {
    v11 = v10;
    v12 = *(_QWORD *)v19;
    do
    {
      v13 = 0;
      do
      {
        if (*(_QWORD *)v19 != v12)
          objc_enumerationMutation(v9);
        v14 = *(_QWORD *)(*((_QWORD *)&v18 + 1) + 8 * v13);
        objc_opt_class();
        if ((objc_opt_isKindOfClass() & 1) != 0)
        {
          objc_msgSend(v8, "setComponentDictionary:", v14);
          v15 = -[SKUIBrickItem initWithCustomPageContext:]([SKUIBrickItem alloc], "initWithCustomPageContext:", v8);
          if (v15
            && -[SKUISwooshPageComponent _isBrickAvailable:withPageContext:](self, "_isBrickAvailable:withPageContext:", v15, v7))
          {
            objc_msgSend(v17, "addObject:", v15);
          }

        }
        ++v13;
      }
      while (v11 != v13);
      v11 = objc_msgSend(v9, "countByEnumeratingWithState:objects:count:", &v18, v22, 16);
    }
    while (v11);
  }

  return v17;
}

- (id)_brickItemsWithChildren:(id)a3 featuredPageContext:(id)a4
{
  id v6;
  id v7;
  void *v8;
  id v9;
  uint64_t v10;
  uint64_t v11;
  uint64_t v12;
  uint64_t v13;
  uint64_t v14;
  SKUIBrickItem *v15;
  id v17;
  __int128 v18;
  __int128 v19;
  __int128 v20;
  __int128 v21;
  _BYTE v22[128];
  uint64_t v23;

  v23 = *MEMORY[0x1E0C80C00];
  v6 = a3;
  v7 = a4;
  objc_msgSend(MEMORY[0x1E0C99DE8], "array");
  v17 = (id)objc_claimAutoreleasedReturnValue();
  v8 = (void *)objc_msgSend(v7, "copy");
  v18 = 0u;
  v19 = 0u;
  v20 = 0u;
  v21 = 0u;
  v9 = v6;
  v10 = objc_msgSend(v9, "countByEnumeratingWithState:objects:count:", &v18, v22, 16);
  if (v10)
  {
    v11 = v10;
    v12 = *(_QWORD *)v19;
    do
    {
      v13 = 0;
      do
      {
        if (*(_QWORD *)v19 != v12)
          objc_enumerationMutation(v9);
        v14 = *(_QWORD *)(*((_QWORD *)&v18 + 1) + 8 * v13);
        objc_opt_class();
        if ((objc_opt_isKindOfClass() & 1) != 0)
        {
          objc_msgSend(v8, "setComponentDictionary:", v14);
          v15 = -[SKUIBrickItem initWithComponentContext:]([SKUIBrickItem alloc], "initWithComponentContext:", v8);
          if (v15
            && -[SKUISwooshPageComponent _isBrickAvailable:withPageContext:](self, "_isBrickAvailable:withPageContext:", v15, v7))
          {
            objc_msgSend(v17, "addObject:", v15);
          }

        }
        ++v13;
      }
      while (v11 != v13);
      v11 = objc_msgSend(v9, "countByEnumeratingWithState:objects:count:", &v18, v22, 16);
    }
    while (v11);
  }

  return v17;
}

- (unint64_t)_defaultVisibleFieldsForItemKind:(int64_t)a3
{
  uint64_t v3;
  uint64_t v4;
  unint64_t result;
  _BYTE v6[16];
  unint64_t v7;
  uint64_t v8;
  uint64_t v9;

  switch(a3)
  {
    case 0:
    case 4:
    case 5:
    case 10:
    case 12:
    case 13:
    case 17:
    case 18:
      v8 = v3;
      v9 = v4;
      SKUILockupStyleDefault((uint64_t)v6);
      result = v7;
      break;
    case 1:
    case 2:
    case 3:
    case 7:
    case 8:
    case 9:
    case 11:
    case 16:
      result = 19;
      break;
    case 6:
      result = 18;
      break;
    case 14:
    case 15:
      result = 530;
      break;
    default:
      result = 0;
      break;
  }
  return result;
}

- (BOOL)_isBrickAvailable:(id)a3 withPageContext:(id)a4
{
  id v5;
  void *v6;
  void *v7;
  int v8;
  void *v9;

  v5 = a4;
  objc_msgSend(a3, "link");
  v6 = (void *)objc_claimAutoreleasedReturnValue();
  v7 = v6;
  if (v6)
  {
    if ((objc_msgSend(v6, "isActionable") & 1) != 0)
    {
      LOBYTE(v8) = 1;
    }
    else
    {
      objc_msgSend(v7, "itemIdentifier");
      v9 = (void *)objc_claimAutoreleasedReturnValue();
      if (v9)
        v8 = objc_msgSend(v5, "isUnavailableItemIdentifier:", v9) ^ 1;
      else
        LOBYTE(v8) = 0;

    }
  }
  else
  {
    LOBYTE(v8) = 0;
  }

  return v8;
}

- (SKUILockupStyle)_lockupStyleWithLockups:(SEL)a3
{
  id v6;
  id v7;
  id v8;
  uint64_t v9;
  uint64_t v10;
  uint64_t v11;
  uint64_t v12;
  void *v13;
  void *v14;
  SKUILockupStyle *result;
  __int128 v16;
  __int128 v17;
  __int128 v18;
  __int128 v19;
  _BYTE v20[128];
  uint64_t v21;

  v21 = *MEMORY[0x1E0C80C00];
  v6 = a4;
  retstr->layoutStyle = 0;
  retstr->visibleFields = 0;
  retstr->artworkSize = 0;
  SKUILockupStyleDefault((uint64_t)retstr);
  v7 = objc_alloc_init(MEMORY[0x1E0CB3788]);
  v16 = 0u;
  v17 = 0u;
  v18 = 0u;
  v19 = 0u;
  v8 = v6;
  v9 = objc_msgSend(v8, "countByEnumeratingWithState:objects:count:", &v16, v20, 16);
  if (v9)
  {
    v10 = v9;
    v11 = *(_QWORD *)v17;
    do
    {
      v12 = 0;
      do
      {
        if (*(_QWORD *)v17 != v11)
          objc_enumerationMutation(v8);
        objc_msgSend(*(id *)(*((_QWORD *)&v16 + 1) + 8 * v12), "item", (_QWORD)v16);
        v13 = (void *)objc_claimAutoreleasedReturnValue();
        v14 = v13;
        if (v13)
          objc_msgSend(v7, "addIndex:", -[SKUISwooshPageComponent _defaultVisibleFieldsForItemKind:](self, "_defaultVisibleFieldsForItemKind:", objc_msgSend(v13, "itemKind")));

        ++v12;
      }
      while (v10 != v12);
      v10 = objc_msgSend(v8, "countByEnumeratingWithState:objects:count:", &v16, v20, 16);
    }
    while (v10);
  }

  if (objc_msgSend(v7, "count") == 1)
    retstr->visibleFields = objc_msgSend(v7, "firstIndex");

  return result;
}

- (id)_lockupsWithChildren:(id)a3 context:(id)a4
{
  id v6;
  id v7;
  void *v8;
  id v9;
  uint64_t v10;
  uint64_t v11;
  uint64_t v12;
  uint64_t v13;
  uint64_t v14;
  void *v15;
  __int128 v17;
  __int128 v18;
  __int128 v19;
  __int128 v20;
  _BYTE v21[128];
  uint64_t v22;

  v22 = *MEMORY[0x1E0C80C00];
  v6 = a3;
  v7 = a4;
  objc_msgSend(MEMORY[0x1E0C99DE8], "array");
  v8 = (void *)objc_claimAutoreleasedReturnValue();
  v17 = 0u;
  v18 = 0u;
  v19 = 0u;
  v20 = 0u;
  v9 = v6;
  v10 = objc_msgSend(v9, "countByEnumeratingWithState:objects:count:", &v17, v21, 16);
  if (v10)
  {
    v11 = v10;
    v12 = *(_QWORD *)v18;
    do
    {
      v13 = 0;
      do
      {
        if (*(_QWORD *)v18 != v12)
          objc_enumerationMutation(v9);
        v14 = *(_QWORD *)(*((_QWORD *)&v17 + 1) + 8 * v13);
        objc_opt_class();
        if ((objc_opt_isKindOfClass() & 1) != 0)
        {
          -[SKUISwooshPageComponent _lockupWithItemIdentifier:context:](self, "_lockupWithItemIdentifier:context:", v14, v7, (_QWORD)v17);
          v15 = (void *)objc_claimAutoreleasedReturnValue();
          if (v15)
            objc_msgSend(v8, "addObject:", v15);

        }
        ++v13;
      }
      while (v11 != v13);
      v11 = objc_msgSend(v9, "countByEnumeratingWithState:objects:count:", &v17, v21, 16);
    }
    while (v11);
  }

  return v8;
}

- (id)_lockupsWithChildren:(id)a3 featuredPageContext:(id)a4
{
  id v5;
  id v6;
  id v7;
  uint64_t v8;
  uint64_t v9;
  uint64_t v10;
  uint64_t v11;
  void *v12;
  id v13;
  void *v14;
  void *v15;
  id v17;
  __int128 v19;
  __int128 v20;
  __int128 v21;
  __int128 v22;
  _BYTE v23[128];
  uint64_t v24;

  v24 = *MEMORY[0x1E0C80C00];
  v5 = a3;
  v6 = a4;
  objc_msgSend(MEMORY[0x1E0C99DE8], "array");
  v17 = (id)objc_claimAutoreleasedReturnValue();
  v19 = 0u;
  v20 = 0u;
  v21 = 0u;
  v22 = 0u;
  v7 = v5;
  v8 = objc_msgSend(v7, "countByEnumeratingWithState:objects:count:", &v19, v23, 16);
  if (v8)
  {
    v9 = v8;
    v10 = *(_QWORD *)v20;
    do
    {
      v11 = 0;
      do
      {
        if (*(_QWORD *)v20 != v10)
          objc_enumerationMutation(v7);
        v12 = *(void **)(*((_QWORD *)&v19 + 1) + 8 * v11);
        objc_opt_class();
        if ((objc_opt_isKindOfClass() & 1) != 0)
        {
          objc_msgSend(v12, "objectForKey:", CFSTR("adamId"));
          v13 = (id)objc_claimAutoreleasedReturnValue();
LABEL_10:
          v14 = v13;
          goto LABEL_12;
        }
        objc_opt_class();
        if ((objc_opt_isKindOfClass() & 1) != 0)
        {
          v13 = v12;
          goto LABEL_10;
        }
        v14 = 0;
LABEL_12:
        objc_opt_class();
        if ((objc_opt_isKindOfClass() & 1) != 0)
        {
          -[SKUISwooshPageComponent _lockupWithItemIdentifier:context:](self, "_lockupWithItemIdentifier:context:", v14, v6);
          v15 = (void *)objc_claimAutoreleasedReturnValue();
          if (v15)
            objc_msgSend(v17, "addObject:", v15);

        }
        ++v11;
      }
      while (v9 != v11);
      v9 = objc_msgSend(v7, "countByEnumeratingWithState:objects:count:", &v19, v23, 16);
    }
    while (v9);
  }

  return v17;
}

- (id)_lockupWithItemIdentifier:(id)a3 context:(id)a4
{
  id v6;
  id v7;
  void *v8;
  SKUILockupComponent *v9;
  SKUILockupComponent *v10;
  SKUILockupComponent *v11;
  uint64_t v12;
  __int128 v14;
  unint64_t visibleFields;

  v6 = a3;
  v7 = a4;
  objc_msgSend(v7, "itemForItemIdentifier:", v6);
  v8 = (void *)objc_claimAutoreleasedReturnValue();
  if (v8)
  {
    v14 = *(_OWORD *)&self->_lockupStyle.artworkSize;
    visibleFields = self->_lockupStyle.visibleFields;
    v9 = -[SKUISwooshPageComponent _newLockupComponentWithItem:defaultStyle:](self, "_newLockupComponentWithItem:defaultStyle:", v8, &v14);
LABEL_6:
    v10 = v9;
    goto LABEL_7;
  }
  if ((objc_msgSend(v7, "isUnavailableItemIdentifier:", v6) & 1) == 0)
  {
    v11 = [SKUILockupComponent alloc];
    v12 = objc_msgSend(v6, "longLongValue");
    v14 = *(_OWORD *)&self->_lockupStyle.artworkSize;
    visibleFields = self->_lockupStyle.visibleFields;
    v9 = -[SKUILockupComponent initWithItemIdentifier:style:](v11, "initWithItemIdentifier:style:", v12, &v14);
    goto LABEL_6;
  }
  v10 = 0;
LABEL_7:

  return v10;
}

- (id)_mediaComponentsWithChildren:(id)a3 context:(id)a4
{
  id v4;
  SKUIGalleryPageComponent *v5;
  void *v6;

  v4 = a4;
  v5 = -[SKUIGalleryPageComponent initWithCustomPageContext:]([SKUIGalleryPageComponent alloc], "initWithCustomPageContext:", v4);

  -[SKUIGalleryPageComponent childComponents](v5, "childComponents");
  v6 = (void *)objc_claimAutoreleasedReturnValue();

  return v6;
}

- (id)_newLockupComponentWithItem:(id)a3 defaultStyle:(SKUILockupStyle *)a4
{
  id v5;
  SKUILockupComponent *v6;
  SKUILockupComponent *v7;
  __int128 v9;
  unint64_t visibleFields;

  v5 = a3;
  if (objc_msgSend(v5, "itemKind") == 17)
    a4->visibleFields = 150;
  v6 = [SKUILockupComponent alloc];
  v9 = *(_OWORD *)&a4->artworkSize;
  visibleFields = a4->visibleFields;
  v7 = -[SKUILockupComponent initWithItem:style:](v6, "initWithItem:style:", v5, &v9);

  return v7;
}

- (void)_reloadMissingDataCount
{
  NSArray *v3;
  uint64_t v4;
  uint64_t v5;
  uint64_t v6;
  uint64_t v7;
  void *v8;
  void *v9;
  NSArray *v10;
  uint64_t v11;
  uint64_t v12;
  uint64_t v13;
  uint64_t v14;
  __int128 v15;
  __int128 v16;
  __int128 v17;
  __int128 v18;
  __int128 v19;
  __int128 v20;
  __int128 v21;
  __int128 v22;
  _BYTE v23[128];
  _BYTE v24[128];
  uint64_t v25;

  v25 = *MEMORY[0x1E0C80C00];
  self->_missingDataCount = 0;
  v19 = 0u;
  v20 = 0u;
  v21 = 0u;
  v22 = 0u;
  v3 = self->_bricks;
  v4 = -[NSArray countByEnumeratingWithState:objects:count:](v3, "countByEnumeratingWithState:objects:count:", &v19, v24, 16);
  if (v4)
  {
    v5 = v4;
    v6 = *(_QWORD *)v20;
    do
    {
      v7 = 0;
      do
      {
        if (*(_QWORD *)v20 != v6)
          objc_enumerationMutation(v3);
        objc_msgSend(*(id *)(*((_QWORD *)&v19 + 1) + 8 * v7), "link");
        v8 = (void *)objc_claimAutoreleasedReturnValue();
        if ((objc_msgSend(v8, "isActionable") & 1) == 0)
        {
          objc_msgSend(v8, "itemIdentifier");
          v9 = (void *)objc_claimAutoreleasedReturnValue();

          if (v9)
            ++self->_missingDataCount;
        }

        ++v7;
      }
      while (v5 != v7);
      v5 = -[NSArray countByEnumeratingWithState:objects:count:](v3, "countByEnumeratingWithState:objects:count:", &v19, v24, 16);
    }
    while (v5);
  }

  v17 = 0u;
  v18 = 0u;
  v15 = 0u;
  v16 = 0u;
  v10 = self->_lockups;
  v11 = -[NSArray countByEnumeratingWithState:objects:count:](v10, "countByEnumeratingWithState:objects:count:", &v15, v23, 16);
  if (v11)
  {
    v12 = v11;
    v13 = *(_QWORD *)v16;
    do
    {
      v14 = 0;
      do
      {
        if (*(_QWORD *)v16 != v13)
          objc_enumerationMutation(v10);
        if (objc_msgSend(*(id *)(*((_QWORD *)&v15 + 1) + 8 * v14), "_needsItemData", (_QWORD)v15))
          ++self->_missingDataCount;
        ++v14;
      }
      while (v12 != v14);
      v12 = -[NSArray countByEnumeratingWithState:objects:count:](v10, "countByEnumeratingWithState:objects:count:", &v15, v23, 16);
    }
    while (v12);
  }

}

- (void)_setSeeAllValuesWithLinkInfo:(id)a3
{
  void *v4;
  NSString *v5;
  NSString *seeAllTitle;
  void *v7;
  NSURL *v8;
  NSURL *seeAllURL;
  id v10;

  v10 = a3;
  objc_msgSend(v10, "objectForKey:", CFSTR("label"));
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  objc_opt_class();
  if ((objc_opt_isKindOfClass() & 1) != 0)
  {
    v5 = (NSString *)objc_msgSend(v4, "copy");
    seeAllTitle = self->_seeAllTitle;
    self->_seeAllTitle = v5;

  }
  objc_msgSend(v10, "objectForKey:", CFSTR("url"));
  v7 = (void *)objc_claimAutoreleasedReturnValue();

  objc_opt_class();
  if ((objc_opt_isKindOfClass() & 1) != 0)
  {
    v8 = (NSURL *)objc_msgSend(objc_alloc(MEMORY[0x1E0C99E98]), "initWithString:", v7);
    seeAllURL = self->_seeAllURL;
    self->_seeAllURL = v8;

  }
}

- (void)_updateLockup:(id)a3 withItem:(id)a4
{
  id v5;
  id v6;
  __int128 v7;
  uint64_t v8;
  _QWORD v9[3];
  __int128 v10;
  uint64_t v11;

  v5 = a3;
  v6 = a4;
  v10 = 0uLL;
  v11 = 0;
  if (v5)
    objc_msgSend(v5, "lockupStyle");
  if (objc_msgSend(v6, "itemKind") == 17)
  {
    SKUILockupStyleDefault((uint64_t)v9);
    v7 = v10;
    v8 = v11;
    if (SKUILockupStyleEqualToLockupStyle(v9, &v7))
    {
      v11 = 150;
      v7 = v10;
      v8 = 150;
      objc_msgSend(v5, "_setLockupStyle:", &v7);
    }
  }
  objc_msgSend(v5, "_setItem:", v6);

}

- (NSArray)bricks
{
  return self->_bricks;
}

- (NSArray)lockups
{
  return self->_lockups;
}

- (SKUILockupStyle)lockupStyle
{
  *retstr = self[2];
  return self;
}

- (NSArray)mediaComponents
{
  return self->_mediaComponents;
}

- (NSString)platformKeyProfile
{
  return self->_platformKeyProfile;
}

- (int64_t)seeAllStyle
{
  return self->_seeAllStyle;
}

- (NSString)seeAllTitle
{
  return self->_seeAllTitle;
}

- (NSURL)seeAllURL
{
  return self->_seeAllURL;
}

- (BOOL)showsIndexNumbers
{
  return self->_showsIndexNumbers;
}

- (int64_t)swooshType
{
  return self->_swooshType;
}

- (NSString)title
{
  return self->_title;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_title, 0);
  objc_storeStrong((id *)&self->_seeAllURL, 0);
  objc_storeStrong((id *)&self->_seeAllTitle, 0);
  objc_storeStrong((id *)&self->_platformKeyProfile, 0);
  objc_storeStrong((id *)&self->_mediaComponents, 0);
  objc_storeStrong((id *)&self->_lockups, 0);
  objc_storeStrong((id *)&self->_bricks, 0);
}

- (void)initWithCustomPageContext:(uint64_t)a3 .cold.1(uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4, uint64_t a5, uint64_t a6, uint64_t a7, uint64_t a8)
{
  OUTLINED_FUNCTION_1();
}

- (void)initWithFeaturedContentContext:(uint64_t)a3 kind:(uint64_t)a4 .cold.1(uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4, uint64_t a5, uint64_t a6, uint64_t a7, uint64_t a8)
{
  OUTLINED_FUNCTION_1();
}

- (void)initWithItemList:(uint64_t)a3 .cold.1(uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4, uint64_t a5, uint64_t a6, uint64_t a7, uint64_t a8)
{
  OUTLINED_FUNCTION_1();
}

- (void)initWithLockups:(uint64_t)a3 swooshType:(uint64_t)a4 title:(uint64_t)a5 .cold.1(uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4, uint64_t a5, uint64_t a6, uint64_t a7, uint64_t a8)
{
  OUTLINED_FUNCTION_1();
}

- (void)initWithRelatedContentContext:(uint64_t)a3 .cold.1(uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4, uint64_t a5, uint64_t a6, uint64_t a7, uint64_t a8)
{
  OUTLINED_FUNCTION_1();
}

- (void)initWithRoomContext:(uint64_t)a3 .cold.1(uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4, uint64_t a5, uint64_t a6, uint64_t a7, uint64_t a8)
{
  OUTLINED_FUNCTION_1();
}

@end
