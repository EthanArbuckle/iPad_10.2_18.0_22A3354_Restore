@implementation GEOPBOfflineDataConfiguration

- (GEOPBOfflineDataConfiguration)init
{
  GEOPBOfflineDataConfiguration *v2;
  GEOPBOfflineDataConfiguration *v3;
  GEOPBOfflineDataConfiguration *v4;
  objc_super v6;

  v6.receiver = self;
  v6.super_class = (Class)GEOPBOfflineDataConfiguration;
  v2 = -[GEOPBOfflineDataConfiguration init](&v6, sel_init);
  v3 = v2;
  if (v2)
  {
    v2->_readerLock._os_unfair_lock_opaque = 0;
    v4 = v2;
  }

  return v3;
}

- (GEOPBOfflineDataConfiguration)initWithData:(id)a3
{
  GEOPBOfflineDataConfiguration *v3;
  GEOPBOfflineDataConfiguration *v4;
  GEOPBOfflineDataConfiguration *v5;
  objc_super v7;

  v7.receiver = self;
  v7.super_class = (Class)GEOPBOfflineDataConfiguration;
  v3 = -[GEOPBOfflineDataConfiguration initWithData:](&v7, sel_initWithData_, a3);
  v4 = v3;
  if (v3)
  {
    v3->_readerLock._os_unfair_lock_opaque = 0;
    v5 = v3;
  }

  return v4;
}

- (void)dealloc
{
  objc_super v3;

  -[GEOPBOfflineDataConfiguration clearActiveVersions]((uint64_t)self);
  -[GEOPBOfflineDataConfiguration clearLatestAvailableVersions]((uint64_t)self);
  v3.receiver = self;
  v3.super_class = (Class)GEOPBOfflineDataConfiguration;
  -[GEOPBOfflineDataConfiguration dealloc](&v3, sel_dealloc);
}

- (void)clearActiveVersions
{
  if (a1)
  {
    os_unfair_lock_lock_with_options();
    *(_BYTE *)(a1 + 108) |= 8u;
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 104));
    *(_BYTE *)(a1 + 108) |= 0x40u;
    *(_QWORD *)(a1 + 40) = 0;
    *(_QWORD *)(a1 + 32) = 0;
    free(*(void **)(a1 + 24));
    *(_QWORD *)(a1 + 24) = 0;
  }
}

- (void)clearLatestAvailableVersions
{
  if (a1)
  {
    os_unfair_lock_lock_with_options();
    *(_BYTE *)(a1 + 108) |= 0x10u;
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 104));
    *(_BYTE *)(a1 + 108) |= 0x40u;
    *(_QWORD *)(a1 + 64) = 0;
    *(_QWORD *)(a1 + 56) = 0;
    free(*(void **)(a1 + 48));
    *(_QWORD *)(a1 + 48) = 0;
  }
}

- (void)_readActiveVersions
{
  void *v2;
  id v3;

  if (a1)
  {
    os_unfair_lock_lock((os_unfair_lock_t)(a1 + 104));
    if ((*(_BYTE *)(a1 + 108) & 8) == 0)
    {
      v2 = *(void **)(a1 + 8);
      if (v2)
      {
        v3 = v2;
        objc_sync_enter(v3);
        GEOPBOfflineDataConfigurationReadSpecified(a1, *(_QWORD *)(a1 + 8), (int *)&_readActiveVersions_tags);
        objc_sync_exit(v3);

      }
    }
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 104));
  }
}

- (uint64_t)_reserveActiveVersions:(uint64_t)a1
{
  uint64_t result;
  unint64_t v4;
  unint64_t v5;
  unint64_t v6;
  unint64_t v7;
  unint64_t v8;
  unint64_t v9;

  result = 0;
  if (a1 && a2)
  {
    v4 = *(_QWORD *)(a1 + 32);
    v5 = v4 + a2;
    if (v4 + a2 <= v4)
      return 0;
    v6 = *(_QWORD *)(a1 + 40);
    if (v6 >= v5)
      return 1;
    v7 = v6 + a2;
    if (v6 + a2)
    {
      v8 = 1;
      while (1)
      {
        v9 = v8;
        if (v8 >= v7)
          break;
        v8 *= 2;
        if ((v9 & 0x8000000000000000) != 0)
        {
          v9 = -1;
          goto LABEL_12;
        }
      }
    }
    else
    {
      v9 = 0;
    }
    if (v9 < a2)
      return 0;
LABEL_12:
    if (24 * v9 < v9)
      return 0;
    result = (uint64_t)malloc_type_realloc(*(void **)(a1 + 24), 24 * v9, 0xA08072F3uLL);
    if (result)
    {
      *(_QWORD *)(a1 + 40) = v9;
      *(_QWORD *)(a1 + 24) = result;
      bzero((void *)(result + 24 * *(_QWORD *)(a1 + 32)), 24 * (v9 - *(_QWORD *)(a1 + 32)));
      return 1;
    }
  }
  return result;
}

- (void)addActiveVersion:(uint64_t)a1
{
  __n128 v4;
  unint64_t v5;

  if (a1)
  {
    -[GEOPBOfflineDataConfiguration _readActiveVersions](a1);
    v4 = *a2;
    v5 = a2[1].n128_u64[0];
    -[GEOPBOfflineDataConfiguration _addNoFlagsActiveVersion:](a1, &v4);
    os_unfair_lock_lock_with_options();
    *(_BYTE *)(a1 + 108) |= 8u;
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 104));
    *(_BYTE *)(a1 + 108) |= 0x40u;
  }
}

- (__n128)_addNoFlagsActiveVersion:(uint64_t)a1
{
  __n128 result;
  uint64_t v5;
  unint64_t v6;

  if (a1)
  {
    if (-[GEOPBOfflineDataConfiguration _reserveActiveVersions:](a1, 1uLL))
    {
      v5 = *(_QWORD *)(a1 + 24) + 24 * *(_QWORD *)(a1 + 32);
      v6 = a2[1].n128_u64[0];
      result = *a2;
      *(__n128 *)v5 = *a2;
      *(_QWORD *)(v5 + 16) = v6;
      ++*(_QWORD *)(a1 + 32);
    }
  }
  return result;
}

- (void)_readLatestAvailableVersions
{
  void *v2;
  id v3;

  if (a1)
  {
    os_unfair_lock_lock((os_unfair_lock_t)(a1 + 104));
    if ((*(_BYTE *)(a1 + 108) & 0x10) == 0)
    {
      v2 = *(void **)(a1 + 8);
      if (v2)
      {
        v3 = v2;
        objc_sync_enter(v3);
        GEOPBOfflineDataConfigurationReadSpecified(a1, *(_QWORD *)(a1 + 8), (int *)&_readLatestAvailableVersions_tags);
        objc_sync_exit(v3);

      }
    }
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 104));
  }
}

- (uint64_t)_reserveLatestAvailableVersions:(uint64_t)a1
{
  uint64_t result;
  unint64_t v4;
  unint64_t v5;
  unint64_t v6;
  unint64_t v7;
  unint64_t v8;
  unint64_t v9;

  result = 0;
  if (a1 && a2)
  {
    v4 = *(_QWORD *)(a1 + 56);
    v5 = v4 + a2;
    if (v4 + a2 <= v4)
      return 0;
    v6 = *(_QWORD *)(a1 + 64);
    if (v6 >= v5)
      return 1;
    v7 = v6 + a2;
    if (v6 + a2)
    {
      v8 = 1;
      while (1)
      {
        v9 = v8;
        if (v8 >= v7)
          break;
        v8 *= 2;
        if ((v9 & 0x8000000000000000) != 0)
        {
          v9 = -1;
          goto LABEL_12;
        }
      }
    }
    else
    {
      v9 = 0;
    }
    if (v9 < a2)
      return 0;
LABEL_12:
    if (24 * v9 < v9)
      return 0;
    result = (uint64_t)malloc_type_realloc(*(void **)(a1 + 48), 24 * v9, 0xEE84AC6CuLL);
    if (result)
    {
      *(_QWORD *)(a1 + 64) = v9;
      *(_QWORD *)(a1 + 48) = result;
      bzero((void *)(result + 24 * *(_QWORD *)(a1 + 56)), 24 * (v9 - *(_QWORD *)(a1 + 56)));
      return 1;
    }
  }
  return result;
}

- (void)addLatestAvailableVersion:(uint64_t)a1
{
  __n128 v4;
  unint64_t v5;

  if (a1)
  {
    -[GEOPBOfflineDataConfiguration _readLatestAvailableVersions](a1);
    v4 = *a2;
    v5 = a2[1].n128_u64[0];
    -[GEOPBOfflineDataConfiguration _addNoFlagsLatestAvailableVersion:](a1, &v4);
    os_unfair_lock_lock_with_options();
    *(_BYTE *)(a1 + 108) |= 0x10u;
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 104));
    *(_BYTE *)(a1 + 108) |= 0x40u;
  }
}

- (__n128)_addNoFlagsLatestAvailableVersion:(uint64_t)a1
{
  __n128 result;
  uint64_t v5;
  unint64_t v6;

  if (a1)
  {
    if (-[GEOPBOfflineDataConfiguration _reserveLatestAvailableVersions:](a1, 1uLL))
    {
      v5 = *(_QWORD *)(a1 + 48) + 24 * *(_QWORD *)(a1 + 56);
      v6 = a2[1].n128_u64[0];
      result = *a2;
      *(__n128 *)v5 = *a2;
      *(_QWORD *)(v5 + 16) = v6;
      ++*(_QWORD *)(a1 + 56);
    }
  }
  return result;
}

- (void)_readTileMetadata
{
  void *v2;
  id v3;

  if (a1)
  {
    os_unfair_lock_lock((os_unfair_lock_t)(a1 + 104));
    if ((*(_BYTE *)(a1 + 108) & 0x20) == 0)
    {
      v2 = *(void **)(a1 + 8);
      if (v2)
      {
        v3 = v2;
        objc_sync_enter(v3);
        GEOPBOfflineDataConfigurationReadSpecified(a1, *(_QWORD *)(a1 + 8), (int *)&_readTileMetadata_tags);
        objc_sync_exit(v3);

      }
    }
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 104));
  }
}

- (id)tileMetadata
{
  id *v1;

  if (a1)
  {
    v1 = a1;
    -[GEOPBOfflineDataConfiguration _readTileMetadata]((uint64_t)a1);
    a1 = (id *)v1[11];
  }
  return a1;
}

- (void)setTileMetadata:(uint64_t)a1
{
  id v4;

  v4 = a2;
  if (a1)
  {
    *(_BYTE *)(a1 + 108) |= 0x20u;
    *(_BYTE *)(a1 + 108) |= 0x40u;
    objc_storeStrong((id *)(a1 + 88), a2);
  }

}

- (id)description
{
  void *v3;
  void *v4;
  void *v5;
  void *v6;
  objc_super v8;

  v3 = (void *)MEMORY[0x1E0CB3940];
  v8.receiver = self;
  v8.super_class = (Class)GEOPBOfflineDataConfiguration;
  -[GEOPBOfflineDataConfiguration description](&v8, sel_description);
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  -[GEOPBOfflineDataConfiguration dictionaryRepresentation](self, "dictionaryRepresentation");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v3, "stringWithFormat:", CFSTR("%@ %@"), v4, v5);
  v6 = (void *)objc_claimAutoreleasedReturnValue();

  return v6;
}

- (id)dictionaryRepresentation
{
  return -[GEOPBOfflineDataConfiguration _dictionaryRepresentation:]((uint64_t)self, 0);
}

- (id)_dictionaryRepresentation:(uint64_t)a1
{
  void *v4;
  void *v5;
  uint64_t v6;
  unint64_t v7;
  void *v8;
  const __CFString *v9;
  void *v10;
  const __CFString *v11;
  void *v12;
  uint64_t v13;
  unint64_t v14;
  void *v15;
  const __CFString *v16;
  void *v17;
  const __CFString *v18;
  void *v19;
  void *v20;
  void *v21;
  const __CFString *v22;
  void *v23;
  void *v24;
  void *v25;
  void *v26;
  id v27;
  id v28;
  _QWORD v30[4];
  id v31;

  if (!a1)
    return 0;
  -[GEOPBOfflineDataConfiguration readAll:](a1, 1);
  objc_msgSend(MEMORY[0x1E0C99E08], "dictionary");
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  if (*(_QWORD *)(a1 + 32))
  {
    v5 = (void *)objc_msgSend(objc_alloc(MEMORY[0x1E0C99DE8]), "initWithCapacity:", *(_QWORD *)(a1 + 32));
    if (*(_QWORD *)(a1 + 32))
    {
      v6 = 0;
      v7 = 0;
      do
      {
        _GEOPBOfflineDataLayerVersionDictionaryRepresentation((int *)(*(_QWORD *)(a1 + 24) + v6));
        v8 = (void *)objc_claimAutoreleasedReturnValue();
        objc_msgSend(v5, "addObject:", v8);

        ++v7;
        v6 += 24;
      }
      while (v7 < *(_QWORD *)(a1 + 32));
    }
    if (a2)
      v9 = CFSTR("activeVersion");
    else
      v9 = CFSTR("active_version");
    objc_msgSend(v4, "setObject:forKey:", v5, v9);

  }
  if ((*(_BYTE *)(a1 + 108) & 1) != 0)
  {
    objc_msgSend(MEMORY[0x1E0CB37E8], "numberWithUnsignedLongLong:", *(_QWORD *)(a1 + 72));
    v10 = (void *)objc_claimAutoreleasedReturnValue();
    if (a2)
      v11 = CFSTR("activeReleaseDataVersion");
    else
      v11 = CFSTR("active_release_data_version");
    objc_msgSend(v4, "setObject:forKey:", v10, v11);

  }
  if (*(_QWORD *)(a1 + 56))
  {
    v12 = (void *)objc_msgSend(objc_alloc(MEMORY[0x1E0C99DE8]), "initWithCapacity:", *(_QWORD *)(a1 + 56));
    if (*(_QWORD *)(a1 + 56))
    {
      v13 = 0;
      v14 = 0;
      do
      {
        _GEOPBOfflineDataLayerVersionDictionaryRepresentation((int *)(*(_QWORD *)(a1 + 48) + v13));
        v15 = (void *)objc_claimAutoreleasedReturnValue();
        objc_msgSend(v12, "addObject:", v15);

        ++v14;
        v13 += 24;
      }
      while (v14 < *(_QWORD *)(a1 + 56));
    }
    if (a2)
      v16 = CFSTR("latestAvailableVersion");
    else
      v16 = CFSTR("latest_available_version");
    objc_msgSend(v4, "setObject:forKey:", v12, v16);

  }
  if ((*(_BYTE *)(a1 + 108) & 2) != 0)
  {
    objc_msgSend(MEMORY[0x1E0CB37E8], "numberWithUnsignedLongLong:", *(_QWORD *)(a1 + 80));
    v17 = (void *)objc_claimAutoreleasedReturnValue();
    if (a2)
      v18 = CFSTR("lastUpdateTimestamp");
    else
      v18 = CFSTR("last_update_timestamp");
    objc_msgSend(v4, "setObject:forKey:", v17, v18);

  }
  -[GEOPBOfflineDataConfiguration tileMetadata]((id *)a1);
  v19 = (void *)objc_claimAutoreleasedReturnValue();
  v20 = v19;
  if (v19)
  {
    if ((a2 & 1) != 0)
    {
      objc_msgSend(v19, "jsonRepresentation");
      v21 = (void *)objc_claimAutoreleasedReturnValue();
      v22 = CFSTR("tileMetadata");
    }
    else
    {
      objc_msgSend(v19, "dictionaryRepresentation");
      v21 = (void *)objc_claimAutoreleasedReturnValue();
      v22 = CFSTR("tile_metadata");
    }
    objc_msgSend(v4, "setObject:forKey:", v21, v22);

  }
  v23 = *(void **)(a1 + 16);
  if (v23)
  {
    objc_msgSend(v23, "dictionaryRepresentation");
    v24 = (void *)objc_claimAutoreleasedReturnValue();
    v25 = v24;
    if (a2)
    {
      objc_msgSend(MEMORY[0x1E0C99E08], "dictionaryWithCapacity:", objc_msgSend(v24, "count"));
      v26 = (void *)objc_claimAutoreleasedReturnValue();
      v30[0] = MEMORY[0x1E0C809B0];
      v30[1] = 3221225472;
      v30[2] = __59__GEOPBOfflineDataConfiguration__dictionaryRepresentation___block_invoke;
      v30[3] = &unk_1E1C00600;
      v27 = v26;
      v31 = v27;
      objc_msgSend(v25, "enumerateKeysAndObjectsUsingBlock:", v30);
      v28 = v27;

      v25 = v28;
    }
    objc_msgSend(v4, "setObject:forKey:", v25, CFSTR("Unknown Fields"));

  }
  return v4;
}

- (id)jsonRepresentation
{
  return -[GEOPBOfflineDataConfiguration _dictionaryRepresentation:]((uint64_t)self, 1);
}

- (void)readAll:(uint64_t)a1
{
  void *v4;
  id v5;
  int *v6;

  if (a1)
  {
    os_unfair_lock_lock((os_unfair_lock_t)(a1 + 104));
    v4 = *(void **)(a1 + 8);
    if (v4)
    {
      v5 = v4;
      objc_sync_enter(v5);
      if (a2)
        v6 = (int *)&readAll__recursiveTag_238;
      else
        v6 = (int *)&readAll__nonRecursiveTag_239;
      GEOPBOfflineDataConfigurationReadSpecified(a1, *(_QWORD *)(a1 + 8), v6);
      objc_sync_exit(v5);

    }
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 104));
  }
}

void __59__GEOPBOfflineDataConfiguration__dictionaryRepresentation___block_invoke(uint64_t a1, void *a2, void *a3)
{
  id v5;
  char isKindOfClass;
  void *v7;
  void *v8;
  void *v9;
  id v10;

  v10 = a2;
  v5 = a3;
  objc_opt_class();
  isKindOfClass = objc_opt_isKindOfClass();
  v7 = *(void **)(a1 + 32);
  if ((isKindOfClass & 1) != 0)
  {
    objc_msgSend(v5, "base64EncodedStringWithOptions:", 0);
    v8 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v10, "description");
    v9 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v7, "setObject:forKey:", v8, v9);

  }
  else
  {
    objc_msgSend(v10, "description");
    v8 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v7, "setObject:forKey:", v5, v8);
  }

}

- (BOOL)readFrom:(id)a3
{
  return GEOPBOfflineDataConfigurationReadAllFrom((uint64_t)self, a3);
}

- (void)writeTo:(id)a3
{
  id v4;
  os_unfair_lock_s *p_readerLock;
  uint64_t v6;
  unint64_t v7;
  uint64_t v8;
  unint64_t v9;
  PBDataReader *v10;
  void *v11;

  v4 = a3;
  p_readerLock = &self->_readerLock;
  os_unfair_lock_lock(&self->_readerLock);
  if (self->_reader && (os_unfair_lock_assert_owner(&self->_readerLock), (*(_BYTE *)&self->_flags & 0x78) == 0))
  {
    v10 = self->_reader;
    objc_sync_enter(v10);
    *(_QWORD *)((char *)self->_reader + (int)*MEMORY[0x1E0D82BD8]) = self->_readerMarkLength;
    -[PBDataReader seekToOffset:](self->_reader, "seekToOffset:", self->_readerMarkPos);
    -[PBDataReader readBytes:](self->_reader, "readBytes:", self->_readerMarkLength - self->_readerMarkPos);
    v11 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v4, "writeData:", v11);
    os_unfair_lock_unlock(p_readerLock);

    objc_sync_exit(v10);
  }
  else
  {
    os_unfair_lock_unlock(&self->_readerLock);
    -[GEOPBOfflineDataConfiguration readAll:]((uint64_t)self, 0);
    if (self->_activeVersionsCount)
    {
      v6 = 0;
      v7 = 0;
      do
      {
        PBDataWriterPlaceMark();
        GEOPBOfflineDataLayerVersionWriteTo((uint64_t)&self->_activeVersions[v6]);
        PBDataWriterRecallMark();
        ++v7;
        ++v6;
      }
      while (v7 < self->_activeVersionsCount);
    }
    if ((*(_BYTE *)&self->_flags & 1) != 0)
      PBDataWriterWriteUint64Field();
    if (self->_latestAvailableVersionsCount)
    {
      v8 = 0;
      v9 = 0;
      do
      {
        PBDataWriterPlaceMark();
        GEOPBOfflineDataLayerVersionWriteTo((uint64_t)&self->_latestAvailableVersions[v8]);
        PBDataWriterRecallMark();
        ++v9;
        ++v8;
      }
      while (v9 < self->_latestAvailableVersionsCount);
    }
    if ((*(_BYTE *)&self->_flags & 2) != 0)
      PBDataWriterWriteUint64Field();
    if (self->_tileMetadata)
      PBDataWriterWriteSubmessage();
    -[PBUnknownFields writeTo:](self->_unknownFields, "writeTo:", v4);
  }

}

- (id)copyWithZone:(_NSZone *)a3
{
  uint64_t v5;
  PBDataReader *reader;
  PBDataReader *v7;
  id v8;
  unint64_t activeVersionsCount;
  unint64_t latestAvailableVersionsCount;
  id v11;
  void *v12;
  PBUnknownFields *v13;

  v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  os_unfair_lock_lock(&self->_readerLock);
  reader = self->_reader;
  if (reader)
  {
    v7 = reader;
    objc_sync_enter(v7);
    if ((*(_BYTE *)&self->_flags & 0x40) == 0)
    {
      *(_QWORD *)((char *)self->_reader + (int)*MEMORY[0x1E0D82BD8]) = self->_readerMarkLength;
      -[PBDataReader seekToOffset:](self->_reader, "seekToOffset:", self->_readerMarkPos);
      GEOPBOfflineDataConfigurationReadAllFrom(v5, self->_reader);
      os_unfair_lock_unlock(&self->_readerLock);
      v8 = (id)v5;
      objc_sync_exit(v7);

      goto LABEL_14;
    }
    objc_sync_exit(v7);

  }
  os_unfair_lock_unlock(&self->_readerLock);
  -[GEOPBOfflineDataConfiguration readAll:]((uint64_t)self, 0);
  activeVersionsCount = self->_activeVersionsCount;
  if (activeVersionsCount)
  {
    -[GEOPBOfflineDataConfiguration _reserveActiveVersions:](v5, activeVersionsCount);
    memcpy(*(void **)(v5 + 24), self->_activeVersions, 24 * self->_activeVersionsCount);
    *(_QWORD *)(v5 + 32) = self->_activeVersionsCount;
  }
  if ((*(_BYTE *)&self->_flags & 1) != 0)
  {
    *(_QWORD *)(v5 + 72) = self->_activeReleaseDataVersion;
    *(_BYTE *)(v5 + 108) |= 1u;
  }
  latestAvailableVersionsCount = self->_latestAvailableVersionsCount;
  if (latestAvailableVersionsCount)
  {
    -[GEOPBOfflineDataConfiguration _reserveLatestAvailableVersions:](v5, latestAvailableVersionsCount);
    memcpy(*(void **)(v5 + 48), self->_latestAvailableVersions, 24 * self->_latestAvailableVersionsCount);
    *(_QWORD *)(v5 + 56) = self->_latestAvailableVersionsCount;
  }
  if ((*(_BYTE *)&self->_flags & 2) != 0)
  {
    *(_QWORD *)(v5 + 80) = self->_lastUpdateTimestamp;
    *(_BYTE *)(v5 + 108) |= 2u;
  }
  v11 = -[GEOPBOfflineTileMetadata copyWithZone:](self->_tileMetadata, "copyWithZone:", a3);
  v12 = *(void **)(v5 + 88);
  *(_QWORD *)(v5 + 88) = v11;

  v13 = self->_unknownFields;
  v8 = *(id *)(v5 + 16);
  *(_QWORD *)(v5 + 16) = v13;
LABEL_14:

  return (id)v5;
}

- (BOOL)isEqual:(id)a3
{
  const void **v4;
  unint64_t activeVersionsCount;
  char v6;
  char flags;
  char v9;
  unint64_t latestAvailableVersionsCount;
  GEOPBOfflineTileMetadata *tileMetadata;

  v4 = (const void **)a3;
  if (!objc_msgSend(v4, "isMemberOfClass:", objc_opt_class()))
    goto LABEL_4;
  -[GEOPBOfflineDataConfiguration readAll:]((uint64_t)self, 1);
  -[GEOPBOfflineDataConfiguration readAll:]((uint64_t)v4, 1);
  activeVersionsCount = self->_activeVersionsCount;
  if ((const void *)activeVersionsCount != v4[4] || memcmp(self->_activeVersions, v4[3], 24 * activeVersionsCount))
    goto LABEL_4;
  flags = (char)self->_flags;
  v9 = *((_BYTE *)v4 + 108);
  if ((flags & 1) != 0)
  {
    if ((*((_BYTE *)v4 + 108) & 1) == 0 || (const void *)self->_activeReleaseDataVersion != v4[9])
      goto LABEL_4;
  }
  else if ((*((_BYTE *)v4 + 108) & 1) != 0)
  {
    goto LABEL_4;
  }
  latestAvailableVersionsCount = self->_latestAvailableVersionsCount;
  if ((const void *)latestAvailableVersionsCount != v4[7]
    || memcmp(self->_latestAvailableVersions, v4[6], 24 * latestAvailableVersionsCount))
  {
    goto LABEL_4;
  }
  if ((flags & 2) != 0)
  {
    if ((v9 & 2) != 0 && (const void *)self->_lastUpdateTimestamp == v4[10])
      goto LABEL_18;
LABEL_4:
    v6 = 0;
    goto LABEL_5;
  }
  if ((v9 & 2) != 0)
    goto LABEL_4;
LABEL_18:
  tileMetadata = self->_tileMetadata;
  if ((unint64_t)tileMetadata | (unint64_t)v4[11])
    v6 = -[GEOPBOfflineTileMetadata isEqual:](tileMetadata, "isEqual:");
  else
    v6 = 1;
LABEL_5:

  return v6;
}

- (unint64_t)hash
{
  uint64_t v3;
  unint64_t v4;
  uint64_t v5;
  unint64_t v6;

  -[GEOPBOfflineDataConfiguration readAll:]((uint64_t)self, 1);
  v3 = PBHashBytes();
  if ((*(_BYTE *)&self->_flags & 1) != 0)
    v4 = 2654435761u * self->_activeReleaseDataVersion;
  else
    v4 = 0;
  v5 = PBHashBytes();
  if ((*(_BYTE *)&self->_flags & 2) != 0)
    v6 = 2654435761u * self->_lastUpdateTimestamp;
  else
    v6 = 0;
  return v4 ^ v3 ^ v5 ^ v6 ^ -[GEOPBOfflineTileMetadata hash](self->_tileMetadata, "hash");
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_tileMetadata, 0);
  objc_storeStrong((id *)&self->_unknownFields, 0);
  objc_storeStrong((id *)&self->_reader, 0);
}

@end
