@implementation CRAudioStatus

- (CRAudioStatus)init
{
  CRAudioStatus *v2;
  const char *v3;
  uint64_t v4;
  CRAudioStatus *v5;
  objc_super v7;

  v7.receiver = self;
  v7.super_class = (Class)CRAudioStatus;
  v2 = -[CRComponentAuth init](&v7, sel_init);
  v5 = v2;
  if (v2)
    objc_msgSend_setComponentName_(v2, v3, (uint64_t)CFSTR("Audio"), v4);
  return v5;
}

- (int64_t)copyComponentStatus
{
  uint64_t v2;
  uint64_t v3;

  if (objc_msgSend_isComponentFailed(self, a2, v2, v3))
    return -1;
  else
    return 1;
}

- (BOOL)isComponentFailed
{
  const __CFAllocator *v2;
  const CFDictionaryKeyCallBacks *v3;
  const CFDictionaryValueCallBacks *v4;
  CFDictionaryRef v5;
  const __CFDictionary *v6;
  kern_return_t MatchingServices;
  kern_return_t v8;
  NSObject *v9;
  BOOL v10;
  NSObject *v12;
  io_iterator_t existing;
  uint8_t buf[4];
  kern_return_t v15;
  void *v16;
  void *v17;
  void *values;
  void *keys[2];

  keys[1] = *(void **)MEMORY[0x1E0C80C00];
  values = CFSTR("Speaker");
  keys[0] = CFSTR("AudioHardwareError");
  v2 = (const __CFAllocator *)*MEMORY[0x1E0C9AE00];
  v3 = (const CFDictionaryKeyCallBacks *)MEMORY[0x1E0C9B390];
  v4 = (const CFDictionaryValueCallBacks *)MEMORY[0x1E0C9B3A0];
  v5 = CFDictionaryCreate((CFAllocatorRef)*MEMORY[0x1E0C9AE00], (const void **)keys, (const void **)&values, 1, MEMORY[0x1E0C9B390], MEMORY[0x1E0C9B3A0]);
  v16 = v5;
  v17 = CFSTR("IOPropertyMatch");
  v6 = CFDictionaryCreate(v2, (const void **)&v17, (const void **)&v16, 1, v3, v4);
  existing = 0;
  MatchingServices = IOServiceGetMatchingServices(*MEMORY[0x1E0CBBAA8], v6, &existing);
  if (MatchingServices)
  {
    v8 = MatchingServices;
    handleForCategory();
    v9 = objc_claimAutoreleasedReturnValue();
    if (os_log_type_enabled(v9, OS_LOG_TYPE_DEFAULT))
    {
      *(_DWORD *)buf = 67109120;
      v15 = v8;
      _os_log_impl(&dword_1D1401000, v9, OS_LOG_TYPE_DEFAULT, "No AudioHardwareError Speaker IO registry, error: %d", buf, 8u);
    }

  }
  else if (existing)
  {
    handleForCategory();
    v12 = objc_claimAutoreleasedReturnValue();
    if (os_log_type_enabled(v12, OS_LOG_TYPE_ERROR))
      sub_1D140DC90();

    IOObjectRelease(existing);
    v10 = 1;
    if (v5)
      goto LABEL_6;
    return v10;
  }
  v10 = 0;
  if (v5)
LABEL_6:
    CFRelease(v5);
  return v10;
}

@end
