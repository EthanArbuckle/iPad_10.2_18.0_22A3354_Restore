...
+ (id)reachabilityWithAddress:(const sockaddr *)a3
{
  ...
  v4 = SCNetworkReachabilityCreateWithAddress((CFAllocatorRef)*MEMORY[0x24BDBD240], a3);
  if (v4)
  {
    v5 = v4;
    v4 = (SCNetworkReachabilityRef)objc_alloc_init((Class)a1);
...
- (void)dealloc
{
  ...
  -[APRKReachability stopNotifier](self, "stopNotifier");
  reachabilityRef = self->_reachabilityRef;
  if (reachabilityRef)
    CFRelease(reachabilityRef);
  v4.receiver = self;
  v4.super_class = (Class)APRKReachability;
  -[APRKReachability dealloc](&v4, sel_dealloc);
}
...
- (int64_t)currentReachabilityStatus
{
  ...
  if (SCNetworkReachabilityGetFlags(self->_reachabilityRef, &flags))
    return -[APRKReachability networkStatusForFlags:](self, "networkStatusForFlags:", flags);
  else
    return 0;
}
