@implementation SISchemaCarPlayHeadUnitContext

- (id)suppressMessageUnderConditions
{
  SISensitiveConditionsSet *v2;

  v2 = objc_alloc_init(SISensitiveConditionsSet);
  -[SISensitiveConditionsSet addCondition:](v2, "addCondition:", 1);
  return v2;
}

- (BOOL)hasVehicleName
{
  return self->_vehicleName != 0;
}

- (void)deleteVehicleName
{
  -[SISchemaCarPlayHeadUnitContext setVehicleName:](self, "setVehicleName:", 0);
}

- (BOOL)hasVehicleModel
{
  return self->_vehicleModel != 0;
}

- (void)deleteVehicleModel
{
  -[SISchemaCarPlayHeadUnitContext setVehicleModel:](self, "setVehicleModel:", 0);
}

- (BOOL)hasVehicleManufacturer
{
  return self->_vehicleManufacturer != 0;
}

- (void)deleteVehicleManufacturer
{
  -[SISchemaCarPlayHeadUnitContext setVehicleManufacturer:](self, "setVehicleManufacturer:", 0);
}

- (BOOL)readFrom:(id)a3
{
  return SISchemaCarPlayHeadUnitContextReadFrom(self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  void *v4;
  void *v5;
  void *v6;
  void *v7;
  id v8;

  v8 = a3;
  -[SISchemaCarPlayHeadUnitContext vehicleName](self, "vehicleName");
  v4 = (void *)objc_claimAutoreleasedReturnValue();

  if (v4)
    PBDataWriterWriteStringField();
  -[SISchemaCarPlayHeadUnitContext vehicleModel](self, "vehicleModel");
  v5 = (void *)objc_claimAutoreleasedReturnValue();

  if (v5)
    PBDataWriterWriteStringField();
  -[SISchemaCarPlayHeadUnitContext vehicleManufacturer](self, "vehicleManufacturer");
  v6 = (void *)objc_claimAutoreleasedReturnValue();

  v7 = v8;
  if (v6)
  {
    PBDataWriterWriteStringField();
    v7 = v8;
  }

}

- (BOOL)isEqual:(id)a3
{
  id v4;
  void *v5;
  void *v6;
  uint64_t v7;
  void *v8;
  void *v9;
  void *v10;
  int v11;
  uint64_t v12;
  void *v13;
  void *v14;
  void *v15;
  int v16;
  uint64_t v17;
  void *v18;
  void *v19;
  void *v20;
  char v21;
  BOOL v22;

  v4 = a3;
  if (!objc_msgSend(v4, "isMemberOfClass:", objc_opt_class()))
    goto LABEL_17;
  -[SISchemaCarPlayHeadUnitContext vehicleName](self, "vehicleName");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "vehicleName");
  v6 = (void *)objc_claimAutoreleasedReturnValue();
  if ((v5 == 0) == (v6 != 0))
    goto LABEL_16;
  -[SISchemaCarPlayHeadUnitContext vehicleName](self, "vehicleName");
  v7 = objc_claimAutoreleasedReturnValue();
  if (v7)
  {
    v8 = (void *)v7;
    -[SISchemaCarPlayHeadUnitContext vehicleName](self, "vehicleName");
    v9 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v4, "vehicleName");
    v10 = (void *)objc_claimAutoreleasedReturnValue();
    v11 = objc_msgSend(v9, "isEqual:", v10);

    if (!v11)
      goto LABEL_17;
  }
  else
  {

  }
  -[SISchemaCarPlayHeadUnitContext vehicleModel](self, "vehicleModel");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "vehicleModel");
  v6 = (void *)objc_claimAutoreleasedReturnValue();
  if ((v5 == 0) == (v6 != 0))
    goto LABEL_16;
  -[SISchemaCarPlayHeadUnitContext vehicleModel](self, "vehicleModel");
  v12 = objc_claimAutoreleasedReturnValue();
  if (v12)
  {
    v13 = (void *)v12;
    -[SISchemaCarPlayHeadUnitContext vehicleModel](self, "vehicleModel");
    v14 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v4, "vehicleModel");
    v15 = (void *)objc_claimAutoreleasedReturnValue();
    v16 = objc_msgSend(v14, "isEqual:", v15);

    if (!v16)
      goto LABEL_17;
  }
  else
  {

  }
  -[SISchemaCarPlayHeadUnitContext vehicleManufacturer](self, "vehicleManufacturer");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "vehicleManufacturer");
  v6 = (void *)objc_claimAutoreleasedReturnValue();
  if ((v5 == 0) != (v6 != 0))
  {
    -[SISchemaCarPlayHeadUnitContext vehicleManufacturer](self, "vehicleManufacturer");
    v17 = objc_claimAutoreleasedReturnValue();
    if (!v17)
    {

LABEL_20:
      v22 = 1;
      goto LABEL_18;
    }
    v18 = (void *)v17;
    -[SISchemaCarPlayHeadUnitContext vehicleManufacturer](self, "vehicleManufacturer");
    v19 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v4, "vehicleManufacturer");
    v20 = (void *)objc_claimAutoreleasedReturnValue();
    v21 = objc_msgSend(v19, "isEqual:", v20);

    if ((v21 & 1) != 0)
      goto LABEL_20;
  }
  else
  {
LABEL_16:

  }
LABEL_17:
  v22 = 0;
LABEL_18:

  return v22;
}

- (unint64_t)hash
{
  NSUInteger v3;
  NSUInteger v4;

  v3 = -[NSString hash](self->_vehicleName, "hash");
  v4 = -[NSString hash](self->_vehicleModel, "hash") ^ v3;
  return v4 ^ -[NSString hash](self->_vehicleManufacturer, "hash");
}

- (id)dictionaryRepresentation
{
  void *v3;
  void *v4;
  void *v5;
  void *v6;
  void *v7;
  void *v8;
  void *v9;

  objc_msgSend(MEMORY[0x1E0C99E08], "dictionary");
  v3 = (void *)objc_claimAutoreleasedReturnValue();
  if (self->_vehicleManufacturer)
  {
    -[SISchemaCarPlayHeadUnitContext vehicleManufacturer](self, "vehicleManufacturer");
    v4 = (void *)objc_claimAutoreleasedReturnValue();
    v5 = (void *)objc_msgSend(v4, "copy");
    objc_msgSend(v3, "setObject:forKeyedSubscript:", v5, CFSTR("vehicleManufacturer"));

  }
  if (self->_vehicleModel)
  {
    -[SISchemaCarPlayHeadUnitContext vehicleModel](self, "vehicleModel");
    v6 = (void *)objc_claimAutoreleasedReturnValue();
    v7 = (void *)objc_msgSend(v6, "copy");
    objc_msgSend(v3, "setObject:forKeyedSubscript:", v7, CFSTR("vehicleModel"));

  }
  if (self->_vehicleName)
  {
    -[SISchemaCarPlayHeadUnitContext vehicleName](self, "vehicleName");
    v8 = (void *)objc_claimAutoreleasedReturnValue();
    v9 = (void *)objc_msgSend(v8, "copy");
    objc_msgSend(v3, "setObject:forKeyedSubscript:", v9, CFSTR("vehicleName"));

  }
  -[SISchemaInstrumentationMessage willProduceDictionaryRepresentation:](self, "willProduceDictionaryRepresentation:", v3);
  return v3;
}

- (NSData)jsonData
{
  void *v2;
  void *v3;

  -[SISchemaCarPlayHeadUnitContext dictionaryRepresentation](self, "dictionaryRepresentation");
  v2 = (void *)objc_claimAutoreleasedReturnValue();
  if (objc_msgSend(MEMORY[0x1E0CB36D8], "isValidJSONObject:", v2))
  {
    objc_msgSend(MEMORY[0x1E0CB36D8], "dataWithJSONObject:options:error:", v2, 0, 0);
    v3 = (void *)objc_claimAutoreleasedReturnValue();
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (SISchemaCarPlayHeadUnitContext)initWithJSON:(id)a3
{
  void *v4;
  SISchemaCarPlayHeadUnitContext *v5;
  uint64_t v7;

  v7 = 0;
  objc_msgSend(MEMORY[0x1E0CB36D8], "JSONObjectWithData:options:error:", a3, 0, &v7);
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = -[SISchemaCarPlayHeadUnitContext initWithDictionary:](self, "initWithDictionary:", v4);
    v5 = self;
  }

  return v5;
}

- (SISchemaCarPlayHeadUnitContext)initWithDictionary:(id)a3
{
  id v4;
  SISchemaCarPlayHeadUnitContext *v5;
  void *v6;
  void *v7;
  void *v8;
  void *v9;
  void *v10;
  void *v11;
  SISchemaCarPlayHeadUnitContext *v12;
  objc_super v14;

  v4 = a3;
  v14.receiver = self;
  v14.super_class = (Class)SISchemaCarPlayHeadUnitContext;
  v5 = -[SISchemaCarPlayHeadUnitContext init](&v14, sel_init);
  if (v5)
  {
    objc_msgSend(v4, "objectForKeyedSubscript:", CFSTR("vehicleName"));
    v6 = (void *)objc_claimAutoreleasedReturnValue();
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
    {
      v7 = (void *)objc_msgSend(v6, "copy");
      -[SISchemaCarPlayHeadUnitContext setVehicleName:](v5, "setVehicleName:", v7);

    }
    objc_msgSend(v4, "objectForKeyedSubscript:", CFSTR("vehicleModel"));
    v8 = (void *)objc_claimAutoreleasedReturnValue();
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
    {
      v9 = (void *)objc_msgSend(v8, "copy");
      -[SISchemaCarPlayHeadUnitContext setVehicleModel:](v5, "setVehicleModel:", v9);

    }
    objc_msgSend(v4, "objectForKeyedSubscript:", CFSTR("vehicleManufacturer"));
    v10 = (void *)objc_claimAutoreleasedReturnValue();
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0)
    {
      v11 = (void *)objc_msgSend(v10, "copy");
      -[SISchemaCarPlayHeadUnitContext setVehicleManufacturer:](v5, "setVehicleManufacturer:", v11);

    }
    v12 = v5;

  }
  return v5;
}

- (NSString)vehicleName
{
  return self->_vehicleName;
}

- (void)setVehicleName:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 8);
}

- (NSString)vehicleModel
{
  return self->_vehicleModel;
}

- (void)setVehicleModel:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 16);
}

- (NSString)vehicleManufacturer
{
  return self->_vehicleManufacturer;
}

- (void)setVehicleManufacturer:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 24);
}

- (void)setHasVehicleName:(BOOL)a3
{
  self->_hasVehicleName = a3;
}

- (void)setHasVehicleModel:(BOOL)a3
{
  self->_hasVehicleModel = a3;
}

- (void)setHasVehicleManufacturer:(BOOL)a3
{
  self->_hasVehicleManufacturer = a3;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_vehicleManufacturer, 0);
  objc_storeStrong((id *)&self->_vehicleModel, 0);
  objc_storeStrong((id *)&self->_vehicleName, 0);
}

@end
