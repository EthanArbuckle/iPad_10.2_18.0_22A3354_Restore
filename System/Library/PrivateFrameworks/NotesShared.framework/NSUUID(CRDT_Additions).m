...
+ (id)CR_unserialized
{
  if (CR_unserialized_onceToken != -1)
    dispatch_once(&CR_unserialized_onceToken, &__block_literal_global_3);
  return (id)CR_unserialized_unserialized;
}

+ (id)CR_unknown
{
  if (CR_unknown_onceToken != -1)
    dispatch_once(&CR_unknown_onceToken, &__block_literal_global_4);
  return (id)CR_unknown_unserialized;
}
