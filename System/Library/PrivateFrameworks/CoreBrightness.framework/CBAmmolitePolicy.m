@implementation CBAmmolitePolicy

- (id)isEnabledPropertyKey
{
  return CFSTR("CBAmmoliteEnabled");
}

- (BOOL)luxIsInActiveRange:(float)a3
{
  float v3;

  -[CBChromaticCorrectionParams luxActivationThreshold](self->_params, "luxActivationThreshold");
  return a3 >= v3;
}

- (void)dealloc
{
  objc_super v2;
  SEL v3;
  CBAmmolitePolicy *v4;

  v4 = self;
  v3 = a2;

  v4->_params = 0;
  v2.receiver = v4;
  v2.super_class = (Class)CBAmmolitePolicy;
  -[CBAmmolitePolicy dealloc](&v2, sel_dealloc);
}

- (const)name
{
  return "Ammolite";
}

- (id)strengthNotification
{
  return CFSTR("AmmoliteStrength");
}

- (BOOL)nitsAreInActiveRange:(float)a3
{
  float v3;

  -[CBChromaticCorrectionParams nitsActivationThreshold](self->_params, "nitsActivationThreshold");
  return a3 >= v3;
}

- (float)cappedRampStartLux:(float)a3
{
  float v3;

  -[CBChromaticCorrectionParams luxActivationThreshold](self->_params, "luxActivationThreshold");
  return fmaxf(a3, v3);
}

- (float)cappedRampTargetLux:(float)a3
{
  float v3;

  -[CBAmmolitePolicy rampTargetLuxCap](self, "rampTargetLuxCap");
  return fmaxf(a3, v3);
}

- (float)rampTargetLuxCap
{
  float v2;
  float v3;
  float v5;

  -[CBChromaticCorrectionParams luxActivationThreshold](self->_params, "luxActivationThreshold");
  v5 = v2;
  -[CBChromaticCorrectionParams rampUpLuxDeltaThreshold](self->_params, "rampUpLuxDeltaThreshold");
  return v5 - v3;
}

- (id)rampIdentifier
{
  return CFSTR("Ammolite");
}

- (CBChromaticCorrectionParams)params
{
  return (CBChromaticCorrectionParams *)objc_getProperty(self, a2, 8, 1);
}

- (void)setParams:(id)a3
{
  objc_setProperty_atomic(self, a2, a3, 8);
}

@end
