@implementation _MRSendHIDReportMessageProtobuf

- (BOOL)hasVirtualDeviceID
{
  return self->_virtualDeviceID != 0;
}

- (BOOL)hasReport
{
  return self->_report != 0;
}

- (id)description
{
  void *v3;
  void *v4;
  void *v5;
  void *v6;
  objc_super v8;

  v3 = (void *)MEMORY[0x1E0CB3940];
  v8.receiver = self;
  v8.super_class = (Class)_MRSendHIDReportMessageProtobuf;
  -[_MRSendHIDReportMessageProtobuf description](&v8, sel_description);
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  -[_MRSendHIDReportMessageProtobuf dictionaryRepresentation](self, "dictionaryRepresentation");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v3, "stringWithFormat:", CFSTR("%@ %@"), v4, v5);
  v6 = (void *)objc_claimAutoreleasedReturnValue();

  return v6;
}

- (id)dictionaryRepresentation
{
  void *v3;
  void *v4;
  NSString *virtualDeviceID;
  NSData *report;

  objc_msgSend(MEMORY[0x1E0C99E08], "dictionary");
  v3 = (void *)objc_claimAutoreleasedReturnValue();
  v4 = v3;
  virtualDeviceID = self->_virtualDeviceID;
  if (virtualDeviceID)
    objc_msgSend(v3, "setObject:forKey:", virtualDeviceID, CFSTR("virtualDeviceID"));
  report = self->_report;
  if (report)
    objc_msgSend(v4, "setObject:forKey:", report, CFSTR("report"));
  return v4;
}

- (BOOL)readFrom:(id)a3
{
  return _MRSendHIDReportMessageProtobufReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  id v4;
  id v5;

  v4 = a3;
  v5 = v4;
  if (self->_virtualDeviceID)
  {
    PBDataWriterWriteStringField();
    v4 = v5;
  }
  if (self->_report)
  {
    PBDataWriterWriteDataField();
    v4 = v5;
  }

}

- (void)copyTo:(id)a3
{
  id v4;
  id v5;

  v4 = a3;
  v5 = v4;
  if (self->_virtualDeviceID)
  {
    objc_msgSend(v4, "setVirtualDeviceID:");
    v4 = v5;
  }
  if (self->_report)
  {
    objc_msgSend(v5, "setReport:");
    v4 = v5;
  }

}

- (id)copyWithZone:(_NSZone *)a3
{
  _QWORD *v5;
  uint64_t v6;
  void *v7;
  uint64_t v8;
  void *v9;

  v5 = (_QWORD *)objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  v6 = -[NSString copyWithZone:](self->_virtualDeviceID, "copyWithZone:", a3);
  v7 = (void *)v5[2];
  v5[2] = v6;

  v8 = -[NSData copyWithZone:](self->_report, "copyWithZone:", a3);
  v9 = (void *)v5[1];
  v5[1] = v8;

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  _QWORD *v4;
  NSString *virtualDeviceID;
  NSData *report;
  char v7;

  v4 = a3;
  if (objc_msgSend(v4, "isMemberOfClass:", objc_opt_class())
    && ((virtualDeviceID = self->_virtualDeviceID, !((unint64_t)virtualDeviceID | v4[2]))
     || -[NSString isEqual:](virtualDeviceID, "isEqual:")))
  {
    report = self->_report;
    if ((unint64_t)report | v4[1])
      v7 = -[NSData isEqual:](report, "isEqual:");
    else
      v7 = 1;
  }
  else
  {
    v7 = 0;
  }

  return v7;
}

- (unint64_t)hash
{
  NSUInteger v3;

  v3 = -[NSString hash](self->_virtualDeviceID, "hash");
  return -[NSData hash](self->_report, "hash") ^ v3;
}

- (void)mergeFrom:(id)a3
{
  _QWORD *v4;
  _QWORD *v5;

  v4 = a3;
  v5 = v4;
  if (v4[2])
  {
    -[_MRSendHIDReportMessageProtobuf setVirtualDeviceID:](self, "setVirtualDeviceID:");
    v4 = v5;
  }
  if (v4[1])
  {
    -[_MRSendHIDReportMessageProtobuf setReport:](self, "setReport:");
    v4 = v5;
  }

}

- (NSString)virtualDeviceID
{
  return self->_virtualDeviceID;
}

- (void)setVirtualDeviceID:(id)a3
{
  objc_storeStrong((id *)&self->_virtualDeviceID, a3);
}

- (NSData)report
{
  return self->_report;
}

- (void)setReport:(id)a3
{
  objc_storeStrong((id *)&self->_report, a3);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_virtualDeviceID, 0);
  objc_storeStrong((id *)&self->_report, 0);
}

@end
