...
{
  return (NSString *)objc_msgSend(MEMORY[0x1E0CB3940], "stringWithFormat:", CFSTR("%@:%lld"), self->_zoneRowID, self->_intID);
}

- (id)matchingJobsWhereSQLClause
{
  return (id)objc_msgSend(MEMORY[0x1E0DE81D0], "formatInjection:", CFSTR("throttle_id = %lld AND zone_rowid = %@"), self->_intID, self->_zoneRowID);
}

- (id)columns
{
  return (id)objc_msgSend(MEMORY[0x1E0DE81E0], "rawInjection:length:", "throttle_id, zone_rowid", 23);
}

- (id)columnsValues
{
  return (id)objc_msgSend(MEMORY[0x1E0DE81D0], "formatInjection:", CFSTR("%lld, %@"), self->_intID, self->_zoneRowID);
}
...
- (void)initWithIntID:(uint64_t)a1 zoneRowID:(NSObject *)a2 .cold.1(uint64_t a1, NSObject *a2)
{
  ...
  v2 = 138412290;
  v3 = a1;
  _os_log_fault_impl(&dword_1CBD43000, a2, OS_LOG_TYPE_FAULT, "[CRIT] Assertion failed: zoneRowID%@", (uint8_t *)&v2, 0xCu);
}
