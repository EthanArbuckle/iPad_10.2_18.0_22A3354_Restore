...
{
  NTFeedTransformationLimit *result;
  objc_super v5;

  v5.receiver = self;
  v5.super_class = (Class)NTFeedTransformationLimit;
  result = -[NTFeedTransformationLimit init](&v5, sel_init);
  if (result)
    result->_limit = a3;
  return result;
}
...
- (NTFeedTransformationLimit)init
{
  ...
  objc_msgSend(MEMORY[0x24BDD17C8], "stringWithFormat:", CFSTR("%@: %s"), CFSTR("Do not call method"), "-[NTFeedTransformationLimit init]");
...
- (id)transformFeedItems:(id)a3
{
  ...
  if (!v4 && os_log_type_enabled(MEMORY[0x24BDACB70], OS_LOG_TYPE_ERROR))
    -[NTFeedTransformationLimit transformFeedItems:].cold.1();
  objc_msgSend(v4, "fc_subarrayWithMaxCount:", -[NTFeedTransformationLimit limit](self, "limit"));
  v5 = (void *)objc_claimAutoreleasedReturnValue();
