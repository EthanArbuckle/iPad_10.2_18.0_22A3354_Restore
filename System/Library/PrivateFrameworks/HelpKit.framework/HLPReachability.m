...
- (void)dealloc
{
  ...
  -[HLPReachability stopNotifier](self, "stopNotifier");
  reachabilityRef = self->_reachabilityRef;
  if (reachabilityRef)
    CFRelease(reachabilityRef);
  v4.receiver = self;
  v4.super_class = (Class)HLPReachability;
  -[HLPReachability dealloc](&v4, sel_dealloc);
}
...
- (int64_t)currentReachabilityStatus
{
  ...
  if (self->_alwaysReturnLocalWiFiStatus)
    return -[HLPReachability localWiFiStatusForFlags:](self, "localWiFiStatusForFlags:", flags);
  return -[HLPReachability networkStatusForFlags:](self, "networkStatusForFlags:", flags);
}
