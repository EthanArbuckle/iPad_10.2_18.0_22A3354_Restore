@implementation TISettingValueDescriptor

- (TISettingValueDescriptor)initWithMetricName:(id)a3 settingName:(id)a4
{
  id v7;
  TISettingValueDescriptor *v8;
  TISettingValueDescriptor *v9;
  objc_super v11;

  v7 = a4;
  v11.receiver = self;
  v11.super_class = (Class)TISettingValueDescriptor;
  v8 = -[TIMetricDescriptor initWithMetricName:](&v11, sel_initWithMetricName_, a3);
  v9 = v8;
  if (v8)
    objc_storeStrong((id *)&v8->_settingName, a4);

  return v9;
}

- (NSString)settingName
{
  return (NSString *)objc_getProperty(self, a2, 16, 1);
}

- (void)setSettingName:(id)a3
{
  objc_setProperty_atomic(self, a2, a3, 16);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_settingName, 0);
}

+ (id)settingValueDescriptorWithMetricName:(id)a3 settingName:(id)a4
{
  id v5;
  id v6;
  TISettingValueDescriptor *v7;

  v5 = a4;
  v6 = a3;
  v7 = -[TISettingValueDescriptor initWithMetricName:settingName:]([TISettingValueDescriptor alloc], "initWithMetricName:settingName:", v6, v5);

  return v7;
}

@end
