...
- (CDPRecoveryKeyStatusProvider)initWithContext:(id)a3
{
  ...
  if (v5)
  {
    objc_msgSend(MEMORY[0x24BE1A3F8], "preflightContext:", v4);
...
- (void)isRecoveryKeySetInOctagonWithError:(os_log_t)log .cold.1(os_log_t log)
{
  ...
  *(_WORD *)v1 = 0;
  _os_log_error_impl(&dword_20DB2C000, log, OS_LOG_TYPE_ERROR, "\"CDPContext cliqueConfiguration returned nil. Cannot check if RK state matches across systems.\", v1, 2u);
}
...
{
  OUTLINED_FUNCTION_0(&dword_20DB2C000, a2, a3, "\"Received error while checking if RK is set in Octagon: %{public}@\", a5, a6, a7, a8, 2u);
  OUTLINED_FUNCTION_4();
}
...
{
  OUTLINED_FUNCTION_0(&dword_20DB2C000, a2, a3, "\"Received error while checking if RK is set in SOS: %{public}@\", a5, a6, a7, a8, 2u);
  OUTLINED_FUNCTION_4();
}
