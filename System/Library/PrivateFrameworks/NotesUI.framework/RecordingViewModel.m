@implementation RecordingViewModel

- (void)handleTimeNotificationWithNotification:(id)a3
{
  sub_1ACA2A658((uint64_t)self, (uint64_t)a2, (uint64_t)a3, (void (*)(char *))sub_1ACA29D40);
}

- (void)handleBackgroundTranscriptionFinished:(id)a3
{
  sub_1ACA2A658((uint64_t)self, (uint64_t)a2, (uint64_t)a3, (void (*)(char *))sub_1ACA29FCC);
}

- (void)audioControllerPlaybackStateChanged:(id)a3
{
  uint64_t v3;
  uint64_t v4;
  uint64_t v5;
  char *v6;
  uint64_t v7;

  v3 = sub_1ACA3F45C();
  v4 = *(_QWORD *)(v3 - 8);
  MEMORY[0x1E0C80A78](v3);
  v6 = (char *)&v7 - ((v5 + 15) & 0xFFFFFFFFFFFFFFF0);
  sub_1ACA3F42C();
  swift_retain();
  sub_1ACA2A700();
  swift_release();
  (*(void (**)(char *, uint64_t))(v4 + 8))(v6, v3);
}

@end
