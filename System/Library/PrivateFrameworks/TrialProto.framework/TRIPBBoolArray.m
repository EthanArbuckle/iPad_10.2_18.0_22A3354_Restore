...
- (TRIPBBoolArray)init
{
  ...
  return -[TRIPBBoolArray init](&v3, sel_init);
}

- (TRIPBBoolArray)initWithValueArray:(id)a3
{
  return -[TRIPBBoolArray initWithValues:count:](self, "initWithValues:count:", *((_QWORD *)a3 + 2), *((_QWORD *)a3 + 3));
}
...
- (TRIPBBoolArray)initWithValues:(const BOOL *)a3 count:(unint64_t)a4
{
  ...
  v6 = -[TRIPBBoolArray init](self, "init");
  v7 = v6;
  if (v6 && a3 && a4)
  {
    ...
    else
    {
      ...
      objc_msgSend(MEMORY[0x1E0C99DA0], "raise:format:", *MEMORY[0x1E0C99850], CFSTR("Failed to allocate %lu bytes"), a4);
    }
  }
  return v7;
}
...
- (TRIPBBoolArray)initWithCapacity:(unint64_t)a3
{
  ...
  v4 = -[TRIPBBoolArray initWithValues:count:](self, "initWithValues:count:", 0, 0);
  v5 = v4;
  if (a3 && v4)
    -[TRIPBBoolArray internalResizeToCapacity:](v4, "internalResizeToCapacity:", a3);
  return v5;
}

- (id)copyWithZone:(_NSZone *)a3
{
  return -[TRIPBBoolArray initWithValues:count:](+[TRIPBBoolArray allocWithZone:](TRIPBBoolArray, "allocWithZone:", a3), "initWithValues:count:", self->_values, self->_count);
}
...
- (void)dealloc
{
  ...
  if (self->_autocreator)
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E0CB3488], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, CFSTR("TRIPBArray.m"), 1861, CFSTR("%@: Autocreator must be cleared before release, autocreator: %@"), objc_opt_class(), self->_autocreator);
  free(self->_values);
  v3.receiver = self;
  v3.super_class = (Class)TRIPBBoolArray;
  -[TRIPBBoolArray dealloc](&v3, sel_dealloc);
}
...
{
  -[TRIPBBoolArray enumerateValuesWithOptions:usingBlock:](self, "enumerateValuesWithOptions:usingBlock:", 0, a3);
}
...
- (void)internalResizeToCapacity:(unint64_t)a3
{
  ...
  if (!v5)
  {
    self->_count = 0;
    self->_capacity = 0;
    objc_msgSend(MEMORY[0x1E0C99DA0], "raise:format:", *MEMORY[0x1E0C99850], CFSTR("Failed to allocate %lu bytes"), a3);
  }
  self->_capacity = a3;
}
...
- (void)addValue:(BOOL)a3
{
  ...
  v3 = a3;
  -[TRIPBBoolArray addValues:count:](self, "addValues:count:", &v3, 1);
}
...
- (void)addValues:(const BOOL *)a3 count:(unint64_t)a4
{
  ...
  if (a3 && a4)
  {
    ...
    if (count + a4 > self->_capacity)
      -[TRIPBBoolArray internalResizeToCapacity:](self, "internalResizeToCapacity:", (v8 & 0xFFFFFFFFFFFFFFF0) + 16);
    self->_count = v8;
...
{
  -[TRIPBBoolArray addValues:count:](self, "addValues:count:", *((_QWORD *)a3 + 2), *((_QWORD *)a3 + 3));
}
...
- (void)removeValueAtIndex:(unint64_t)a3
{
  ...
  if (count + 31 < self->_capacity)
    -[TRIPBBoolArray internalResizeToCapacity:](self, "internalResizeToCapacity:", (v6 & 0xFFFFFFFFFFFFFFF0) + 16);
}
...
- (void)removeAll
{
  ...
  if (self->_capacity >= 0x21)
    -[TRIPBBoolArray internalResizeToCapacity:](self, "internalResizeToCapacity:", 16);
}
