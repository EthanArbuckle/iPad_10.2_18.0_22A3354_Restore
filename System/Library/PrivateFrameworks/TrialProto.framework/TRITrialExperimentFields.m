@implementation TRITrialExperimentFields

- (BOOL)hasClientExperimentId
{
  return self->_clientExperimentId != 0;
}

- (BOOL)hasClientTreatmentId
{
  return self->_clientTreatmentId != 0;
}

- (id)description
{
  void *v3;
  void *v4;
  void *v5;
  void *v6;
  objc_super v8;

  v3 = (void *)MEMORY[0x1E0CB3940];
  v8.receiver = self;
  v8.super_class = (Class)TRITrialExperimentFields;
  -[TRITrialExperimentFields description](&v8, sel_description);
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  -[TRITrialExperimentFields dictionaryRepresentation](self, "dictionaryRepresentation");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v3, "stringWithFormat:", CFSTR("%@ %@"), v4, v5);
  v6 = (void *)objc_claimAutoreleasedReturnValue();

  return v6;
}

- (id)dictionaryRepresentation
{
  void *v3;
  void *v4;
  NSString *clientExperimentId;
  NSString *clientTreatmentId;

  objc_msgSend(MEMORY[0x1E0C99E08], "dictionary");
  v3 = (void *)objc_claimAutoreleasedReturnValue();
  v4 = v3;
  clientExperimentId = self->_clientExperimentId;
  if (clientExperimentId)
    objc_msgSend(v3, "setObject:forKey:", clientExperimentId, CFSTR("client_experiment_id"));
  clientTreatmentId = self->_clientTreatmentId;
  if (clientTreatmentId)
    objc_msgSend(v4, "setObject:forKey:", clientTreatmentId, CFSTR("client_treatment_id"));
  return v4;
}

- (BOOL)readFrom:(id)a3
{
  return TRITrialExperimentFieldsReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  id v4;
  id v5;

  v4 = a3;
  v5 = v4;
  if (self->_clientExperimentId)
  {
    PBDataWriterWriteStringField();
    v4 = v5;
  }
  if (self->_clientTreatmentId)
  {
    PBDataWriterWriteStringField();
    v4 = v5;
  }

}

- (void)copyTo:(id)a3
{
  id v4;
  id v5;

  v4 = a3;
  v5 = v4;
  if (self->_clientExperimentId)
  {
    objc_msgSend(v4, "setClientExperimentId:");
    v4 = v5;
  }
  if (self->_clientTreatmentId)
  {
    objc_msgSend(v5, "setClientTreatmentId:");
    v4 = v5;
  }

}

- (id)copyWithZone:(_NSZone *)a3
{
  _QWORD *v5;
  uint64_t v6;
  void *v7;
  uint64_t v8;
  void *v9;

  v5 = (_QWORD *)objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  v6 = -[NSString copyWithZone:](self->_clientExperimentId, "copyWithZone:", a3);
  v7 = (void *)v5[1];
  v5[1] = v6;

  v8 = -[NSString copyWithZone:](self->_clientTreatmentId, "copyWithZone:", a3);
  v9 = (void *)v5[2];
  v5[2] = v8;

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  _QWORD *v4;
  NSString *clientExperimentId;
  NSString *clientTreatmentId;
  char v7;

  v4 = a3;
  if (objc_msgSend(v4, "isMemberOfClass:", objc_opt_class())
    && ((clientExperimentId = self->_clientExperimentId, !((unint64_t)clientExperimentId | v4[1]))
     || -[NSString isEqual:](clientExperimentId, "isEqual:")))
  {
    clientTreatmentId = self->_clientTreatmentId;
    if ((unint64_t)clientTreatmentId | v4[2])
      v7 = -[NSString isEqual:](clientTreatmentId, "isEqual:");
    else
      v7 = 1;
  }
  else
  {
    v7 = 0;
  }

  return v7;
}

- (unint64_t)hash
{
  NSUInteger v3;

  v3 = -[NSString hash](self->_clientExperimentId, "hash");
  return -[NSString hash](self->_clientTreatmentId, "hash") ^ v3;
}

- (void)mergeFrom:(id)a3
{
  _QWORD *v4;
  _QWORD *v5;

  v4 = a3;
  v5 = v4;
  if (v4[1])
  {
    -[TRITrialExperimentFields setClientExperimentId:](self, "setClientExperimentId:");
    v4 = v5;
  }
  if (v4[2])
  {
    -[TRITrialExperimentFields setClientTreatmentId:](self, "setClientTreatmentId:");
    v4 = v5;
  }

}

- (NSString)clientExperimentId
{
  return self->_clientExperimentId;
}

- (void)setClientExperimentId:(id)a3
{
  objc_storeStrong((id *)&self->_clientExperimentId, a3);
}

- (NSString)clientTreatmentId
{
  return self->_clientTreatmentId;
}

- (void)setClientTreatmentId:(id)a3
{
  objc_storeStrong((id *)&self->_clientTreatmentId, a3);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_clientTreatmentId, 0);
  objc_storeStrong((id *)&self->_clientExperimentId, 0);
}

@end
