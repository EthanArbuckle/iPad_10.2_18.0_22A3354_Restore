...
- (id)description
{
  ...
  return (id)objc_msgSend_stringWithFormat_(MEMORY[0x1E0CB3940], a2, (uint64_t)CFSTR("bbox %p - min:[%f %f %f] max:[%f %f %f]"), v2, self, *(float *)self->min, *(float *)&self->min[4], *(float *)&self->min[8], *(float *)self->max, *(float *)&self->max[4], *(float *)&self->max[8]);
}
