@implementation UIPreviewAction

+ (UIPreviewAction)actionWithTitle:(NSString *)title style:(UIPreviewActionStyle)style handler:(void *)handler
{
  void *v8;
  NSString *v9;
  id v10;

  v8 = handler;
  v9 = title;
  v10 = objc_alloc_init((Class)a1);
  objc_msgSend(v10, "setTitle:", v9);

  objc_msgSend(v10, "setStyle:", style);
  objc_msgSend(v10, "setHandler:", v8);

  return (UIPreviewAction *)v10;
}

+ (id)_actionWithTitle:(id)a3 style:(int64_t)a4 image:(id)a5 handler:(id)a6
{
  id v10;
  id v11;
  id v12;
  id v13;

  v10 = a6;
  v11 = a5;
  v12 = a3;
  v13 = objc_alloc_init((Class)a1);
  objc_msgSend(v13, "setTitle:", v12);

  objc_msgSend(v13, "setStyle:", a4);
  objc_msgSend(v13, "setHandler:", v10);

  objc_msgSend(v13, "setImage:", v11);
  return v13;
}

+ (id)_actionWithTitle:(id)a3 color:(id)a4 image:(id)a5 handler:(id)a6
{
  id v10;
  id v11;
  id v12;
  id v13;
  id v14;

  v10 = a6;
  v11 = a5;
  v12 = a4;
  v13 = a3;
  v14 = objc_alloc_init((Class)a1);
  objc_msgSend(v14, "setTitle:", v13);

  objc_msgSend(v14, "setStyle:", 0);
  objc_msgSend(v14, "_setColor:", v12);

  objc_msgSend(v14, "setImage:", v11);
  objc_msgSend(v14, "setHandler:", v10);

  return v14;
}

+ (id)_actionWithTitle:(id)a3 style:(int64_t)a4 color:(id)a5 image:(id)a6 handler:(id)a7
{
  id v12;
  id v13;
  id v14;
  id v15;
  id v16;

  v12 = a7;
  v13 = a6;
  v14 = a5;
  v15 = a3;
  v16 = objc_alloc_init((Class)a1);
  objc_msgSend(v16, "setTitle:", v15);

  objc_msgSend(v16, "setStyle:", a4);
  objc_msgSend(v16, "_setColor:", v14);

  objc_msgSend(v16, "setImage:", v13);
  objc_msgSend(v16, "setHandler:", v12);

  return v16;
}

- (id)_effectiveColor
{
  void *v3;

  -[UIPreviewAction _color](self, "_color");
  v3 = (void *)objc_claimAutoreleasedReturnValue();
  if (!v3)
  {
    if (-[UIPreviewAction style](self, "style") == 2)
    {
      +[UIColor systemRedColor](UIColor, "systemRedColor");
      v3 = (void *)objc_claimAutoreleasedReturnValue();
    }
    else
    {
      v3 = 0;
    }
  }
  return v3;
}

- (id)_effectiveImage
{
  void *v3;

  -[UIPreviewAction image](self, "image");
  v3 = (void *)objc_claimAutoreleasedReturnValue();
  if (!v3)
  {
    if (-[UIPreviewAction style](self, "style") == 1)
    {
      objc_msgSend((id)objc_opt_class(), "_checkmarkImage");
      v3 = (void *)objc_claimAutoreleasedReturnValue();
    }
    else
    {
      v3 = 0;
    }
  }
  return v3;
}

+ (id)_checkmarkImage
{
  return _UIImageWithName(CFSTR("UIPreviewMenuItemCheckmark.png"));
}

- (id)copyWithZone:(_NSZone *)a3
{
  void *v4;
  void *v5;
  void *v6;
  void *v7;
  void *v8;
  void *v9;

  v4 = (void *)objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  -[UIPreviewAction title](self, "title");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "setTitle:", v5);

  objc_msgSend(v4, "setStyle:", -[UIPreviewAction style](self, "style"));
  -[UIPreviewAction handler](self, "handler");
  v6 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "setHandler:", v6);

  -[UIPreviewAction identifier](self, "identifier");
  v7 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "setIdentifier:", v7);

  -[UIPreviewAction _color](self, "_color");
  v8 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "_setColor:", v8);

  -[UIPreviewAction image](self, "image");
  v9 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "setImage:", v9);

  return v4;
}

+ (id)_actionWithPreviewMenuItem:(id)a3
{
  id v4;
  void *v5;
  uint64_t v6;
  void *v7;
  void *v8;
  id v9;
  void *v10;
  _QWORD v12[4];
  id v13;

  v4 = a3;
  objc_msgSend(v4, "title");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  v6 = objc_msgSend(v4, "style");
  objc_msgSend(v4, "_color");
  v7 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "image");
  v8 = (void *)objc_claimAutoreleasedReturnValue();
  v12[0] = MEMORY[0x1E0C809B0];
  v12[1] = 3221225472;
  v12[2] = __46__UIPreviewAction__actionWithPreviewMenuItem___block_invoke;
  v12[3] = &unk_1E16C1210;
  v13 = v4;
  v9 = v4;
  objc_msgSend(a1, "_actionWithTitle:style:color:image:handler:", v5, v6, v7, v8, v12);
  v10 = (void *)objc_claimAutoreleasedReturnValue();

  return v10;
}

void __46__UIPreviewAction__actionWithPreviewMenuItem___block_invoke(uint64_t a1, uint64_t a2, void *a3)
{
  void *v4;
  void (**v5)(_QWORD, _QWORD, _QWORD);
  id v6;

  v6 = a3;
  objc_msgSend(*(id *)(a1 + 32), "handler");
  v4 = (void *)objc_claimAutoreleasedReturnValue();

  if (v4)
  {
    objc_msgSend(*(id *)(a1 + 32), "handler");
    v5 = (void (**)(_QWORD, _QWORD, _QWORD))objc_claimAutoreleasedReturnValue();
    ((void (**)(_QWORD, _QWORD, id))v5)[2](v5, *(_QWORD *)(a1 + 32), v6);

  }
}

- (NSString)title
{
  return self->_title;
}

- (void)setTitle:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 8);
}

- (UIImage)image
{
  return self->_image;
}

- (void)setImage:(id)a3
{
  objc_storeStrong((id *)&self->_image, a3);
}

- (NSString)identifier
{
  return self->_identifier;
}

- (void)setIdentifier:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 24);
}

- (UIColor)_color
{
  return self->_color;
}

- (void)_setColor:(id)a3
{
  objc_storeStrong((id *)&self->_color, a3);
}

- (int64_t)style
{
  return self->_style;
}

- (void)setStyle:(int64_t)a3
{
  self->_style = a3;
}

- (void)handler
{
  return self->_handler;
}

- (void)setHandler:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 48);
}

- (void).cxx_destruct
{
  objc_storeStrong(&self->_handler, 0);
  objc_storeStrong((id *)&self->_color, 0);
  objc_storeStrong((id *)&self->_identifier, 0);
  objc_storeStrong((id *)&self->_image, 0);
  objc_storeStrong((id *)&self->_title, 0);
}

@end
