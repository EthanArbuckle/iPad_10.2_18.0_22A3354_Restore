...
{
  return -[TAPersistenceManagerSettings initWithDirectoryURLOrDefault:storeFileNameOrDefault:](self, "initWithDirectoryURLOrDefault:storeFileNameOrDefault:", 0, 0);
}
...
- (TAPersistenceManagerSettings)initWithDirectoryURLOrDefault:(id)a3 storeFileNameOrDefault:(id)a4
{
  id v6;
  __CFString *v7;
  TAPersistenceManagerSettings *v8;
  NSURL *v9;
  ...
  v8 = -[TAPersistenceManagerSettings init](&v13, sel_init);
  if (v8)
  {
    if (v6)
    {
    ...
    {
      objc_msgSend(MEMORY[0x24BDBCF48], "fileURLWithPath:isDirectory:", CFSTR("/var/root/Library/Caches/locationd/TrackingAvoidance/"), 1);
    ...
    if (v7)
      ...
      v11 = CFSTR("trackingavoidance.bin");
...
- (id)_getStoreURL
{
  return (id)objc_msgSend(MEMORY[0x24BDBCF48], "fileURLWithPath:relativeToURL:", self->_persistenceStoreFileName, self->_persistenceDirectoryURL);
}
