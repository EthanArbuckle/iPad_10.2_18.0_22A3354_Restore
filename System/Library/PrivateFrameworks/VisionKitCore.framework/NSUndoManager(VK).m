...
- (void)vk_performUndoGroupWithActionName:()VK undoGroup:
{
  ...
  else
  {
    ...
    if (os_log_type_enabled(v9, OS_LOG_TYPE_ERROR))
      -[NSUndoManager(VK) vk_performUndoGroupWithActionName:undoGroup:].cold.1(v9);
...
- (void)vk_performUndoGroupWithActionName:()VK undoGroup:.cold.1(os_log_t log)
{
  ...
  *(_WORD *)v1 = 0;
  _os_log_error_impl(&dword_1D2E0D000, log, OS_LOG_TYPE_ERROR, "Attempted to endUndoGrouping for a non-0 groupingLevel undoManager", v1, 2u);
}
