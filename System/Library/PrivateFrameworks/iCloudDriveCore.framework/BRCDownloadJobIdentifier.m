...
{
  return (NSString *)objc_msgSend(MEMORY[0x24BDD17C8], "stringWithFormat:", CFSTR("%lld-%@-%s"), self->_itemDBRowID, self->_etag, BRCPrettyPrintEnum());
}

- (id)matchingJobsWhereSQLClause
{
  return (id)objc_msgSend(MEMORY[0x24BEDD638], "formatInjection:", CFSTR("throttle_id = %lld AND download_etag = %@ AND download_kind = %u"), self->_itemDBRowID, self->_etag, self->_kind);
}

- (id)columns
{
  return (id)objc_msgSend(MEMORY[0x24BEDD648], "rawInjection:length:", "throttle_id, download_etag, download_kind", 41);
}

- (id)columnsValues
{
  return (id)objc_msgSend(MEMORY[0x24BEDD638], "formatInjection:", CFSTR("%lld, %@, %u"), self->_itemDBRowID, self->_etag, self->_kind);
}
