...
- (BRCFSDownloaderBatchEnumerator)initWithSyncContext:(id)a3 kind:(int)a4 now:(int64_t)a5 retryQueueKick:(int64_t *)a6
{
  ...
  if (v11)
  {
    ...
    v16 = objc_msgSend(v14, "fetch:", CFSTR("  SELECT throttle_id, next_retry_stamp, app_library_rowid,           download_etag, transfer_stage, unit_count_completed           , transfer_size, download_priority      FROM client_downloads    WHERE throttle_state = 1    AND throttle_state != 0      AND transfer_queue = %@      AND transfer_operation IS NULL      AND download_kind = %d ORDER BY download_priority DESC, download_request_stamp DESC"), v15, v8);
    rs = v12->_rs;
    ...
    if ((enumeratedThrottleIds & 1) != 0)
    {
      ...
      if (os_log_type_enabled(v11, OS_LOG_TYPE_DEBUG))
      {
        *(_DWORD *)buf = 134218242;
        ...
        v25 = 2112;
        v26 = v10;
        _os_log_debug_impl(&dword_230455000, v11, OS_LOG_TYPE_DEBUG, "[DEBUG] Throttle ID %lld was already enumerated. Will schedule it for retry to the next batch%@", buf, 0x16u);
      }
...
{
  return -[PQLResultSet longLongAtIndex:](self->_rs, "longLongAtIndex:", 6);
}

- (int64_t)transferQOS
{
  return -[PQLResultSet unsignedLongAtIndex:](self->_rs, "unsignedLongAtIndex:", 7);
}
