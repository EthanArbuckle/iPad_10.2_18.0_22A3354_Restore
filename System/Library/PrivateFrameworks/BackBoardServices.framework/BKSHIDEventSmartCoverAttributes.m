@implementation BKSHIDEventSmartCoverAttributes

- (id)copyWithZone:(_NSZone *)a3
{
  id result;
  objc_super v5;

  v5.receiver = self;
  v5.super_class = (Class)BKSHIDEventSmartCoverAttributes;
  result = -[BKSHIDEventBaseAttributes copyWithZone:](&v5, sel_copyWithZone_, a3);
  *((_DWORD *)result + 12) = self->_smartCoverState;
  *((_DWORD *)result + 13) = self->_wakeAnimationStyle;
  return result;
}

- (BOOL)isEqual:(id)a3
{
  _DWORD *v4;
  BOOL v5;
  objc_super v7;

  v4 = a3;
  v7.receiver = self;
  v7.super_class = (Class)BKSHIDEventSmartCoverAttributes;
  v5 = -[BKSHIDEventBaseAttributes isEqual:](&v7, sel_isEqual_, v4)
    && (objc_opt_class(), (objc_opt_isKindOfClass() & 1) != 0)
    && self->_smartCoverState == v4[12]
    && self->_wakeAnimationStyle == v4[13];

  return v5;
}

- (void)appendDescriptionToFormatter:(id)a3
{
  id v4;
  id v5;
  id v6;
  objc_super v7;

  v4 = a3;
  v7.receiver = self;
  v7.super_class = (Class)BKSHIDEventSmartCoverAttributes;
  -[BKSHIDEventBaseAttributes appendDescriptionToFormatter:](&v7, sel_appendDescriptionToFormatter_, v4);
  v5 = (id)objc_msgSend(v4, "appendUnsignedInteger:withName:", self->_smartCoverState, CFSTR("smartCoverState"));
  v6 = (id)objc_msgSend(v4, "appendUnsignedInteger:withName:", self->_wakeAnimationStyle, CFSTR("wakeAnimationStyle"));

}

- (int)smartCoverState
{
  return self->_smartCoverState;
}

- (void)setSmartCoverState:(int)a3
{
  self->_smartCoverState = a3;
}

- (int)wakeAnimationStyle
{
  return self->_wakeAnimationStyle;
}

- (void)setWakeAnimationStyle:(int)a3
{
  self->_wakeAnimationStyle = a3;
}

+ (id)protobufSchema
{
  _QWORD block[5];

  block[0] = MEMORY[0x1E0C809B0];
  block[1] = 3221225472;
  block[2] = __49__BKSHIDEventSmartCoverAttributes_protobufSchema__block_invoke;
  block[3] = &__block_descriptor_40_e5_v8__0l;
  block[4] = a1;
  if (protobufSchema_onceToken_554 != -1)
    dispatch_once(&protobufSchema_onceToken_554, block);
  return (id)protobufSchema_schema_553;
}

void __49__BKSHIDEventSmartCoverAttributes_protobufSchema__block_invoke(uint64_t a1)
{
  uint64_t v1;
  void *v2;

  objc_msgSend(MEMORY[0x1E0D01838], "buildSchemaForClass:builder:", *(_QWORD *)(a1 + 32), &__block_literal_global_555);
  v1 = objc_claimAutoreleasedReturnValue();
  v2 = (void *)protobufSchema_schema_553;
  protobufSchema_schema_553 = v1;

}

void __49__BKSHIDEventSmartCoverAttributes_protobufSchema__block_invoke_2(uint64_t a1, void *a2)
{
  id v2;

  v2 = a2;
  objc_msgSend(v2, "addField:", "_smartCoverState");
  objc_msgSend(v2, "addField:", "_wakeAnimationStyle");

}

@end
