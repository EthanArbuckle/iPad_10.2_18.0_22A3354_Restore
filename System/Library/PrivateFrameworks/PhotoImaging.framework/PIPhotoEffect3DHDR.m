...
+ (id)kernel
{
  if (kernel_onceToken_56 != -1)
    dispatch_once(&kernel_onceToken_56, &__block_literal_global_57);
  return (id)kernel_kernel_55;
}

void __28__PIPhotoEffect3DHDR_kernel__block_invoke()
{
  uint64_t v0;
  void *v1;
  NSObject *v2;
  ...
  objc_msgSend(MEMORY[0x1E0C9DD88], "kernelWithString:", CFSTR("kernel vec4 _hdrOffsetPos(sampler image) { vec4 im = sample(image, samplerCoord(image)); float r = max(im.r - 1.0f, 0.0f); float g = max(im.g - 1.0f, 0.0f); float b = max(im.b - 1.0f, 0.0f); return vec4(r, g, b, 0.0f); }"));
  v0 = objc_claimAutoreleasedReturnValue();
  v1 = (void *)kernel_kernel_55;
  kernel_kernel_55 = v0;
...
- (id)outputImage
{
  ...
  -[PIPhotoEffect3DHDR photoEffectName](self, "photoEffectName");
