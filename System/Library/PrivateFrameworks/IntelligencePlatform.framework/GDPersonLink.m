@implementation GDPersonLink

- (GDRelationshipIdentifier)relationshipIdentifier
{
  return self->_relationshipIdentifier;
}

- (NSString)relationshipLabel
{
  return self->_relationshipLabel;
}

- (GDEntityIdentifier)linkedPersonEntityIdentifier
{
  return self->_linkedPersonEntityIdentifier;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_linkedPersonEntityIdentifier, 0);
  objc_storeStrong((id *)&self->_relationshipLabel, 0);
  objc_storeStrong((id *)&self->_relationshipIdentifier, 0);
}

- (GDPersonLink)initWithRelationshipIdTriplesIterator:(id)a3
{
  id v4;
  GDPersonLink *v5;
  id v6;
  const char *v7;
  uint64_t v8;
  const char *v9;
  uint64_t v10;
  uint64_t v11;
  uint64_t v12;
  uint64_t v13;
  void *v14;
  GDRelationshipIdentifier *v15;
  const char *v16;
  uint64_t v17;
  void *v18;
  const char *v19;
  uint64_t v20;
  GDRelationshipIdentifier *relationshipIdentifier;
  void *v22;
  const char *v23;
  const char *v24;
  uint64_t v25;
  uint64_t v26;
  void *relationshipLabel;
  GDEntityIdentifier *v28;
  const char *v29;
  uint64_t v30;
  const char *v31;
  uint64_t v32;
  GDEntityIdentifier *linkedPersonEntityIdentifier;
  GDPersonLink *v34;
  id v36;
  __int128 v37;
  __int128 v38;
  __int128 v39;
  __int128 v40;
  objc_super v41;
  _BYTE v42[128];
  uint64_t v43;

  v43 = *MEMORY[0x1E0C80C00];
  v4 = a3;
  v41.receiver = self;
  v41.super_class = (Class)GDPersonLink;
  v5 = -[GDPersonLink init](&v41, sel_init);
  if (!v5)
  {
LABEL_18:
    v34 = v5;
    goto LABEL_21;
  }
  v39 = 0u;
  v40 = 0u;
  v37 = 0u;
  v38 = 0u;
  v36 = v4;
  v6 = v4;
  v8 = objc_msgSend_countByEnumeratingWithState_objects_count_(v6, v7, (uint64_t)&v37, v42, 16);
  if (!v8)
    goto LABEL_16;
  v11 = v8;
  v12 = *(_QWORD *)v38;
  do
  {
    v13 = 0;
    do
    {
      if (*(_QWORD *)v38 != v12)
        objc_enumerationMutation(v6);
      v14 = *(void **)(*((_QWORD *)&v37 + 1) + 8 * v13);
      if (!v5->_relationshipIdentifier)
      {
        v15 = [GDRelationshipIdentifier alloc];
        objc_msgSend_relationshipId(v14, v16, v17);
        v18 = (void *)objc_claimAutoreleasedReturnValue();
        v20 = objc_msgSend_initWithString_(v15, v19, (uint64_t)v18);
        relationshipIdentifier = v5->_relationshipIdentifier;
        v5->_relationshipIdentifier = (GDRelationshipIdentifier *)v20;

      }
      objc_msgSend_relationshipPredicate(v14, v9, v10);
      v22 = (void *)objc_claimAutoreleasedReturnValue();
      if (objc_msgSend_isEqual_(v22, v23, (uint64_t)CFSTR("nm_relationshipLabel")))
      {
        objc_msgSend_object(v14, v24, v25);
        v26 = objc_claimAutoreleasedReturnValue();
        relationshipLabel = v5->_relationshipLabel;
        v5->_relationshipLabel = (NSString *)v26;
      }
      else
      {
        if (!objc_msgSend_isEqual_(v22, v24, (uint64_t)CFSTR("nm_personIdentifier")))
          goto LABEL_14;
        v28 = [GDEntityIdentifier alloc];
        objc_msgSend_object(v14, v29, v30);
        relationshipLabel = (void *)objc_claimAutoreleasedReturnValue();
        v32 = objc_msgSend_initWithString_(v28, v31, (uint64_t)relationshipLabel);
        linkedPersonEntityIdentifier = v5->_linkedPersonEntityIdentifier;
        v5->_linkedPersonEntityIdentifier = (GDEntityIdentifier *)v32;

      }
LABEL_14:

      ++v13;
    }
    while (v11 != v13);
    v11 = objc_msgSend_countByEnumeratingWithState_objects_count_(v6, v9, (uint64_t)&v37, v42, 16);
  }
  while (v11);
LABEL_16:

  if (v5->_relationshipIdentifier)
  {
    v4 = v36;
    if (v5->_linkedPersonEntityIdentifier)
      goto LABEL_18;
    v34 = 0;
  }
  else
  {
    v34 = 0;
    v4 = v36;
  }
LABEL_21:

  return v34;
}

@end
