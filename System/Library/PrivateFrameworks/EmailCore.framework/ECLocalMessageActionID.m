@implementation ECLocalMessageActionID

- (int64_t)databaseID
{
  return *(int64_t *)((char *)&self->super.isa + OBJC_IVAR___ECLocalMessageActionID_databaseID);
}

- (NSString)stringValue
{
  void *v2;

  swift_bridgeObjectRetain();
  v2 = (void *)sub_1B9B2E9C8();
  swift_bridgeObjectRelease();
  return (NSString *)v2;
}

- (ECLocalMessageActionID)initWithDatabaseID:(int64_t)a3
{
  ECLocalMessageActionID *v3;
  id v4;
  uint64_t v5;
  uint64_t v6;
  uint64_t v7;
  uint64_t *v8;
  objc_super v10;

  *(Class *)((char *)&self->super.isa + OBJC_IVAR___ECLocalMessageActionID_databaseID) = (Class)a3;
  v3 = self;
  v4 = (id)EFStringWithInt64();
  v5 = sub_1B9B2E9D4();
  v7 = v6;

  v8 = (uint64_t *)((char *)v3 + OBJC_IVAR___ECLocalMessageActionID_stringValue);
  *v8 = v5;
  v8[1] = v7;

  v10.receiver = v3;
  v10.super_class = (Class)ECLocalMessageActionID;
  return -[ECLocalMessageActionID init](&v10, sel_init);
}

- (NSString)description
{
  ECLocalMessageActionID *v2;
  NSString *v3;
  void *v4;

  v2 = self;
  v3 = -[ECLocalMessageActionID stringValue](v2, sel_stringValue);
  sub_1B9B2E9D4();

  v4 = (void *)sub_1B9B2E9C8();
  swift_bridgeObjectRelease();
  return (NSString *)v4;
}

- (BOOL)isEqual:(id)a3
{
  ECLocalMessageActionID *v4;
  ECLocalMessageActionID *v5;
  BOOL v6;
  _OWORD v8[2];

  if (a3)
  {
    v4 = self;
    swift_unknownObjectRetain();
    sub_1B9B2E9E0();
    swift_unknownObjectRelease();
  }
  else
  {
    memset(v8, 0, sizeof(v8));
    v5 = self;
  }
  v6 = ECLocalMessageActionID.isEqual(_:)((uint64_t)v8);

  sub_1B9B15BCC((uint64_t)v8);
  return v6;
}

- (int64_t)hash
{
  ECLocalMessageActionID *v2;
  int64_t v3;

  v2 = self;
  -[ECLocalMessageActionID databaseID](v2, sel_databaseID);
  v3 = sub_1B9B2E9EC();

  return v3;
}

- (ECLocalMessageActionID)init
{
  ECLocalMessageActionID *result;

  result = (ECLocalMessageActionID *)_swift_stdlib_reportUnimplementedInitializer();
  __break(1u);
  return result;
}

- (void).cxx_destruct
{
  swift_bridgeObjectRelease();
}

@end
