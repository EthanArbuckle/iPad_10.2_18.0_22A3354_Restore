@implementation WOHealthBridgeKeyedDate

- (id)description
{
  void *v3;
  void *v4;
  void *v5;
  void *v6;
  objc_super v8;

  v3 = (void *)MEMORY[0x24BDD17C8];
  v8.receiver = self;
  v8.super_class = (Class)WOHealthBridgeKeyedDate;
  -[WOHealthBridgeKeyedDate description](&v8, sel_description);
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  -[WOHealthBridgeKeyedDate dictionaryRepresentation](self, "dictionaryRepresentation");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v3, "stringWithFormat:", CFSTR("%@ %@"), v4, v5);
  v6 = (void *)objc_claimAutoreleasedReturnValue();

  return v6;
}

- (id)dictionaryRepresentation
{
  void *v3;
  void *v4;
  NSString *key;
  void *v6;

  objc_msgSend(MEMORY[0x24BDBCED8], "dictionary");
  v3 = (void *)objc_claimAutoreleasedReturnValue();
  v4 = v3;
  key = self->_key;
  if (key)
    objc_msgSend(v3, "setObject:forKey:", key, CFSTR("key"));
  objc_msgSend(MEMORY[0x24BDD16E0], "numberWithDouble:", self->_timeSinceReferenceDate);
  v6 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "setObject:forKey:", v6, CFSTR("timeSinceReferenceDate"));

  return v4;
}

- (BOOL)readFrom:(id)a3
{
  return WOHealthBridgeKeyedDateReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  id v4;
  id v5;

  v4 = a3;
  if (!self->_key)
    -[WOHealthBridgeKeyedDate writeTo:].cold.1();
  v5 = v4;
  PBDataWriterWriteStringField();
  PBDataWriterWriteDoubleField();

}

- (void)copyTo:(id)a3
{
  NSString *key;
  double *v5;

  key = self->_key;
  v5 = (double *)a3;
  objc_msgSend(v5, "setKey:", key);
  v5[1] = self->_timeSinceReferenceDate;

}

- (id)copyWithZone:(_NSZone *)a3
{
  uint64_t v5;
  uint64_t v6;
  void *v7;

  v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  v6 = -[NSString copyWithZone:](self->_key, "copyWithZone:", a3);
  v7 = *(void **)(v5 + 16);
  *(_QWORD *)(v5 + 16) = v6;

  *(double *)(v5 + 8) = self->_timeSinceReferenceDate;
  return (id)v5;
}

- (BOOL)isEqual:(id)a3
{
  id v4;
  NSString *key;
  BOOL v6;

  v4 = a3;
  v6 = objc_msgSend(v4, "isMemberOfClass:", objc_opt_class())
    && ((key = self->_key, !((unint64_t)key | *((_QWORD *)v4 + 2)))
     || -[NSString isEqual:](key, "isEqual:"))
    && self->_timeSinceReferenceDate == *((double *)v4 + 1);

  return v6;
}

- (unint64_t)hash
{
  NSUInteger v3;
  double timeSinceReferenceDate;
  double v5;
  long double v6;
  double v7;
  unint64_t v8;
  unint64_t v9;
  double v10;
  unint64_t v11;

  v3 = -[NSString hash](self->_key, "hash");
  timeSinceReferenceDate = self->_timeSinceReferenceDate;
  v5 = -timeSinceReferenceDate;
  if (timeSinceReferenceDate >= 0.0)
    v5 = self->_timeSinceReferenceDate;
  v6 = floor(v5 + 0.5);
  v7 = (v5 - v6) * 1.84467441e19;
  v8 = 2654435761u * (unint64_t)fmod(v6, 1.84467441e19);
  v9 = v8 + (unint64_t)v7;
  v10 = fabs(v7);
  if (v7 <= 0.0)
    v9 = v8;
  v11 = v8 - (unint64_t)v10;
  if (v7 >= 0.0)
    v11 = v9;
  return v11 ^ v3;
}

- (void)mergeFrom:(id)a3
{
  double *v4;
  double *v5;

  v4 = (double *)a3;
  if (*((_QWORD *)v4 + 2))
  {
    v5 = v4;
    -[WOHealthBridgeKeyedDate setKey:](self, "setKey:");
    v4 = v5;
  }
  self->_timeSinceReferenceDate = v4[1];

}

- (NSString)key
{
  return self->_key;
}

- (void)setKey:(id)a3
{
  objc_storeStrong((id *)&self->_key, a3);
}

- (double)timeSinceReferenceDate
{
  return self->_timeSinceReferenceDate;
}

- (void)setTimeSinceReferenceDate:(double)a3
{
  self->_timeSinceReferenceDate = a3;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_key, 0);
}

- (void)writeTo:.cold.1()
{
  __assert_rtn("-[WOHealthBridgeKeyedDate writeTo:]", "WOHealthBridgeKeyedDate.m", 79, "nil != self->_key");
}

@end
