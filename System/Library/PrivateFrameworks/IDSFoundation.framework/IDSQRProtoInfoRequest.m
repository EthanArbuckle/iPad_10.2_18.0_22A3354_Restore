@implementation IDSQRProtoInfoRequest

- (BOOL)hasAccessToken
{
  return self->_accessToken != 0;
}

- (id)description
{
  void *v3;
  void *v4;
  const char *v5;
  uint64_t v6;
  double v7;
  void *v8;
  const char *v9;
  double v10;
  void *v11;
  objc_super v13;

  v3 = (void *)MEMORY[0x1E0CB3940];
  v13.receiver = self;
  v13.super_class = (Class)IDSQRProtoInfoRequest;
  -[IDSQRProtoInfoRequest description](&v13, sel_description);
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_dictionaryRepresentation(self, v5, v6, v7);
  v8 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_stringWithFormat_(v3, v9, (uint64_t)CFSTR("%@ %@"), v10, v4, v8);
  v11 = (void *)objc_claimAutoreleasedReturnValue();

  return v11;
}

- (id)dictionaryRepresentation
{
  uint64_t v2;
  double v3;
  void *v5;
  const char *v6;
  double v7;
  void *v8;
  const char *v9;
  double v10;
  const char *v11;
  double v12;
  NSData *accessToken;

  objc_msgSend_dictionary(MEMORY[0x1E0C99E08], a2, v2, v3);
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_numberWithUnsignedInt_(MEMORY[0x1E0CB37E8], v6, self->_infoFlags, v7);
  v8 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend_setObject_forKey_(v5, v9, (uint64_t)v8, v10, CFSTR("info_flags"));

  accessToken = self->_accessToken;
  if (accessToken)
    objc_msgSend_setObject_forKey_(v5, v11, (uint64_t)accessToken, v12, CFSTR("access_token"));
  return v5;
}

- (BOOL)readFrom:(id)a3
{
  return IDSQRProtoInfoRequestReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  id v4;

  v4 = a3;
  PBDataWriterWriteUint32Field();
  if (self->_accessToken)
    PBDataWriterWriteDataField();

}

- (void)copyTo:(id)a3
{
  double v3;
  NSData *accessToken;

  *((_DWORD *)a3 + 4) = self->_infoFlags;
  accessToken = self->_accessToken;
  if (accessToken)
    objc_msgSend_setAccessToken_(a3, a2, (uint64_t)accessToken, v3);
}

- (id)copyWithZone:(_NSZone *)a3
{
  void *v5;
  const char *v6;
  double v7;
  void *v8;
  const char *v9;
  uint64_t v10;
  double v11;
  uint64_t v12;
  const char *v13;
  double v14;
  uint64_t v15;
  void *v16;

  v5 = (void *)objc_opt_class();
  v8 = (void *)objc_msgSend_allocWithZone_(v5, v6, (uint64_t)a3, v7);
  v12 = objc_msgSend_init(v8, v9, v10, v11);
  *(_DWORD *)(v12 + 16) = self->_infoFlags;
  v15 = objc_msgSend_copyWithZone_(self->_accessToken, v13, (uint64_t)a3, v14);
  v16 = *(void **)(v12 + 8);
  *(_QWORD *)(v12 + 8) = v15;

  return (id)v12;
}

- (BOOL)isEqual:(id)a3
{
  _QWORD *v4;
  uint64_t v5;
  const char *v6;
  double v7;
  const char *v8;
  double v9;
  NSData *accessToken;
  uint64_t v11;
  char isEqual;

  v4 = a3;
  v5 = objc_opt_class();
  if (objc_msgSend_isMemberOfClass_(v4, v6, v5, v7) && self->_infoFlags == *((_DWORD *)v4 + 4))
  {
    accessToken = self->_accessToken;
    v11 = v4[1];
    if ((unint64_t)accessToken | v11)
      isEqual = objc_msgSend_isEqual_(accessToken, v8, v11, v9);
    else
      isEqual = 1;
  }
  else
  {
    isEqual = 0;
  }

  return isEqual;
}

- (unint64_t)hash
{
  uint64_t v2;
  double v3;
  uint64_t v4;

  v4 = 2654435761 * self->_infoFlags;
  return objc_msgSend_hash(self->_accessToken, a2, v2, v3) ^ v4;
}

- (void)mergeFrom:(id)a3
{
  double v3;
  uint64_t v4;

  self->_infoFlags = *((_DWORD *)a3 + 4);
  v4 = *((_QWORD *)a3 + 1);
  if (v4)
    objc_msgSend_setAccessToken_(self, a2, v4, v3);
}

- (unsigned)infoFlags
{
  return self->_infoFlags;
}

- (void)setInfoFlags:(unsigned int)a3
{
  self->_infoFlags = a3;
}

- (NSData)accessToken
{
  return self->_accessToken;
}

- (void)setAccessToken:(id)a3
{
  objc_storeStrong((id *)&self->_accessToken, a3);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_accessToken, 0);
}

@end
