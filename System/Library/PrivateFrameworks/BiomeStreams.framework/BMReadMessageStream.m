...
- (NSString)identifier
{
  return (NSString *)objc_msgSend(MEMORY[0x1E0D01D20], "streamIdentifierForStream:", 26);
}

- (BMReadMessageStream)init
{
  BMReadMessageStream *v2;
  void *v3;
  void *v4;
  void *v5;
  uint64_t v6;
  ...
  v2 = -[BMReadMessageStream init](&v10, sel_init);
  if (v2)
  {
    BMRootLibraryBridge();
    ...
    if (!v2->_storeStream)
    {
      ...
      if (os_log_type_enabled(v8, OS_LOG_TYPE_FAULT))
        -[BMReadMessageStream init].cold.1(v8);
...
- (id)publisher
{
  ...
  -[BMReadMessageStream identifier](self, "identifier");
  v4 = (void *)objc_claimAutoreleasedReturnValue();
...
- (void)init
{
  ...
  *(_WORD *)v1 = 0;
  _os_log_fault_impl(&dword_18D810000, log, OS_LOG_TYPE_FAULT, "Unable to initialize BMPublicStreamReadMessage, please consult error logs for cause", v1, 2u);
}
