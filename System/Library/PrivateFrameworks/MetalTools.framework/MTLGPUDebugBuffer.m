...
{
  return -[MTLGPUDebugBuffer initWithBuffer:device:offset:length:track:](self, "initWithBuffer:device:offset:length:track:", a3, a4, 0, objc_msgSend(a3, "length"), 1);
}

- (MTLGPUDebugBuffer)initWithBuffer:(id)a3 device:(id)a4 offset:(unint64_t)a5 length:(unint64_t)a6
{
  return -[MTLGPUDebugBuffer initWithBuffer:device:offset:length:track:](self, "initWithBuffer:device:offset:length:track:", a3, a4, a5, a6, 1);
}

- (MTLGPUDebugBuffer)initWithBuffer:(id)a3 device:(id)a4 offset:(unint64_t)a5 length:(unint64_t)a6 track:(BOOL)a7
{
  return -[MTLGPUDebugBuffer initWithBuffer:heap:device:offset:length:track:](self, "initWithBuffer:heap:device:offset:length:track:", a3, 0, a4, a5, a6, a7);
}

- (MTLGPUDebugBuffer)initWithBuffer:(id)a3 heap:(id)a4 device:(id)a5
{
  return -[MTLGPUDebugBuffer initWithBuffer:heap:device:offset:length:track:](self, "initWithBuffer:heap:device:offset:length:track:", a3, a4, a5, 0, objc_msgSend(a3, "length"), 1);
}
...
- (MTLGPUDebugBuffer)initWithBuffer:(id)a3 heap:(id)a4 device:(id)a5 offset:(unint64_t)a6 length:(unint64_t)a7 track:(BOOL)a8
{
  _BOOL4 v8;
  id v12;
  MTLGPUDebugBuffer *v14;
  MTLGPUDebugBuffer *v15;
  objc_super v17;
  ...
  v14 = -[MTLToolsResource initWithBaseObject:parent:heap:](&v17, sel_initWithBaseObject_parent_heap_, a3, a4, v12);
  v15 = v14;
  if (v14)
  {
    v14->_offset = a6;
    v14->_length = a7;
    v14->_bufferEndAddress = a7 + a6 + objc_msgSend(a3, "gpuAddress");
    if (v8 && (*((_BYTE *)a5 + 286) & 0x80) != 0)
    {
      v15->_descriptorHeap = (char *)a5 + 296;
      -[MTLGPUDebugBuffer setBufferIndex:](v15, "setBufferIndex:", GPUDebugBufferDescriptorHeap::createHandle((std::mutex *)((char *)a5 + 296), v15));
    }
  }
  return v15;
}
...
- (void)dealloc
{
  ...
  if (self->_descriptorHeap)
    -[MTLGPUDebugBuffer setBufferIndex:](self, "setBufferIndex:", 0);
  v3.receiver = self;
...
- (void)setBufferIndex:(unint64_t)a3
{
  ...
  if (bufferIndex != a3)
  {
    ...
    if (a3)
      MTLGPUDebugSetBufferForResourceHandle((MTLGPUDebugDevice *)self->super.super.super._device, a3, self);
  }
}
...
- (unint64_t)handleForOffset:(unint64_t)a3
{
  ...
  return a3
       + (bufferIndex << 44)
       + self->_offset
       + -[MTLToolsObject gpuAddress](self->super.super.super._baseObject, "gpuAddress");
}
...
{
  return -[MTLGPUDebugBuffer handleForOffset:](self, "handleForOffset:", 0);
}
