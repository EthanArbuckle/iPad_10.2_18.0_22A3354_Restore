...
- (BOOL)evaluateWithContext:(id)a3
{
  ...
  if (evaluateWithContext__sMobileCoreServices)
    return (objc_msgSend((id)objc_msgSend((id)-[objc_class documentProxyForName:type:MIMEType:](NSClassFromString(CFSTR("LSDocumentProxy")), "documentProxyForName:type:MIMEType:", 0, self->super._value, 0), "applicationsAvailableForOpeningWithError:", 0), "count") != 0) ^ (self->super._operator == 1);
  else
    return 0;
}
