@implementation SCWDatabaseJSONStore

void __41__SCWDatabaseJSONStore_readWithAccessor___block_invoke_2(uint64_t a1, void *a2)
{
  id v3;
  NSObject *v4;
  void *v5;
  id v6;
  _QWORD block[5];
  id v8;
  id v9;

  v3 = a2;
  objc_msgSend(*(id *)(a1 + 32), "accessQueue");
  v4 = objc_claimAutoreleasedReturnValue();
  block[0] = MEMORY[0x1E0C809B0];
  block[1] = 3221225472;
  block[2] = __41__SCWDatabaseJSONStore_readWithAccessor___block_invoke_3;
  block[3] = &unk_1E5AF1C18;
  v5 = *(void **)(a1 + 40);
  block[4] = *(_QWORD *)(a1 + 32);
  v8 = v3;
  v9 = v5;
  v6 = v3;
  dispatch_sync(v4, block);

}

- (void)setCloudBackupEnabled:(BOOL)a3
{
  self->_cloudBackupEnabled = a3;
  -[SCWDatabaseJSONStore _applyCloudBackupEnabledPropertyToStoreURL](self, "_applyCloudBackupEnabledPropertyToStoreURL");
}

- (void)readZone:(id)a3 withAccessor:(id)a4
{
  id v6;
  id v7;
  id v8;
  id v9;
  _QWORD v10[5];
  id v11;
  id v12;

  v6 = a3;
  v7 = a4;
  v10[0] = MEMORY[0x1E0C809B0];
  v10[1] = 3221225472;
  v10[2] = __46__SCWDatabaseJSONStore_readZone_withAccessor___block_invoke;
  v10[3] = &unk_1E5AF1C68;
  v11 = v6;
  v12 = v7;
  v10[4] = self;
  v8 = v6;
  v9 = v7;
  -[SCWDatabaseJSONStore readWithAccessor:](self, "readWithAccessor:", v10);

}

- (void)readWithAccessor:(id)a3
{
  id v4;
  NSObject *v5;
  uint64_t v6;
  id v7;
  void *v8;
  void *v9;
  id v10;
  _QWORD v11[5];
  id v12;
  id v13;
  _QWORD block[5];
  id v15;
  uint64_t *v16;
  uint64_t v17;
  uint64_t *v18;
  uint64_t v19;
  char v20;

  v4 = a3;
  v17 = 0;
  v18 = &v17;
  v19 = 0x2020000000;
  v20 = 0;
  -[SCWDatabaseJSONStore accessQueue](self, "accessQueue");
  v5 = objc_claimAutoreleasedReturnValue();
  v6 = MEMORY[0x1E0C809B0];
  block[0] = MEMORY[0x1E0C809B0];
  block[1] = 3221225472;
  block[2] = __41__SCWDatabaseJSONStore_readWithAccessor___block_invoke;
  block[3] = &unk_1E5AF1BF0;
  block[4] = self;
  v7 = v4;
  v15 = v7;
  v16 = &v17;
  dispatch_sync(v5, block);

  if (!*((_BYTE *)v18 + 24))
  {
    -[SCWDatabaseJSONStore fileCoordinator](self, "fileCoordinator");
    v8 = (void *)objc_claimAutoreleasedReturnValue();
    -[SCWDatabaseJSONStore storeURL](self, "storeURL");
    v9 = (void *)objc_claimAutoreleasedReturnValue();
    v13 = 0;
    v11[0] = v6;
    v11[1] = 3221225472;
    v11[2] = __41__SCWDatabaseJSONStore_readWithAccessor___block_invoke_2;
    v11[3] = &unk_1E5AF1C40;
    v11[4] = self;
    v12 = v7;
    objc_msgSend(v8, "coordinateReadingItemAtURL:options:error:byAccessor:", v9, 0, &v13, v11);
    v10 = v13;

  }
  _Block_object_dispose(&v17, 8);

}

- (OS_dispatch_queue)accessQueue
{
  return self->_accessQueue;
}

- (NSFileCoordinator)fileCoordinator
{
  return self->_fileCoordinator;
}

- (void)_applyCloudBackupEnabledPropertyToStoreURL
{
  const __CFString *v3;
  int v4;
  uint64_t v5;
  __int16 v6;
  const __CFString *v7;
  __int16 v8;
  uint64_t v9;
  uint64_t v10;

  v10 = *MEMORY[0x1E0C80C00];
  v3 = CFSTR("disable");
  v4 = 134218498;
  v5 = a1;
  v6 = 2114;
  if ((a2 & 1) != 0)
    v3 = CFSTR("enable");
  v7 = v3;
  v8 = 2114;
  v9 = a3;
  _os_log_error_impl(&dword_1ABCEF000, MEMORY[0x1E0C81028], OS_LOG_TYPE_ERROR, "%p failed to %{public}@ cloud backup for JSON database store: %{public}@", (uint8_t *)&v4, 0x20u);
}

- (NSURL)storeURL
{
  return self->_storeURL;
}

- (BOOL)isCloudBackupEnabled
{
  return self->_cloudBackupEnabled;
}

- (NSURL)presentedItemURL
{
  return (NSURL *)objc_getProperty(self, a2, 40, 1);
}

uint64_t __41__SCWDatabaseJSONStore_readWithAccessor___block_invoke_3(uint64_t a1)
{
  objc_msgSend(*(id *)(a1 + 32), "_reloadIfNeededFromFileURL:", *(_QWORD *)(a1 + 40));
  return (*(uint64_t (**)(void))(*(_QWORD *)(a1 + 48) + 16))();
}

- (BOOL)_reloadIfNeededFromFileURL:(id)a3
{
  id v4;
  id v5;
  unsigned int v6;
  unsigned int v7;
  BOOL v8;
  BOOL v9;
  unsigned int value;

  v4 = a3;
  if (objc_msgSend(v4, "checkResourceIsReachableAndReturnError:", 0))
  {
    value = 0;
    v5 = objc_retainAutorelease(v4);
    getxattr((const char *)objc_msgSend(v5, "fileSystemRepresentation"), "com.apple.stocks.jsonStoreChangeTag", &value, 4uLL, 0, 0);
    v6 = value;
    v7 = -[SCWDatabaseJSONStore lastKnownStoreChangeTag](self, "lastKnownStoreChangeTag");
    v8 = v6 == v7;
    v9 = v6 != v7;
    if (!v8)
    {
      -[SCWDatabaseJSONStore _loadFromFileURL:](self, "_loadFromFileURL:", v5);
      -[SCWDatabaseJSONStore setLastKnownStoreChangeTag:](self, "setLastKnownStoreChangeTag:", value);
    }
  }
  else
  {
    v9 = 0;
  }
  -[SCWDatabaseJSONStore setLoadedFromDisk:](self, "setLoadedFromDisk:", 1);

  return v9;
}

- (void)setLoadedFromDisk:(BOOL)a3
{
  self->_loadedFromDisk = a3;
}

- (void)setLastKnownStoreChangeTag:(unsigned int)a3
{
  self->_lastKnownStoreChangeTag = a3;
}

- (unsigned)lastKnownStoreChangeTag
{
  return self->_lastKnownStoreChangeTag;
}

uint64_t __41__SCWDatabaseJSONStore_readWithAccessor___block_invoke(uint64_t a1)
{
  uint64_t result;

  result = objc_msgSend(*(id *)(a1 + 32), "loadedFromDisk");
  if ((_DWORD)result)
  {
    result = (*(uint64_t (**)(void))(*(_QWORD *)(a1 + 40) + 16))();
    *(_BYTE *)(*(_QWORD *)(*(_QWORD *)(a1 + 48) + 8) + 24) = 1;
  }
  return result;
}

- (BOOL)loadedFromDisk
{
  return self->_loadedFromDisk;
}

void __46__SCWDatabaseJSONStore_readZone_withAccessor___block_invoke(uint64_t a1)
{
  uint64_t v1;
  id v2;

  v1 = *(_QWORD *)(a1 + 48);
  objc_msgSend(*(id *)(a1 + 32), "zoneStoreForSchema:", *(_QWORD *)(a1 + 40));
  v2 = (id)objc_claimAutoreleasedReturnValue();
  (*(void (**)(uint64_t, id))(v1 + 16))(v1, v2);

}

- (id)zoneStoreForSchema:(id)a3
{
  id v4;
  void *v5;
  void *v6;
  void *v7;

  v4 = a3;
  -[SCWDatabaseJSONStore zoneStoresByName](self, "zoneStoresByName");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "zoneName");
  v6 = (void *)objc_claimAutoreleasedReturnValue();

  objc_msgSend(v5, "objectForKeyedSubscript:", v6);
  v7 = (void *)objc_claimAutoreleasedReturnValue();

  return v7;
}

- (NSMutableDictionary)zoneStoresByName
{
  return self->_zoneStoresByName;
}

- (void)_loadFromFileURL:(id)a3
{
  id v4;
  void *v5;
  void *v6;
  id v7;
  void *v8;
  void *v9;
  void *v10;
  NSDate *v11;
  NSDate *lastSyncDate;
  void *v13;
  NSDate *v14;
  NSDate *lastDirtyDate;
  uint64_t v16;
  void *v17;
  CKServerChangeToken *v18;
  CKServerChangeToken *serverChangeToken;
  void *v20;
  uint64_t v21;
  uint64_t v22;
  uint64_t v23;
  uint64_t i;
  void *v25;
  void *v26;
  unint64_t v27;
  uint64_t j;
  void *v29;
  void *v30;
  void *v31;
  void *v32;
  void *v33;
  void *v34;
  uint64_t v35;
  uint64_t v36;
  uint64_t v37;
  uint64_t k;
  uint64_t v39;
  uint64_t v40;
  void *v41;
  id v42;
  void *v43;
  void *v44;
  uint64_t v45;
  uint64_t v46;
  uint64_t v47;
  uint64_t m;
  uint64_t v49;
  void *v50;
  void *v51;
  id v52;
  void *v53;
  void *v54;
  void *v55;
  uint64_t v56;
  void *v57;
  void *v58;
  void *v59;
  void *v60;
  void *v61;
  unint64_t v62;
  void *v63;
  uint64_t v64;
  uint64_t v65;
  void *v66;
  SCWZoneJSONStore *v67;
  void *v68;
  void *v69;
  void *v70;
  id obj;
  void *v72;
  void *v73;
  uint64_t v74;
  uint64_t v75;
  void *v76;
  void *v77;
  uint64_t v78;
  void *v79;
  void *v80;
  id v81;
  void *v82;
  void *v83;
  id v84;
  __int128 v85;
  __int128 v86;
  __int128 v87;
  __int128 v88;
  id v89;
  __int128 v90;
  __int128 v91;
  __int128 v92;
  __int128 v93;
  __int128 v94;
  __int128 v95;
  __int128 v96;
  __int128 v97;
  __int128 v98;
  __int128 v99;
  __int128 v100;
  __int128 v101;
  id v102;
  uint8_t buf[4];
  SCWDatabaseJSONStore *v104;
  __int16 v105;
  id v106;
  __int16 v107;
  uint64_t v108;
  __int16 v109;
  uint64_t v110;
  _BYTE v111[128];
  _BYTE v112[128];
  _BYTE v113[128];
  _BYTE v114[128];
  uint64_t v115;

  v115 = *MEMORY[0x1E0C80C00];
  v4 = a3;
  objc_msgSend(MEMORY[0x1E0C99E08], "dictionary");
  v79 = (void *)objc_claimAutoreleasedReturnValue();
  v73 = (void *)objc_opt_new();
  v69 = v4;
  objc_msgSend(MEMORY[0x1E0CB3940], "stringWithContentsOfURL:encoding:error:", v4, 4, 0);
  v70 = (void *)objc_claimAutoreleasedReturnValue();
  if (v70)
  {
    objc_msgSend(v70, "dataUsingEncoding:", 4);
    v5 = (void *)objc_claimAutoreleasedReturnValue();
    v102 = 0;
    objc_msgSend(MEMORY[0x1E0CB36D8], "JSONObjectWithData:options:error:", v5, 0, &v102);
    v6 = (void *)objc_claimAutoreleasedReturnValue();
    v7 = v102;
    v8 = v7;
    if (v6)
    {
      v81 = v7;
      v83 = v5;
      objc_msgSend(v6, "objectForKeyedSubscript:", CFSTR("database"));
      v9 = (void *)objc_claimAutoreleasedReturnValue();
      objc_msgSend(v9, "objectForKeyedSubscript:", CFSTR("lastSyncDate"));
      v10 = (void *)objc_claimAutoreleasedReturnValue();
      objc_msgSend(v73, "dateFromString:", v10);
      v11 = (NSDate *)objc_claimAutoreleasedReturnValue();
      lastSyncDate = self->_lastSyncDate;
      self->_lastSyncDate = v11;

      objc_msgSend(v9, "objectForKeyedSubscript:", CFSTR("lastDirtyDate"));
      v13 = (void *)objc_claimAutoreleasedReturnValue();
      objc_msgSend(v73, "dateFromString:", v13);
      v14 = (NSDate *)objc_claimAutoreleasedReturnValue();
      lastDirtyDate = self->_lastDirtyDate;
      self->_lastDirtyDate = v14;

      v16 = objc_opt_class();
      objc_msgSend(v9, "objectForKeyedSubscript:", CFSTR("serverChangeToken"));
      v17 = (void *)objc_claimAutoreleasedReturnValue();
      -[SCWDatabaseJSONStore _decodeCodableObjectOfClass:from:error:](self, "_decodeCodableObjectOfClass:from:error:", v16, v17, 0);
      v18 = (CKServerChangeToken *)objc_claimAutoreleasedReturnValue();
      serverChangeToken = self->_serverChangeToken;
      self->_serverChangeToken = v18;

      v100 = 0u;
      v101 = 0u;
      v98 = 0u;
      v99 = 0u;
      objc_msgSend(v9, "objectForKeyedSubscript:", CFSTR("zones"));
      v20 = (void *)objc_claimAutoreleasedReturnValue();
      v21 = objc_msgSend(v20, "countByEnumeratingWithState:objects:count:", &v98, v114, 16);
      if (v21)
      {
        v22 = v21;
        v23 = *(_QWORD *)v99;
        do
        {
          for (i = 0; i != v22; ++i)
          {
            if (*(_QWORD *)v99 != v23)
              objc_enumerationMutation(v20);
            v25 = *(void **)(*((_QWORD *)&v98 + 1) + 8 * i);
            objc_msgSend(v25, "objectForKeyedSubscript:", CFSTR("name"));
            v26 = (void *)objc_claimAutoreleasedReturnValue();
            objc_msgSend(v79, "setObject:forKeyedSubscript:", v25, v26);

          }
          v22 = objc_msgSend(v20, "countByEnumeratingWithState:objects:count:", &v98, v114, 16);
        }
        while (v22);
      }

      v8 = v81;
      v5 = v83;
    }
    else if (os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_ERROR))
    {
      -[SCWDatabaseJSONStore _loadFromFileURL:].cold.1();
    }

  }
  v96 = 0u;
  v97 = 0u;
  v94 = 0u;
  v95 = 0u;
  objc_msgSend(v79, "allKeys");
  obj = (id)objc_claimAutoreleasedReturnValue();
  v75 = objc_msgSend(obj, "countByEnumeratingWithState:objects:count:", &v94, v113, 16);
  if (v75)
  {
    v74 = *(_QWORD *)v95;
    v27 = 0x1E0C99000uLL;
    do
    {
      for (j = 0; j != v75; ++j)
      {
        if (*(_QWORD *)v95 != v74)
          objc_enumerationMutation(obj);
        v29 = *(void **)(*((_QWORD *)&v94 + 1) + 8 * j);
        objc_msgSend(v79, "objectForKeyedSubscript:", v29);
        v30 = (void *)objc_claimAutoreleasedReturnValue();
        -[SCWDatabaseJSONStore zoneStoresByName](self, "zoneStoresByName");
        v31 = (void *)objc_claimAutoreleasedReturnValue();
        v82 = v29;
        objc_msgSend(v31, "objectForKeyedSubscript:", v29);
        v32 = (void *)objc_claimAutoreleasedReturnValue();

        if (v32)
        {
          v78 = j;
          objc_msgSend(*(id *)(v27 + 3560), "array");
          v33 = (void *)objc_claimAutoreleasedReturnValue();
          v90 = 0u;
          v91 = 0u;
          v92 = 0u;
          v93 = 0u;
          v80 = v30;
          objc_msgSend(v30, "objectForKeyedSubscript:", CFSTR("serverRecords"));
          v34 = (void *)objc_claimAutoreleasedReturnValue();
          v35 = objc_msgSend(v34, "countByEnumeratingWithState:objects:count:", &v90, v112, 16);
          if (v35)
          {
            v36 = v35;
            v37 = *(_QWORD *)v91;
            do
            {
              for (k = 0; k != v36; ++k)
              {
                if (*(_QWORD *)v91 != v37)
                  objc_enumerationMutation(v34);
                v39 = *(_QWORD *)(*((_QWORD *)&v90 + 1) + 8 * k);
                v40 = objc_opt_class();
                v89 = 0;
                -[SCWDatabaseJSONStore _decodeCodableObjectOfClass:from:error:](self, "_decodeCodableObjectOfClass:from:error:", v40, v39, &v89);
                v41 = (void *)objc_claimAutoreleasedReturnValue();
                v42 = v89;
                if (v41)
                {
                  objc_msgSend(v33, "addObject:", v41);
                }
                else if (os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_ERROR))
                {
                  *(_DWORD *)buf = 134218242;
                  v104 = self;
                  v105 = 2114;
                  v106 = v42;
                  _os_log_error_impl(&dword_1ABCEF000, MEMORY[0x1E0C81028], OS_LOG_TYPE_ERROR, "%p failed to decode server record with error: %{public}@", buf, 0x16u);
                }

              }
              v36 = objc_msgSend(v34, "countByEnumeratingWithState:objects:count:", &v90, v112, 16);
            }
            while (v36);
          }

          objc_msgSend(*(id *)(v27 + 3560), "array");
          v43 = (void *)objc_claimAutoreleasedReturnValue();
          v85 = 0u;
          v86 = 0u;
          v87 = 0u;
          v88 = 0u;
          objc_msgSend(v80, "objectForKeyedSubscript:", CFSTR("pendingCommands"));
          v44 = (void *)objc_claimAutoreleasedReturnValue();
          v45 = objc_msgSend(v44, "countByEnumeratingWithState:objects:count:", &v85, v111, 16);
          if (v45)
          {
            v46 = v45;
            v47 = *(_QWORD *)v86;
            do
            {
              for (m = 0; m != v46; ++m)
              {
                if (*(_QWORD *)v86 != v47)
                  objc_enumerationMutation(v44);
                v49 = *(_QWORD *)(*((_QWORD *)&v85 + 1) + 8 * m);
                -[SCWDatabaseJSONStore allowedCommandClasses](self, "allowedCommandClasses");
                v50 = (void *)objc_claimAutoreleasedReturnValue();
                v84 = 0;
                -[SCWDatabaseJSONStore _decodeCodableObjectOfClasses:from:error:](self, "_decodeCodableObjectOfClasses:from:error:", v50, v49, &v84);
                v51 = (void *)objc_claimAutoreleasedReturnValue();
                v52 = v84;

                if (v51)
                {
                  objc_msgSend(v43, "addObject:", v51);
                }
                else if (os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_ERROR))
                {
                  *(_DWORD *)buf = 134218242;
                  v104 = self;
                  v105 = 2114;
                  v106 = v52;
                  _os_log_error_impl(&dword_1ABCEF000, MEMORY[0x1E0C81028], OS_LOG_TYPE_ERROR, "%p failed to decode command with error: %{public}@", buf, 0x16u);
                }

              }
              v46 = objc_msgSend(v44, "countByEnumeratingWithState:objects:count:", &v85, v111, 16);
            }
            while (v46);
          }

          objc_msgSend(v80, "objectForKeyedSubscript:", CFSTR("lastSyncDate"));
          v53 = (void *)objc_claimAutoreleasedReturnValue();
          objc_msgSend(v73, "dateFromString:", v53);
          v77 = (void *)objc_claimAutoreleasedReturnValue();

          objc_msgSend(v80, "objectForKeyedSubscript:", CFSTR("lastDirtyDate"));
          v54 = (void *)objc_claimAutoreleasedReturnValue();
          objc_msgSend(v73, "dateFromString:", v54);
          v55 = (void *)objc_claimAutoreleasedReturnValue();

          v56 = objc_opt_class();
          objc_msgSend(v80, "objectForKeyedSubscript:", CFSTR("serverChangeToken"));
          v57 = (void *)objc_claimAutoreleasedReturnValue();
          -[SCWDatabaseJSONStore _decodeCodableObjectOfClass:from:error:](self, "_decodeCodableObjectOfClass:from:error:", v56, v57, 0);
          v58 = (void *)objc_claimAutoreleasedReturnValue();

          objc_msgSend(v80, "objectForKeyedSubscript:", CFSTR("schemaVersion"));
          v59 = (void *)objc_claimAutoreleasedReturnValue();
          -[SCWDatabaseJSONStore databaseSchema](self, "databaseSchema");
          v60 = (void *)objc_claimAutoreleasedReturnValue();
          objc_msgSend(v60, "schemaForZoneName:", v82);
          v61 = (void *)objc_claimAutoreleasedReturnValue();

          v62 = objc_msgSend(v61, "version");
          v76 = v59;
          if (v62 <= objc_msgSend(v59, "unsignedIntegerValue"))
          {
            v66 = v77;
            j = v78;
          }
          else
          {
            v72 = v58;
            j = v78;
            if (os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_DEFAULT))
            {
              objc_msgSend(v61, "zoneName");
              v63 = (void *)objc_claimAutoreleasedReturnValue();
              v64 = objc_msgSend(v59, "unsignedIntegerValue");
              v65 = objc_msgSend(v61, "version");
              *(_DWORD *)buf = 134218754;
              v104 = self;
              v105 = 2114;
              v106 = v63;
              v107 = 2048;
              v108 = v64;
              v109 = 2048;
              v110 = v65;
              _os_log_impl(&dword_1ABCEF000, MEMORY[0x1E0C81028], OS_LOG_TYPE_DEFAULT, "%p discarding sync state for zone %{public}@ due to version mismatch, persisted=%lu, current=%lu", buf, 0x2Au);

            }
            v66 = 0;
            v55 = 0;
            v58 = 0;
          }
          v67 = -[SCWZoneJSONStore initWithZoneSchema:serverRecords:lastSyncDate:lastDirtyDate:serverChangeToken:pendingCommands:]([SCWZoneJSONStore alloc], "initWithZoneSchema:serverRecords:lastSyncDate:lastDirtyDate:serverChangeToken:pendingCommands:", v61, v33, v66, v55, v58, v43);
          -[SCWDatabaseJSONStore zoneStoresByName](self, "zoneStoresByName");
          v68 = (void *)objc_claimAutoreleasedReturnValue();
          objc_msgSend(v68, "setObject:forKeyedSubscript:", v67, v82);

          v27 = 0x1E0C99000;
          v30 = v80;
        }
        else
        {
          if (os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_DEFAULT))
          {
            *(_DWORD *)buf = 134218242;
            v104 = self;
            v105 = 2114;
            v106 = v82;
            _os_log_impl(&dword_1ABCEF000, MEMORY[0x1E0C81028], OS_LOG_TYPE_DEFAULT, "%p preserving unknown zone %{public}@", buf, 0x16u);
          }
          -[SCWDatabaseJSONStore foreignZoneDictsByName](self, "foreignZoneDictsByName");
          v33 = (void *)objc_claimAutoreleasedReturnValue();
          objc_msgSend(v33, "setObject:forKeyedSubscript:", v30, v82);
        }

      }
      v75 = objc_msgSend(obj, "countByEnumeratingWithState:objects:count:", &v94, v113, 16);
    }
    while (v75);
  }

  ++self->_diskReadCount;
}

- (NSSet)allowedCommandClasses
{
  return self->_allowedCommandClasses;
}

- (id)_decodeCodableObjectOfClasses:(id)a3 from:(id)a4 error:(id *)a5
{
  id v7;
  id v8;
  void *v9;
  void *v10;

  v7 = a3;
  v8 = a4;
  if (objc_msgSend(v8, "length"))
  {
    v9 = (void *)objc_msgSend(objc_alloc(MEMORY[0x1E0C99D50]), "initWithBase64EncodedString:options:", v8, 0);
    objc_msgSend(MEMORY[0x1E0CB3710], "unarchivedObjectOfClasses:fromData:error:", v7, v9, a5);
    v10 = (void *)objc_claimAutoreleasedReturnValue();

  }
  else
  {
    v10 = 0;
  }

  return v10;
}

- (id)_decodeCodableObjectOfClass:(Class)a3 from:(id)a4 error:(id *)a5
{
  id v7;
  void *v8;
  void *v9;

  v7 = a4;
  if (objc_msgSend(v7, "length"))
  {
    v8 = (void *)objc_msgSend(objc_alloc(MEMORY[0x1E0C99D50]), "initWithBase64EncodedString:options:", v7, 0);
    objc_msgSend(MEMORY[0x1E0CB3710], "unarchivedObjectOfClass:fromData:error:", a3, v8, a5);
    v9 = (void *)objc_claimAutoreleasedReturnValue();

  }
  else
  {
    v9 = 0;
  }

  return v9;
}

- (SCWDatabaseSchema)databaseSchema
{
  return self->_databaseSchema;
}

- (SCWDatabaseJSONStore)initWithSchema:(id)a3 fileURL:(id)a4 allowedCommandClasses:(id)a5
{
  id v9;
  id v10;
  id v11;
  SCWDatabaseJSONStore *v12;
  SCWDatabaseJSONStore *v13;
  NSMutableDictionary *v14;
  NSMutableDictionary *zoneStoresByName;
  NSMutableDictionary *v16;
  NSMutableDictionary *foreignZoneDictsByName;
  uint64_t v18;
  NSHashTable *observers;
  NSObject *v20;
  dispatch_queue_t v21;
  OS_dispatch_queue *accessQueue;
  NSObject *v23;
  NSObject *v24;
  dispatch_queue_t v25;
  OS_dispatch_queue *workQueue;
  SCWOperationThrottler *v27;
  SCWOperationThrottler *saveThrottler;
  uint64_t v29;
  NSFileCoordinator *fileCoordinator;
  uint64_t v31;
  NSOperationQueue *presentedItemOperationQueue;
  uint64_t v33;
  NSSet *allowedCommandClasses;
  void *v35;
  uint64_t v36;
  uint64_t v37;
  uint64_t v38;
  uint64_t i;
  void *v40;
  SCWZoneJSONStore *v41;
  void *v42;
  void *v43;
  id v45;
  __int128 v46;
  __int128 v47;
  __int128 v48;
  __int128 v49;
  objc_super v50;
  _BYTE v51[128];
  uint64_t v52;

  v52 = *MEMORY[0x1E0C80C00];
  v9 = a3;
  v10 = a4;
  v11 = a5;
  v50.receiver = self;
  v50.super_class = (Class)SCWDatabaseJSONStore;
  v12 = -[SCWDatabaseJSONStore init](&v50, sel_init);
  v13 = v12;
  if (v12)
  {
    v45 = v10;
    objc_storeStrong((id *)&v12->_storeURL, a4);
    objc_storeStrong((id *)&v13->_databaseSchema, a3);
    v14 = (NSMutableDictionary *)objc_alloc_init(MEMORY[0x1E0C99E08]);
    zoneStoresByName = v13->_zoneStoresByName;
    v13->_zoneStoresByName = v14;

    v16 = (NSMutableDictionary *)objc_alloc_init(MEMORY[0x1E0C99E08]);
    foreignZoneDictsByName = v13->_foreignZoneDictsByName;
    v13->_foreignZoneDictsByName = v16;

    v18 = objc_msgSend(objc_alloc(MEMORY[0x1E0CB3690]), "initWithOptions:capacity:", 517, 0);
    observers = v13->_observers;
    v13->_observers = (NSHashTable *)v18;

    dispatch_queue_attr_make_with_autorelease_frequency(0, DISPATCH_AUTORELEASE_FREQUENCY_WORK_ITEM);
    v20 = objc_claimAutoreleasedReturnValue();
    v21 = dispatch_queue_create("SCWDatabaseJSONStore.access", v20);
    accessQueue = v13->_accessQueue;
    v13->_accessQueue = (OS_dispatch_queue *)v21;

    dispatch_queue_attr_make_with_autorelease_frequency(0, DISPATCH_AUTORELEASE_FREQUENCY_WORK_ITEM);
    v23 = objc_claimAutoreleasedReturnValue();
    dispatch_queue_attr_make_with_qos_class(v23, QOS_CLASS_BACKGROUND, 0);
    v24 = objc_claimAutoreleasedReturnValue();
    v25 = dispatch_queue_create("SCWDatabaseJSONStore.work", v24);
    workQueue = v13->_workQueue;
    v13->_workQueue = (OS_dispatch_queue *)v25;

    v27 = -[SCWOperationThrottler initWithDelegate:updateQueue:]([SCWOperationThrottler alloc], "initWithDelegate:updateQueue:", v13, v13->_workQueue);
    saveThrottler = v13->_saveThrottler;
    v13->_saveThrottler = v27;

    v29 = objc_msgSend(objc_alloc(MEMORY[0x1E0CB3600]), "initWithFilePresenter:", v13);
    fileCoordinator = v13->_fileCoordinator;
    v13->_fileCoordinator = (NSFileCoordinator *)v29;

    objc_storeStrong((id *)&v13->_presentedItemURL, v13->_storeURL);
    v31 = objc_opt_new();
    presentedItemOperationQueue = v13->_presentedItemOperationQueue;
    v13->_presentedItemOperationQueue = (NSOperationQueue *)v31;

    -[NSOperationQueue setMaxConcurrentOperationCount:](v13->_presentedItemOperationQueue, "setMaxConcurrentOperationCount:", 1);
    if (objc_msgSend(v11, "count"))
      objc_msgSend(MEMORY[0x1E0C99E60], "setWithArray:", v11);
    else
      objc_msgSend(MEMORY[0x1E0C99E60], "setWithObject:", objc_opt_class());
    v33 = objc_claimAutoreleasedReturnValue();
    allowedCommandClasses = v13->_allowedCommandClasses;
    v13->_allowedCommandClasses = (NSSet *)v33;

    v48 = 0u;
    v49 = 0u;
    v46 = 0u;
    v47 = 0u;
    objc_msgSend(v9, "zoneSchemas");
    v35 = (void *)objc_claimAutoreleasedReturnValue();
    v36 = objc_msgSend(v35, "countByEnumeratingWithState:objects:count:", &v46, v51, 16);
    if (v36)
    {
      v37 = v36;
      v38 = *(_QWORD *)v47;
      do
      {
        for (i = 0; i != v37; ++i)
        {
          if (*(_QWORD *)v47 != v38)
            objc_enumerationMutation(v35);
          v40 = *(void **)(*((_QWORD *)&v46 + 1) + 8 * i);
          v41 = -[SCWZoneJSONStore initWithZoneSchema:serverRecords:lastSyncDate:lastDirtyDate:serverChangeToken:pendingCommands:]([SCWZoneJSONStore alloc], "initWithZoneSchema:serverRecords:lastSyncDate:lastDirtyDate:serverChangeToken:pendingCommands:", v40, 0, 0, 0, 0, 0);
          -[SCWDatabaseJSONStore zoneStoresByName](v13, "zoneStoresByName");
          v42 = (void *)objc_claimAutoreleasedReturnValue();
          objc_msgSend(v40, "zoneName");
          v43 = (void *)objc_claimAutoreleasedReturnValue();
          objc_msgSend(v42, "setObject:forKeyedSubscript:", v41, v43);

        }
        v37 = objc_msgSend(v35, "countByEnumeratingWithState:objects:count:", &v46, v51, 16);
      }
      while (v37);
    }

    -[SCWDatabaseJSONStore _listenForChangesToFileURL:](v13, "_listenForChangesToFileURL:", v13->_storeURL);
    v10 = v45;
  }

  return v13;
}

- (void)_listenForChangesToFileURL:(id)a3
{
  id v4;
  void *v5;
  id v6;
  int v7;
  NSObject *v8;
  dispatch_source_t v9;
  NSObject *v10;
  uint64_t v11;
  id v12;
  NSObject *v13;
  NSObject *v14;
  _QWORD v15[4];
  int v16;
  _QWORD handler[5];
  id v18;

  v4 = a3;
  objc_msgSend(v4, "path");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v5, "stringByDeletingLastPathComponent");
  v6 = objc_retainAutorelease((id)objc_claimAutoreleasedReturnValue());
  v7 = open((const char *)objc_msgSend(v6, "fileSystemRepresentation"), 0x8000);

  -[SCWDatabaseJSONStore workQueue](self, "workQueue");
  v8 = objc_claimAutoreleasedReturnValue();
  v9 = dispatch_source_create(MEMORY[0x1E0C80DE8], v7, 0xEuLL, v8);
  -[SCWDatabaseJSONStore setChangeListenerSource:](self, "setChangeListenerSource:", v9);

  -[SCWDatabaseJSONStore changeListenerSource](self, "changeListenerSource");
  v10 = objc_claimAutoreleasedReturnValue();
  v11 = MEMORY[0x1E0C809B0];
  handler[0] = MEMORY[0x1E0C809B0];
  handler[1] = 3221225472;
  handler[2] = __51__SCWDatabaseJSONStore__listenForChangesToFileURL___block_invoke;
  handler[3] = &unk_1E5AF1C90;
  handler[4] = self;
  v18 = v4;
  v12 = v4;
  dispatch_source_set_event_handler(v10, handler);

  -[SCWDatabaseJSONStore changeListenerSource](self, "changeListenerSource");
  v13 = objc_claimAutoreleasedReturnValue();
  v15[0] = v11;
  v15[1] = 3221225472;
  v15[2] = __51__SCWDatabaseJSONStore__listenForChangesToFileURL___block_invoke_47;
  v15[3] = &__block_descriptor_36_e5_v8__0l;
  v16 = v7;
  dispatch_source_set_cancel_handler(v13, v15);

  -[SCWDatabaseJSONStore changeListenerSource](self, "changeListenerSource");
  v14 = objc_claimAutoreleasedReturnValue();
  dispatch_resume(v14);

}

- (OS_dispatch_source)changeListenerSource
{
  return self->_changeListenerSource;
}

- (OS_dispatch_queue)workQueue
{
  return self->_workQueue;
}

- (void)setChangeListenerSource:(id)a3
{
  objc_storeStrong((id *)&self->_changeListenerSource, a3);
}

- (void)addObserver:(id)a3
{
  id v4;
  id v5;

  v4 = a3;
  -[SCWDatabaseJSONStore observers](self, "observers");
  v5 = (id)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v5, "addObject:", v4);

}

- (NSHashTable)observers
{
  return self->_observers;
}

+ (id)preferredFileURLForSchema:(id)a3 parentDirectoryURL:(id)a4
{
  void *v5;
  id v6;
  id v7;
  void *v8;
  uint64_t v9;
  const __CFString *v10;
  void *v11;
  void *v12;

  v5 = (void *)MEMORY[0x1E0CB3940];
  v6 = a4;
  v7 = a3;
  objc_msgSend(v7, "name");
  v8 = (void *)objc_claimAutoreleasedReturnValue();
  v9 = objc_msgSend(v7, "environment");

  v10 = CFSTR("sandbox");
  if (v9 == 1)
    v10 = CFSTR("production");
  objc_msgSend(v5, "stringWithFormat:", CFSTR("%@-%@-dbstore.json"), v8, v10);
  v11 = (void *)objc_claimAutoreleasedReturnValue();

  objc_msgSend(v6, "URLByAppendingPathComponent:", v11);
  v12 = (void *)objc_claimAutoreleasedReturnValue();

  return v12;
}

- (SCWDatabaseJSONStore)initWithSchema:(id)a3 parentDirectoryURL:(id)a4 allowedCommandClasses:(id)a5
{
  id v8;
  id v9;
  id v10;
  void *v11;
  SCWDatabaseJSONStore *v12;

  v8 = a5;
  v9 = a4;
  v10 = a3;
  objc_msgSend((id)objc_opt_class(), "preferredFileURLForSchema:parentDirectoryURL:", v10, v9);
  v11 = (void *)objc_claimAutoreleasedReturnValue();

  v12 = -[SCWDatabaseJSONStore initWithSchema:fileURL:allowedCommandClasses:](self, "initWithSchema:fileURL:allowedCommandClasses:", v10, v11, v8);
  return v12;
}

- (SCWDatabaseJSONStore)initWithSchema:(id)a3 fileURL:(id)a4
{
  return -[SCWDatabaseJSONStore initWithSchema:fileURL:allowedCommandClasses:](self, "initWithSchema:fileURL:allowedCommandClasses:", a3, a4, 0);
}

- (SCWDatabaseJSONStore)initWithSchema:(id)a3 parentDirectoryURL:(id)a4
{
  return -[SCWDatabaseJSONStore initWithSchema:parentDirectoryURL:allowedCommandClasses:](self, "initWithSchema:parentDirectoryURL:allowedCommandClasses:", a3, a4, 0);
}

- (void)writeWithAccessor:(id)a3
{
  id v4;
  void *v5;
  void *v6;
  id v7;
  id v8;
  void *v9;
  _QWORD v10[5];
  id v11;
  id v12;

  v4 = a3;
  -[SCWDatabaseJSONStore fileCoordinator](self, "fileCoordinator");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  -[SCWDatabaseJSONStore storeURL](self, "storeURL");
  v6 = (void *)objc_claimAutoreleasedReturnValue();
  v12 = 0;
  v10[0] = MEMORY[0x1E0C809B0];
  v10[1] = 3221225472;
  v10[2] = __42__SCWDatabaseJSONStore_writeWithAccessor___block_invoke;
  v10[3] = &unk_1E5AF1C40;
  v10[4] = self;
  v11 = v4;
  v7 = v4;
  objc_msgSend(v5, "coordinateReadingItemAtURL:options:error:byAccessor:", v6, 0, &v12, v10);
  v8 = v12;

  -[SCWDatabaseJSONStore saveThrottler](self, "saveThrottler");
  v9 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v9, "tickle");

}

void __42__SCWDatabaseJSONStore_writeWithAccessor___block_invoke(uint64_t a1, void *a2)
{
  id v3;
  NSObject *v4;
  void *v5;
  id v6;
  _QWORD block[5];
  id v8;
  id v9;

  v3 = a2;
  objc_msgSend(*(id *)(a1 + 32), "accessQueue");
  v4 = objc_claimAutoreleasedReturnValue();
  block[0] = MEMORY[0x1E0C809B0];
  block[1] = 3221225472;
  block[2] = __42__SCWDatabaseJSONStore_writeWithAccessor___block_invoke_2;
  block[3] = &unk_1E5AF1C18;
  v5 = *(void **)(a1 + 40);
  block[4] = *(_QWORD *)(a1 + 32);
  v8 = v3;
  v9 = v5;
  v6 = v3;
  dispatch_sync(v4, block);

}

uint64_t __42__SCWDatabaseJSONStore_writeWithAccessor___block_invoke_2(uint64_t a1)
{
  uint64_t result;

  objc_msgSend(*(id *)(a1 + 32), "_reloadIfNeededFromFileURL:", *(_QWORD *)(a1 + 40));
  (*(void (**)(void))(*(_QWORD *)(a1 + 48) + 16))();
  result = objc_msgSend(*(id *)(a1 + 32), "havePendingChanges");
  if ((result & 1) == 0)
  {
    objc_msgSend(*(id *)(a1 + 32), "setHavePendingChanges:", 1);
    return objc_msgSend(MEMORY[0x1E0CB3600], "addFilePresenter:", *(_QWORD *)(a1 + 32));
  }
  return result;
}

- (void)reloadWithAccessor:(id)a3
{
  id v4;
  void *v5;
  void *v6;
  id v7;
  id v8;
  _QWORD v9[5];
  id v10;
  id v11;

  v4 = a3;
  -[SCWDatabaseJSONStore fileCoordinator](self, "fileCoordinator");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  -[SCWDatabaseJSONStore storeURL](self, "storeURL");
  v6 = (void *)objc_claimAutoreleasedReturnValue();
  v11 = 0;
  v9[0] = MEMORY[0x1E0C809B0];
  v9[1] = 3221225472;
  v9[2] = __43__SCWDatabaseJSONStore_reloadWithAccessor___block_invoke;
  v9[3] = &unk_1E5AF1C40;
  v9[4] = self;
  v10 = v4;
  v7 = v4;
  objc_msgSend(v5, "coordinateReadingItemAtURL:options:error:byAccessor:", v6, 0, &v11, v9);
  v8 = v11;

}

void __43__SCWDatabaseJSONStore_reloadWithAccessor___block_invoke(uint64_t a1, void *a2)
{
  id v3;
  NSObject *v4;
  void *v5;
  id v6;
  _QWORD block[5];
  id v8;
  id v9;

  v3 = a2;
  objc_msgSend(*(id *)(a1 + 32), "accessQueue");
  v4 = objc_claimAutoreleasedReturnValue();
  block[0] = MEMORY[0x1E0C809B0];
  block[1] = 3221225472;
  block[2] = __43__SCWDatabaseJSONStore_reloadWithAccessor___block_invoke_2;
  block[3] = &unk_1E5AF1C18;
  v5 = *(void **)(a1 + 40);
  block[4] = *(_QWORD *)(a1 + 32);
  v8 = v3;
  v9 = v5;
  v6 = v3;
  dispatch_sync(v4, block);

}

uint64_t __43__SCWDatabaseJSONStore_reloadWithAccessor___block_invoke_2(uint64_t a1)
{
  objc_msgSend(*(id *)(a1 + 32), "_reloadIfNeededFromFileURL:", *(_QWORD *)(a1 + 40));
  return (*(uint64_t (**)(void))(*(_QWORD *)(a1 + 48) + 16))();
}

- (void)writeZone:(id)a3 withAccessor:(id)a4
{
  id v6;
  id v7;
  id v8;
  id v9;
  _QWORD v10[5];
  id v11;
  id v12;

  v6 = a3;
  v7 = a4;
  v10[0] = MEMORY[0x1E0C809B0];
  v10[1] = 3221225472;
  v10[2] = __47__SCWDatabaseJSONStore_writeZone_withAccessor___block_invoke;
  v10[3] = &unk_1E5AF1C68;
  v11 = v6;
  v12 = v7;
  v10[4] = self;
  v8 = v6;
  v9 = v7;
  -[SCWDatabaseJSONStore writeWithAccessor:](self, "writeWithAccessor:", v10);

}

void __47__SCWDatabaseJSONStore_writeZone_withAccessor___block_invoke(uint64_t a1)
{
  uint64_t v1;
  id v2;

  v1 = *(_QWORD *)(a1 + 48);
  objc_msgSend(*(id *)(a1 + 32), "zoneStoreForSchema:", *(_QWORD *)(a1 + 40));
  v2 = (id)objc_claimAutoreleasedReturnValue();
  (*(void (**)(uint64_t, id))(v1 + 16))(v1, v2);

}

- (void)removeObserver:(id)a3
{
  id v4;
  id v5;

  v4 = a3;
  -[SCWDatabaseJSONStore observers](self, "observers");
  v5 = (id)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v5, "removeObject:", v4);

}

- (void)operationThrottlerPerformOperation:(id)a3
{
  void *v4;
  int v5;
  SCWDatabaseJSONStore *v6;
  uint64_t v7;

  v7 = *MEMORY[0x1E0C80C00];
  if (os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_DEFAULT))
  {
    v5 = 134217984;
    v6 = self;
    _os_log_impl(&dword_1ABCEF000, MEMORY[0x1E0C81028], OS_LOG_TYPE_DEFAULT, "%p JSON store will flush changes to disk", (uint8_t *)&v5, 0xCu);
  }
  -[SCWDatabaseJSONStore storeURL](self, "storeURL");
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  -[SCWDatabaseJSONStore _saveIfNeededToFileURL:](self, "_saveIfNeededToFileURL:", v4);

}

- (void)savePresentedItemChangesWithCompletionHandler:(id)a3
{
  void (**v4)(id, _QWORD);
  void *v5;
  int v6;
  SCWDatabaseJSONStore *v7;
  uint64_t v8;

  v8 = *MEMORY[0x1E0C80C00];
  v4 = (void (**)(id, _QWORD))a3;
  if (os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_DEFAULT))
  {
    v6 = 134217984;
    v7 = self;
    _os_log_impl(&dword_1ABCEF000, MEMORY[0x1E0C81028], OS_LOG_TYPE_DEFAULT, "%p JSON store will flush changes to disk for another reader", (uint8_t *)&v6, 0xCu);
  }
  -[SCWDatabaseJSONStore storeURL](self, "storeURL");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  -[SCWDatabaseJSONStore _saveIfNeededToFileURL:](self, "_saveIfNeededToFileURL:", v5);

  v4[2](v4, 0);
}

- (BOOL)flushToDiskWithTimeout:(double)a3
{
  void *v4;

  -[SCWDatabaseJSONStore storeURL](self, "storeURL", a3);
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  -[SCWDatabaseJSONStore _saveIfNeededToFileURL:](self, "_saveIfNeededToFileURL:", v4);

  return 1;
}

- (void)_saveToFileURL:(id)a3
{
  void *v4;
  void *v5;
  void *v6;
  uint64_t v7;
  uint64_t v8;
  uint64_t v9;
  uint64_t i;
  void *v11;
  void *v12;
  uint64_t j;
  void *v14;
  void *v15;
  void *v16;
  uint64_t v17;
  uint64_t v18;
  uint64_t v19;
  uint64_t k;
  void *v21;
  void *v22;
  void *v23;
  uint64_t v24;
  uint64_t v25;
  uint64_t v26;
  uint64_t m;
  void *v28;
  void *v29;
  void *v30;
  void *v31;
  void *v32;
  void *v33;
  void *v34;
  void *v35;
  void *v36;
  void *v37;
  void *v38;
  void *v39;
  void *v40;
  void *v41;
  void *v42;
  void *v43;
  void *v44;
  void *v45;
  void *v46;
  void *v47;
  void *v48;
  void *v49;
  id v50;
  void *v51;
  int v52;
  id v53;
  void *v54;
  id v55;
  id obj;
  uint64_t v57;
  void *v58;
  void *v59;
  uint64_t v60;
  uint32_t value;
  id v62;
  id v63;
  __int128 v64;
  __int128 v65;
  __int128 v66;
  __int128 v67;
  __int128 v68;
  __int128 v69;
  __int128 v70;
  __int128 v71;
  __int128 v72;
  __int128 v73;
  __int128 v74;
  __int128 v75;
  __int128 v76;
  __int128 v77;
  __int128 v78;
  __int128 v79;
  _BYTE v80[128];
  _BYTE v81[128];
  _BYTE v82[128];
  _BYTE v83[128];
  uint64_t v84;

  v84 = *MEMORY[0x1E0C80C00];
  v55 = a3;
  v59 = (void *)objc_opt_new();
  objc_msgSend(MEMORY[0x1E0C99DE8], "array");
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  v76 = 0u;
  v77 = 0u;
  v78 = 0u;
  v79 = 0u;
  -[SCWDatabaseJSONStore foreignZoneDictsByName](self, "foreignZoneDictsByName");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v5, "allValues");
  v6 = (void *)objc_claimAutoreleasedReturnValue();

  v7 = objc_msgSend(v6, "countByEnumeratingWithState:objects:count:", &v76, v83, 16);
  if (v7)
  {
    v8 = v7;
    v9 = *(_QWORD *)v77;
    do
    {
      for (i = 0; i != v8; ++i)
      {
        if (*(_QWORD *)v77 != v9)
          objc_enumerationMutation(v6);
        objc_msgSend(v4, "addObject:", *(_QWORD *)(*((_QWORD *)&v76 + 1) + 8 * i));
      }
      v8 = objc_msgSend(v6, "countByEnumeratingWithState:objects:count:", &v76, v83, 16);
    }
    while (v8);
  }

  v74 = 0u;
  v75 = 0u;
  v72 = 0u;
  v73 = 0u;
  -[SCWDatabaseJSONStore zoneStoresByName](self, "zoneStoresByName");
  v11 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v11, "allValues");
  v12 = (void *)objc_claimAutoreleasedReturnValue();

  obj = v12;
  v58 = v4;
  v60 = objc_msgSend(v12, "countByEnumeratingWithState:objects:count:", &v72, v82, 16);
  if (v60)
  {
    v57 = *(_QWORD *)v73;
    do
    {
      for (j = 0; j != v60; ++j)
      {
        if (*(_QWORD *)v73 != v57)
          objc_enumerationMutation(obj);
        v14 = *(void **)(*((_QWORD *)&v72 + 1) + 8 * j);
        objc_msgSend(MEMORY[0x1E0C99DE8], "array");
        v15 = (void *)objc_claimAutoreleasedReturnValue();
        v68 = 0u;
        v69 = 0u;
        v70 = 0u;
        v71 = 0u;
        objc_msgSend(v14, "serverRecords");
        v16 = (void *)objc_claimAutoreleasedReturnValue();
        v17 = objc_msgSend(v16, "countByEnumeratingWithState:objects:count:", &v68, v81, 16);
        if (v17)
        {
          v18 = v17;
          v19 = *(_QWORD *)v69;
          do
          {
            for (k = 0; k != v18; ++k)
            {
              if (*(_QWORD *)v69 != v19)
                objc_enumerationMutation(v16);
              -[SCWDatabaseJSONStore _encodeCodableObject:](self, "_encodeCodableObject:", *(_QWORD *)(*((_QWORD *)&v68 + 1) + 8 * k));
              v21 = (void *)objc_claimAutoreleasedReturnValue();
              objc_msgSend(v15, "addObject:", v21);

            }
            v18 = objc_msgSend(v16, "countByEnumeratingWithState:objects:count:", &v68, v81, 16);
          }
          while (v18);
        }

        objc_msgSend(MEMORY[0x1E0C99DE8], "array");
        v22 = (void *)objc_claimAutoreleasedReturnValue();
        v64 = 0u;
        v65 = 0u;
        v66 = 0u;
        v67 = 0u;
        objc_msgSend(v14, "pendingCommands");
        v23 = (void *)objc_claimAutoreleasedReturnValue();
        v24 = objc_msgSend(v23, "countByEnumeratingWithState:objects:count:", &v64, v80, 16);
        if (v24)
        {
          v25 = v24;
          v26 = *(_QWORD *)v65;
          do
          {
            for (m = 0; m != v25; ++m)
            {
              if (*(_QWORD *)v65 != v26)
                objc_enumerationMutation(v23);
              -[SCWDatabaseJSONStore _encodeCodableObject:](self, "_encodeCodableObject:", *(_QWORD *)(*((_QWORD *)&v64 + 1) + 8 * m));
              v28 = (void *)objc_claimAutoreleasedReturnValue();
              objc_msgSend(v22, "addObject:", v28);

            }
            v25 = objc_msgSend(v23, "countByEnumeratingWithState:objects:count:", &v64, v80, 16);
          }
          while (v25);
        }

        objc_msgSend(MEMORY[0x1E0C99E08], "dictionary");
        v29 = (void *)objc_claimAutoreleasedReturnValue();
        objc_msgSend(v14, "zoneSchema");
        v30 = (void *)objc_claimAutoreleasedReturnValue();
        objc_msgSend(v30, "zoneName");
        v31 = (void *)objc_claimAutoreleasedReturnValue();
        objc_msgSend(v29, "setObject:forKeyedSubscript:", v31, CFSTR("name"));

        objc_msgSend(v29, "setObject:forKeyedSubscript:", v15, CFSTR("serverRecords"));
        objc_msgSend(v29, "setObject:forKeyedSubscript:", v22, CFSTR("pendingCommands"));
        objc_msgSend(v14, "lastSyncDate");
        v32 = (void *)objc_claimAutoreleasedReturnValue();
        objc_msgSend(v59, "stringFromDate:", v32);
        v33 = (void *)objc_claimAutoreleasedReturnValue();
        objc_msgSend(v29, "setObject:forKeyedSubscript:", v33, CFSTR("lastSyncDate"));

        objc_msgSend(v14, "lastDirtyDate");
        v34 = (void *)objc_claimAutoreleasedReturnValue();
        objc_msgSend(v59, "stringFromDate:", v34);
        v35 = (void *)objc_claimAutoreleasedReturnValue();
        objc_msgSend(v29, "setObject:forKeyedSubscript:", v35, CFSTR("lastDirtyDate"));

        objc_msgSend(v14, "serverChangeToken");
        v36 = (void *)objc_claimAutoreleasedReturnValue();
        -[SCWDatabaseJSONStore _encodeCodableObject:](self, "_encodeCodableObject:", v36);
        v37 = (void *)objc_claimAutoreleasedReturnValue();
        objc_msgSend(v29, "setObject:forKeyedSubscript:", v37, CFSTR("serverChangeToken"));

        v38 = (void *)MEMORY[0x1E0CB37E8];
        objc_msgSend(v14, "zoneSchema");
        v39 = (void *)objc_claimAutoreleasedReturnValue();
        objc_msgSend(v38, "numberWithUnsignedInteger:", objc_msgSend(v39, "version"));
        v40 = (void *)objc_claimAutoreleasedReturnValue();
        objc_msgSend(v29, "setObject:forKeyedSubscript:", v40, CFSTR("schemaVersion"));

        v4 = v58;
        objc_msgSend(v58, "addObject:", v29);

      }
      v60 = objc_msgSend(obj, "countByEnumeratingWithState:objects:count:", &v72, v82, 16);
    }
    while (v60);
  }

  objc_msgSend(MEMORY[0x1E0C99E08], "dictionary");
  v41 = (void *)objc_claimAutoreleasedReturnValue();
  -[SCWDatabaseJSONStore lastSyncDate](self, "lastSyncDate");
  v42 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v59, "stringFromDate:", v42);
  v43 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v41, "setObject:forKeyedSubscript:", v43, CFSTR("lastSyncDate"));

  -[SCWDatabaseJSONStore lastDirtyDate](self, "lastDirtyDate");
  v44 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v59, "stringFromDate:", v44);
  v45 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v41, "setObject:forKeyedSubscript:", v45, CFSTR("lastDirtyDate"));

  -[SCWDatabaseJSONStore serverChangeToken](self, "serverChangeToken");
  v46 = (void *)objc_claimAutoreleasedReturnValue();
  -[SCWDatabaseJSONStore _encodeCodableObject:](self, "_encodeCodableObject:", v46);
  v47 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v41, "setObject:forKeyedSubscript:", v47, CFSTR("serverChangeToken"));

  objc_msgSend(v41, "setObject:forKeyedSubscript:", v4, CFSTR("zones"));
  objc_msgSend(MEMORY[0x1E0C99E08], "dictionary");
  v48 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v48, "setObject:forKeyedSubscript:", v41, CFSTR("database"));
  v63 = 0;
  objc_msgSend(MEMORY[0x1E0CB36D8], "dataWithJSONObject:options:error:", v48, 0, &v63);
  v49 = (void *)objc_claimAutoreleasedReturnValue();
  v50 = v63;
  if (v49)
  {
    v51 = (void *)objc_msgSend(objc_alloc(MEMORY[0x1E0CB3940]), "initWithData:encoding:", v49, 4);
    v62 = v50;
    v52 = objc_msgSend(v51, "writeToURL:atomically:encoding:error:", v55, 1, 4, &v62);
    v53 = v62;

    if (v52)
    {
      value = arc4random_uniform(0xFFFFFFFE) + 1;
      setxattr((const char *)objc_msgSend(objc_retainAutorelease(v55), "fileSystemRepresentation"), "com.apple.stocks.jsonStoreChangeTag", &value, 4uLL, 0, 0);
      -[SCWDatabaseJSONStore setLastKnownStoreChangeTag:](self, "setLastKnownStoreChangeTag:", value);
      v54 = v58;
    }
    else
    {
      v54 = v58;
      if (os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_ERROR))
        -[SCWDatabaseJSONStore _saveToFileURL:].cold.2();
    }

  }
  else
  {
    if (os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_ERROR))
      -[SCWDatabaseJSONStore _saveToFileURL:].cold.1();
    v53 = v50;
    v54 = v58;
  }
  -[SCWDatabaseJSONStore _applyCloudBackupEnabledPropertyToStoreURL](self, "_applyCloudBackupEnabledPropertyToStoreURL");
  ++self->_diskWriteCount;

}

- (void)_saveIfNeededToFileURL:(id)a3
{
  void *v4;
  void *v5;
  id v6;
  _QWORD v7[5];
  id v8;

  -[SCWDatabaseJSONStore fileCoordinator](self, "fileCoordinator", a3);
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  -[SCWDatabaseJSONStore storeURL](self, "storeURL");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  v7[4] = self;
  v8 = 0;
  v7[0] = MEMORY[0x1E0C809B0];
  v7[1] = 3221225472;
  v7[2] = __47__SCWDatabaseJSONStore__saveIfNeededToFileURL___block_invoke;
  v7[3] = &unk_1E5AF1CB8;
  objc_msgSend(v4, "coordinateWritingItemAtURL:options:error:byAccessor:", v5, 4, &v8, v7);
  v6 = v8;

}

void __47__SCWDatabaseJSONStore__saveIfNeededToFileURL___block_invoke(uint64_t a1, void *a2)
{
  id v3;
  NSObject *v4;
  id v5;
  _QWORD block[5];
  id v7;

  v3 = a2;
  objc_msgSend(*(id *)(a1 + 32), "accessQueue");
  v4 = objc_claimAutoreleasedReturnValue();
  block[0] = MEMORY[0x1E0C809B0];
  block[1] = 3221225472;
  block[2] = __47__SCWDatabaseJSONStore__saveIfNeededToFileURL___block_invoke_2;
  block[3] = &unk_1E5AF1C90;
  block[4] = *(_QWORD *)(a1 + 32);
  v7 = v3;
  v5 = v3;
  dispatch_sync(v4, block);

}

uint64_t __47__SCWDatabaseJSONStore__saveIfNeededToFileURL___block_invoke_2(uint64_t a1)
{
  uint64_t result;

  result = objc_msgSend(*(id *)(a1 + 32), "havePendingChanges");
  if ((_DWORD)result)
  {
    objc_msgSend(*(id *)(a1 + 32), "_saveToFileURL:", *(_QWORD *)(a1 + 40));
    objc_msgSend(*(id *)(a1 + 32), "setHavePendingChanges:", 0);
    return objc_msgSend(MEMORY[0x1E0CB3600], "removeFilePresenter:", *(_QWORD *)(a1 + 32));
  }
  return result;
}

void __51__SCWDatabaseJSONStore__listenForChangesToFileURL___block_invoke(uint64_t a1)
{
  NSObject *v2;
  id v3;
  uint64_t v4;
  uint64_t v5;
  void *v6;
  uint64_t v7;
  uint64_t v8;
  uint64_t v9;
  __int128 v10;
  __int128 v11;
  __int128 v12;
  __int128 v13;
  _QWORD block[4];
  id v15;
  uint64_t v16;
  uint64_t *v17;
  uint64_t v18;
  uint64_t *v19;
  uint64_t v20;
  char v21;
  uint8_t v22[128];
  uint8_t buf[4];
  uint64_t v24;
  uint64_t v25;

  v25 = *MEMORY[0x1E0C80C00];
  v18 = 0;
  v19 = &v18;
  v20 = 0x2020000000;
  v21 = 0;
  objc_msgSend(*(id *)(a1 + 32), "accessQueue");
  v2 = objc_claimAutoreleasedReturnValue();
  block[0] = MEMORY[0x1E0C809B0];
  block[1] = 3221225472;
  block[2] = __51__SCWDatabaseJSONStore__listenForChangesToFileURL___block_invoke_2;
  block[3] = &unk_1E5AF1CE0;
  v3 = *(id *)(a1 + 40);
  v4 = *(_QWORD *)(a1 + 32);
  v15 = v3;
  v16 = v4;
  v17 = &v18;
  dispatch_sync(v2, block);

  if (*((_BYTE *)v19 + 24))
  {
    if (os_log_type_enabled(MEMORY[0x1E0C81028], OS_LOG_TYPE_DEFAULT))
    {
      v5 = *(_QWORD *)(a1 + 32);
      *(_DWORD *)buf = 134217984;
      v24 = v5;
      _os_log_impl(&dword_1ABCEF000, MEMORY[0x1E0C81028], OS_LOG_TYPE_DEFAULT, "%p JSON store will notify of external changes from another writer", buf, 0xCu);
    }
    v12 = 0u;
    v13 = 0u;
    v10 = 0u;
    v11 = 0u;
    objc_msgSend(*(id *)(a1 + 32), "observers", 0);
    v6 = (void *)objc_claimAutoreleasedReturnValue();
    v7 = objc_msgSend(v6, "countByEnumeratingWithState:objects:count:", &v10, v22, 16);
    if (v7)
    {
      v8 = *(_QWORD *)v11;
      do
      {
        v9 = 0;
        do
        {
          if (*(_QWORD *)v11 != v8)
            objc_enumerationMutation(v6);
          objc_msgSend(*(id *)(*((_QWORD *)&v10 + 1) + 8 * v9++), "storeCoordinatorDiscoveredExternalChanges:", *(_QWORD *)(a1 + 32));
        }
        while (v7 != v9);
        v7 = objc_msgSend(v6, "countByEnumeratingWithState:objects:count:", &v10, v22, 16);
      }
      while (v7);
    }

  }
  _Block_object_dispose(&v18, 8);
}

uint64_t __51__SCWDatabaseJSONStore__listenForChangesToFileURL___block_invoke_2(uint64_t a1)
{
  int v2;
  uint64_t result;
  int value;

  value = 0;
  getxattr((const char *)objc_msgSend(objc_retainAutorelease(*(id *)(a1 + 32)), "fileSystemRepresentation"), "com.apple.stocks.jsonStoreChangeTag", &value, 4uLL, 0, 0);
  v2 = value;
  result = objc_msgSend(*(id *)(a1 + 40), "lastKnownStoreChangeTag");
  *(_BYTE *)(*(_QWORD *)(*(_QWORD *)(a1 + 48) + 8) + 24) = v2 != (_DWORD)result;
  return result;
}

uint64_t __51__SCWDatabaseJSONStore__listenForChangesToFileURL___block_invoke_47(uint64_t a1)
{
  return close(*(_DWORD *)(a1 + 32));
}

- (id)_encodeCodableObject:(id)a3
{
  __CFString *v3;
  void *v4;
  uint64_t v5;
  void *v6;
  __CFString *v7;
  uint64_t v9;

  v3 = &stru_1E5AFDA90;
  if (a3)
  {
    v9 = 0;
    objc_msgSend(MEMORY[0x1E0CB36F8], "archivedDataWithRootObject:requiringSecureCoding:error:", a3, 1, &v9);
    v4 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v4, "base64EncodedStringWithOptions:", 0);
    v5 = objc_claimAutoreleasedReturnValue();
    v6 = (void *)v5;
    if (v5)
      v7 = (__CFString *)v5;
    else
      v7 = &stru_1E5AFDA90;
    v3 = v7;

  }
  return v3;
}

- (NSDate)lastSyncDate
{
  return self->_lastSyncDate;
}

- (void)setLastSyncDate:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 16);
}

- (NSDate)lastDirtyDate
{
  return self->_lastDirtyDate;
}

- (void)setLastDirtyDate:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 24);
}

- (CKServerChangeToken)serverChangeToken
{
  return self->_serverChangeToken;
}

- (void)setServerChangeToken:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 32);
}

- (NSOperationQueue)presentedItemOperationQueue
{
  return (NSOperationQueue *)objc_getProperty(self, a2, 48, 1);
}

- (void)setFileCoordinator:(id)a3
{
  objc_storeStrong((id *)&self->_fileCoordinator, a3);
}

- (void)setAccessQueue:(id)a3
{
  objc_storeStrong((id *)&self->_accessQueue, a3);
}

- (void)setDatabaseSchema:(id)a3
{
  objc_storeStrong((id *)&self->_databaseSchema, a3);
}

- (void)setObservers:(id)a3
{
  objc_storeStrong((id *)&self->_observers, a3);
}

- (void)setZoneStoresByName:(id)a3
{
  objc_storeStrong((id *)&self->_zoneStoresByName, a3);
}

- (NSMutableDictionary)foreignZoneDictsByName
{
  return self->_foreignZoneDictsByName;
}

- (void)setForeignZoneDictsByName:(id)a3
{
  objc_storeStrong((id *)&self->_foreignZoneDictsByName, a3);
}

- (BOOL)havePendingChanges
{
  return self->_havePendingChanges;
}

- (void)setHavePendingChanges:(BOOL)a3
{
  self->_havePendingChanges = a3;
}

- (void)setWorkQueue:(id)a3
{
  objc_storeStrong((id *)&self->_workQueue, a3);
}

- (SCWOperationThrottler)saveThrottler
{
  return self->_saveThrottler;
}

- (void)setSaveThrottler:(id)a3
{
  objc_storeStrong((id *)&self->_saveThrottler, a3);
}

- (void)setAllowedCommandClasses:(id)a3
{
  objc_storeStrong((id *)&self->_allowedCommandClasses, a3);
}

- (unint64_t)diskReadCount
{
  return self->_diskReadCount;
}

- (unint64_t)diskWriteCount
{
  return self->_diskWriteCount;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_storeURL, 0);
  objc_storeStrong((id *)&self->_allowedCommandClasses, 0);
  objc_storeStrong((id *)&self->_changeListenerSource, 0);
  objc_storeStrong((id *)&self->_saveThrottler, 0);
  objc_storeStrong((id *)&self->_workQueue, 0);
  objc_storeStrong((id *)&self->_foreignZoneDictsByName, 0);
  objc_storeStrong((id *)&self->_zoneStoresByName, 0);
  objc_storeStrong((id *)&self->_observers, 0);
  objc_storeStrong((id *)&self->_databaseSchema, 0);
  objc_storeStrong((id *)&self->_accessQueue, 0);
  objc_storeStrong((id *)&self->_fileCoordinator, 0);
  objc_storeStrong((id *)&self->_presentedItemOperationQueue, 0);
  objc_storeStrong((id *)&self->_presentedItemURL, 0);
  objc_storeStrong((id *)&self->_serverChangeToken, 0);
  objc_storeStrong((id *)&self->_lastDirtyDate, 0);
  objc_storeStrong((id *)&self->_lastSyncDate, 0);
}

- (void)_saveToFileURL:.cold.1()
{
  uint64_t v0;
  uint64_t v1;
  uint64_t v2;
  uint64_t v3;
  uint64_t v4;
  uint8_t v5;

  OUTLINED_FUNCTION_0();
  OUTLINED_FUNCTION_1(&dword_1ABCEF000, MEMORY[0x1E0C81028], v0, "%p failed to generate database JSON with error: %{public}@", v1, v2, v3, v4, v5);
  OUTLINED_FUNCTION_2();
}

- (void)_saveToFileURL:.cold.2()
{
  uint64_t v0;
  uint64_t v1;
  uint64_t v2;
  uint64_t v3;
  uint64_t v4;
  uint8_t v5;

  OUTLINED_FUNCTION_0();
  OUTLINED_FUNCTION_1(&dword_1ABCEF000, MEMORY[0x1E0C81028], v0, "%p failed to write database JSON to disk with error: %{public}@", v1, v2, v3, v4, v5);
  OUTLINED_FUNCTION_2();
}

- (void)_loadFromFileURL:.cold.1()
{
  uint64_t v0;
  uint64_t v1;
  uint64_t v2;
  uint64_t v3;
  uint64_t v4;
  uint8_t v5;

  OUTLINED_FUNCTION_0();
  OUTLINED_FUNCTION_1(&dword_1ABCEF000, MEMORY[0x1E0C81028], v0, "%p failed to decode database JSON with error: %{public}@", v1, v2, v3, v4, v5);
  OUTLINED_FUNCTION_2();
}

@end
