...
- (void)dealloc
{
  ...
  {
    MEMORY[0x219A02728]();
    NSZoneFree((NSZone *)-[TSURegularExpression zone](self, "zone"), self->_reserved);
  }
  v3.receiver = self;
  v3.super_class = (Class)TSURegularExpression;
  -[TSURegularExpression dealloc](&v3, sel_dealloc);
}

- (TSURegularExpression)initWithExpressionString:(id)a3
{
  return -[TSURegularExpression initWithExpressionString:options:](self, "initWithExpressionString:options:", a3, 0);
}
...
- (TSURegularExpression)initWithExpressionString:(id)a3 options:(unsigned int)a4
{
  ...
  v6 = -[TSURegularExpression init](&v14, sel_init);
  if (v6)
  {
    ...
    *((_QWORD *)v6 + 2) = v10;
    ...
    if ((_DWORD)v12)
    {
      NSLog((NSString *)CFSTR("*** %s: error %d"), "-[TSURegularExpression initWithExpressionString:options:]", v12);
...
{
  return -[TSURegularExpression initWithExpressionString:](self, "initWithExpressionString:", &stru_24D61C228);
}

- (id)copyWithZone:(_NSZone *)a3
{
  if ((_NSZone *)-[TSURegularExpression zone](self, "zone") == a3)
    return self;
  else
...
- (_NSRange)matchedRangeForCString:(const char *)a3 range:(_NSRange)a4 subexpressionRanges:(_NSRange *)a5 count:(unsigned int)a6
{
  ...
  v11 = a6;
  v12 = a6;
  if (-[TSURegularExpression numberOfSubexpressions](self, "numberOfSubexpressions") < a6)
    v12 = -[TSURegularExpression numberOfSubexpressions](self, "numberOfSubexpressions");
  v13 = v12 + 1;
  if (v12 + 1 >= 6)
...
- (_NSRange)matchedRangeForString:(id)a3 range:(_NSRange)a4 subexpressionRanges:(_NSRange *)a5 count:(unsigned int)a6
{
  ...
  v12 = -[TSURegularExpression matchedRangeForCString:range:subexpressionRanges:count:](self, "matchedRangeForCString:range:subexpressionRanges:count:", -[__CFData bytes](ASCIIData, "bytes"), location, length, a5, v6);
  v14 = v13;
...
- (TSURegularExpression)initWithCoder:(id)a3
{
  ...
  v6 = objc_msgSend(a3, "versionForClassName:", CFSTR("TSURegularExpression"));
  v7 = v6;
  if (v6 < 2)
  {
    if (v6 == 1)
      return -[TSURegularExpression initWithExpressionString:](self, "initWithExpressionString:", objc_msgSend(a3, "decodeObject"));
  }
  else
  {
    v8 = MOFullMethodName((uint64_t)self, a2);
    NSLog((NSString *)CFSTR("%@: class version %u cannot read instances archived with version %lu"), v8, 1, v7);
    -[TSURegularExpression dealloc](self, "dealloc");
    return 0;
  }
  return self;
}
