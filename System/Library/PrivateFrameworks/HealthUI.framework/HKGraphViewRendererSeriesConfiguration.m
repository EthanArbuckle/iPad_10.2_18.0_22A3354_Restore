@implementation HKGraphViewRendererSeriesConfiguration

- (HKGraphViewRendererSeriesConfiguration)initWithRendererSeriesRows:(id)a3
{
  id v5;
  HKGraphViewRendererSeriesConfiguration *v6;
  HKGraphViewRendererSeriesConfiguration *v7;
  objc_super v9;

  v5 = a3;
  v9.receiver = self;
  v9.super_class = (Class)HKGraphViewRendererSeriesConfiguration;
  v6 = -[HKGraphViewRendererSeriesConfiguration init](&v9, sel_init);
  v7 = v6;
  if (v6)
    objc_storeStrong((id *)&v6->_seriesRows, a3);

  return v7;
}

- (NSArray)seriesRows
{
  return self->_seriesRows;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_seriesRows, 0);
}

@end
