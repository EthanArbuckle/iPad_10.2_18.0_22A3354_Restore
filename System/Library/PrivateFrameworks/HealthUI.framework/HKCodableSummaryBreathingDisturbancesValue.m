@implementation HKCodableSummaryBreathingDisturbancesValue

- (BOOL)hasQuantity
{
  return self->_quantity != 0;
}

- (void)setClassificationRawValue:(int64_t)a3
{
  *(_BYTE *)&self->_has |= 1u;
  self->_classificationRawValue = a3;
}

- (void)setHasClassificationRawValue:(BOOL)a3
{
  *(_BYTE *)&self->_has = *(_BYTE *)&self->_has & 0xFE | a3;
}

- (BOOL)hasClassificationRawValue
{
  return *(_BYTE *)&self->_has & 1;
}

- (BOOL)hasLevelViewDataConfiguration
{
  return self->_levelViewDataConfiguration != 0;
}

- (id)description
{
  void *v3;
  void *v4;
  void *v5;
  void *v6;
  objc_super v8;

  v3 = (void *)MEMORY[0x1E0CB3940];
  v8.receiver = self;
  v8.super_class = (Class)HKCodableSummaryBreathingDisturbancesValue;
  -[HKCodableSummaryBreathingDisturbancesValue description](&v8, sel_description);
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  -[HKCodableSummaryBreathingDisturbancesValue dictionaryRepresentation](self, "dictionaryRepresentation");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v3, "stringWithFormat:", CFSTR("%@ %@"), v4, v5);
  v6 = (void *)objc_claimAutoreleasedReturnValue();

  return v6;
}

- (id)dictionaryRepresentation
{
  void *v3;
  HKCodableSummaryQuantityValue *quantity;
  void *v5;
  void *v6;
  HKCodableLevelViewDataConfiguration *levelViewDataConfiguration;
  void *v8;

  objc_msgSend(MEMORY[0x1E0C99E08], "dictionary");
  v3 = (void *)objc_claimAutoreleasedReturnValue();
  quantity = self->_quantity;
  if (quantity)
  {
    -[HKCodableSummaryQuantityValue dictionaryRepresentation](quantity, "dictionaryRepresentation");
    v5 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v3, "setObject:forKey:", v5, CFSTR("quantity"));

  }
  if ((*(_BYTE *)&self->_has & 1) != 0)
  {
    objc_msgSend(MEMORY[0x1E0CB37E8], "numberWithLongLong:", self->_classificationRawValue);
    v6 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v3, "setObject:forKey:", v6, CFSTR("classificationRawValue"));

  }
  levelViewDataConfiguration = self->_levelViewDataConfiguration;
  if (levelViewDataConfiguration)
  {
    -[HKCodableLevelViewDataConfiguration dictionaryRepresentation](levelViewDataConfiguration, "dictionaryRepresentation");
    v8 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v3, "setObject:forKey:", v8, CFSTR("levelViewDataConfiguration"));

  }
  return v3;
}

- (BOOL)readFrom:(id)a3
{
  return HKCodableSummaryBreathingDisturbancesValueReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  id v4;
  id v5;

  v4 = a3;
  v5 = v4;
  if (self->_quantity)
  {
    PBDataWriterWriteSubmessage();
    v4 = v5;
  }
  if ((*(_BYTE *)&self->_has & 1) != 0)
  {
    PBDataWriterWriteInt64Field();
    v4 = v5;
  }
  if (self->_levelViewDataConfiguration)
  {
    PBDataWriterWriteSubmessage();
    v4 = v5;
  }

}

- (void)copyTo:(id)a3
{
  id v4;
  id v5;

  v4 = a3;
  v5 = v4;
  if (self->_quantity)
  {
    objc_msgSend(v4, "setQuantity:");
    v4 = v5;
  }
  if ((*(_BYTE *)&self->_has & 1) != 0)
  {
    *((_QWORD *)v4 + 1) = self->_classificationRawValue;
    *((_BYTE *)v4 + 32) |= 1u;
  }
  if (self->_levelViewDataConfiguration)
  {
    objc_msgSend(v5, "setLevelViewDataConfiguration:");
    v4 = v5;
  }

}

- (id)copyWithZone:(_NSZone *)a3
{
  uint64_t v5;
  id v6;
  void *v7;
  id v8;
  void *v9;

  v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  v6 = -[HKCodableSummaryQuantityValue copyWithZone:](self->_quantity, "copyWithZone:", a3);
  v7 = *(void **)(v5 + 24);
  *(_QWORD *)(v5 + 24) = v6;

  if ((*(_BYTE *)&self->_has & 1) != 0)
  {
    *(_QWORD *)(v5 + 8) = self->_classificationRawValue;
    *(_BYTE *)(v5 + 32) |= 1u;
  }
  v8 = -[HKCodableLevelViewDataConfiguration copyWithZone:](self->_levelViewDataConfiguration, "copyWithZone:", a3);
  v9 = *(void **)(v5 + 16);
  *(_QWORD *)(v5 + 16) = v8;

  return (id)v5;
}

- (BOOL)isEqual:(id)a3
{
  id v4;
  HKCodableSummaryQuantityValue *quantity;
  HKCodableLevelViewDataConfiguration *levelViewDataConfiguration;
  char v7;

  v4 = a3;
  if (!objc_msgSend(v4, "isMemberOfClass:", objc_opt_class()))
    goto LABEL_11;
  quantity = self->_quantity;
  if ((unint64_t)quantity | *((_QWORD *)v4 + 3))
  {
    if (!-[HKCodableSummaryQuantityValue isEqual:](quantity, "isEqual:"))
      goto LABEL_11;
  }
  if ((*(_BYTE *)&self->_has & 1) != 0)
  {
    if ((*((_BYTE *)v4 + 32) & 1) == 0 || self->_classificationRawValue != *((_QWORD *)v4 + 1))
      goto LABEL_11;
  }
  else if ((*((_BYTE *)v4 + 32) & 1) != 0)
  {
LABEL_11:
    v7 = 0;
    goto LABEL_12;
  }
  levelViewDataConfiguration = self->_levelViewDataConfiguration;
  if ((unint64_t)levelViewDataConfiguration | *((_QWORD *)v4 + 2))
    v7 = -[HKCodableLevelViewDataConfiguration isEqual:](levelViewDataConfiguration, "isEqual:");
  else
    v7 = 1;
LABEL_12:

  return v7;
}

- (unint64_t)hash
{
  unint64_t v3;
  uint64_t v4;

  v3 = -[HKCodableSummaryQuantityValue hash](self->_quantity, "hash");
  if ((*(_BYTE *)&self->_has & 1) != 0)
    v4 = 2654435761 * self->_classificationRawValue;
  else
    v4 = 0;
  return v4 ^ v3 ^ -[HKCodableLevelViewDataConfiguration hash](self->_levelViewDataConfiguration, "hash");
}

- (void)mergeFrom:(id)a3
{
  _QWORD *v4;
  HKCodableSummaryQuantityValue *quantity;
  uint64_t v6;
  HKCodableLevelViewDataConfiguration *levelViewDataConfiguration;
  uint64_t v8;
  _QWORD *v9;

  v4 = a3;
  quantity = self->_quantity;
  v6 = v4[3];
  v9 = v4;
  if (quantity)
  {
    if (!v6)
      goto LABEL_7;
    -[HKCodableSummaryQuantityValue mergeFrom:](quantity, "mergeFrom:");
  }
  else
  {
    if (!v6)
      goto LABEL_7;
    -[HKCodableSummaryBreathingDisturbancesValue setQuantity:](self, "setQuantity:");
  }
  v4 = v9;
LABEL_7:
  if ((v4[4] & 1) != 0)
  {
    self->_classificationRawValue = v4[1];
    *(_BYTE *)&self->_has |= 1u;
  }
  levelViewDataConfiguration = self->_levelViewDataConfiguration;
  v8 = v4[2];
  if (levelViewDataConfiguration)
  {
    if (v8)
    {
      -[HKCodableLevelViewDataConfiguration mergeFrom:](levelViewDataConfiguration, "mergeFrom:");
LABEL_14:
      v4 = v9;
    }
  }
  else if (v8)
  {
    -[HKCodableSummaryBreathingDisturbancesValue setLevelViewDataConfiguration:](self, "setLevelViewDataConfiguration:");
    goto LABEL_14;
  }

}

- (HKCodableSummaryQuantityValue)quantity
{
  return self->_quantity;
}

- (void)setQuantity:(id)a3
{
  objc_storeStrong((id *)&self->_quantity, a3);
}

- (int64_t)classificationRawValue
{
  return self->_classificationRawValue;
}

- (HKCodableLevelViewDataConfiguration)levelViewDataConfiguration
{
  return self->_levelViewDataConfiguration;
}

- (void)setLevelViewDataConfiguration:(id)a3
{
  objc_storeStrong((id *)&self->_levelViewDataConfiguration, a3);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_quantity, 0);
  objc_storeStrong((id *)&self->_levelViewDataConfiguration, 0);
}

@end
