@implementation ICASSmartFolderCreationData

- (ICASSmartFolderCreationData)initWithFolderCreationApproach:(id)a3 countOfTags:(id)a4 filterCondition:(id)a5 enabledFiltersArray:(id)a6
{
  id v11;
  id v12;
  id v13;
  id v14;
  ICASSmartFolderCreationData *v15;
  ICASSmartFolderCreationData *v16;
  objc_super v18;

  v11 = a3;
  v12 = a4;
  v13 = a5;
  v14 = a6;
  v18.receiver = self;
  v18.super_class = (Class)ICASSmartFolderCreationData;
  v15 = -[ICASSmartFolderCreationData init](&v18, sel_init);
  v16 = v15;
  if (v15)
  {
    objc_storeStrong((id *)&v15->_folderCreationApproach, a3);
    objc_storeStrong((id *)&v16->_countOfTags, a4);
    objc_storeStrong((id *)&v16->_filterCondition, a5);
    objc_storeStrong((id *)&v16->_enabledFiltersArray, a6);
  }

  return v16;
}

+ (NSString)dataName
{
  return (NSString *)CFSTR("SmartFolderCreationData");
}

- (id)toDict
{
  void *v3;
  uint64_t v4;
  void *v5;
  void *v6;
  uint64_t v7;
  void *v8;
  void *v9;
  uint64_t v10;
  void *v11;
  void *v12;
  uint64_t v13;
  void *v14;
  void *v15;
  _QWORD v17[4];
  _QWORD v18[5];

  v18[4] = *MEMORY[0x1E0C80C00];
  v17[0] = CFSTR("folderCreationApproach");
  -[ICASSmartFolderCreationData folderCreationApproach](self, "folderCreationApproach");
  v3 = (void *)objc_claimAutoreleasedReturnValue();
  if (v3)
  {
    -[ICASSmartFolderCreationData folderCreationApproach](self, "folderCreationApproach");
    v4 = objc_claimAutoreleasedReturnValue();
  }
  else
  {
    v4 = objc_opt_new();
  }
  v5 = (void *)v4;
  v18[0] = v4;
  v17[1] = CFSTR("countOfTags");
  -[ICASSmartFolderCreationData countOfTags](self, "countOfTags");
  v6 = (void *)objc_claimAutoreleasedReturnValue();
  if (v6)
  {
    -[ICASSmartFolderCreationData countOfTags](self, "countOfTags");
    v7 = objc_claimAutoreleasedReturnValue();
  }
  else
  {
    v7 = objc_opt_new();
  }
  v8 = (void *)v7;
  v18[1] = v7;
  v17[2] = CFSTR("filterCondition");
  -[ICASSmartFolderCreationData filterCondition](self, "filterCondition");
  v9 = (void *)objc_claimAutoreleasedReturnValue();
  if (v9)
  {
    -[ICASSmartFolderCreationData filterCondition](self, "filterCondition");
    v10 = objc_claimAutoreleasedReturnValue();
  }
  else
  {
    v10 = objc_opt_new();
  }
  v11 = (void *)v10;
  v18[2] = v10;
  v17[3] = CFSTR("enabledFiltersArray");
  -[ICASSmartFolderCreationData enabledFiltersArray](self, "enabledFiltersArray");
  v12 = (void *)objc_claimAutoreleasedReturnValue();
  if (v12)
  {
    -[ICASSmartFolderCreationData enabledFiltersArray](self, "enabledFiltersArray");
    v13 = objc_claimAutoreleasedReturnValue();
  }
  else
  {
    v13 = objc_opt_new();
  }
  v14 = (void *)v13;
  v18[3] = v13;
  objc_msgSend(MEMORY[0x1E0C99D80], "dictionaryWithObjects:forKeys:count:", v18, v17, 4);
  v15 = (void *)objc_claimAutoreleasedReturnValue();

  return v15;
}

- (ICASFolderCreationApproach)folderCreationApproach
{
  return self->_folderCreationApproach;
}

- (NSNumber)countOfTags
{
  return self->_countOfTags;
}

- (ICASFilterCondition)filterCondition
{
  return self->_filterCondition;
}

- (NSArray)enabledFiltersArray
{
  return self->_enabledFiltersArray;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_enabledFiltersArray, 0);
  objc_storeStrong((id *)&self->_filterCondition, 0);
  objc_storeStrong((id *)&self->_countOfTags, 0);
  objc_storeStrong((id *)&self->_folderCreationApproach, 0);
}

@end
