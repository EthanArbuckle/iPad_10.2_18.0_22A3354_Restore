@implementation ICASFullErrorData

- (ICASFullErrorData)initWithFullErrorCode:(id)a3 errorString:(id)a4
{
  id v7;
  id v8;
  ICASFullErrorData *v9;
  ICASFullErrorData *v10;
  objc_super v12;

  v7 = a3;
  v8 = a4;
  v12.receiver = self;
  v12.super_class = (Class)ICASFullErrorData;
  v9 = -[ICASFullErrorData init](&v12, sel_init);
  v10 = v9;
  if (v9)
  {
    objc_storeStrong((id *)&v9->_fullErrorCode, a3);
    objc_storeStrong((id *)&v10->_errorString, a4);
  }

  return v10;
}

+ (NSString)dataName
{
  return (NSString *)CFSTR("FullErrorData");
}

- (id)toDict
{
  void *v3;
  uint64_t v4;
  void *v5;
  void *v6;
  uint64_t v7;
  void *v8;
  void *v9;
  _QWORD v11[2];
  _QWORD v12[3];

  v12[2] = *MEMORY[0x1E0C80C00];
  v11[0] = CFSTR("fullErrorCode");
  -[ICASFullErrorData fullErrorCode](self, "fullErrorCode");
  v3 = (void *)objc_claimAutoreleasedReturnValue();
  if (v3)
  {
    -[ICASFullErrorData fullErrorCode](self, "fullErrorCode");
    v4 = objc_claimAutoreleasedReturnValue();
  }
  else
  {
    v4 = objc_opt_new();
  }
  v5 = (void *)v4;
  v11[1] = CFSTR("errorString");
  v12[0] = v4;
  -[ICASFullErrorData errorString](self, "errorString");
  v6 = (void *)objc_claimAutoreleasedReturnValue();
  if (v6)
  {
    -[ICASFullErrorData errorString](self, "errorString");
    v7 = objc_claimAutoreleasedReturnValue();
  }
  else
  {
    v7 = objc_opt_new();
  }
  v8 = (void *)v7;
  v12[1] = v7;
  objc_msgSend(MEMORY[0x1E0C99D80], "dictionaryWithObjects:forKeys:count:", v12, v11, 2);
  v9 = (void *)objc_claimAutoreleasedReturnValue();

  return v9;
}

- (NSNumber)fullErrorCode
{
  return self->_fullErrorCode;
}

- (NSString)errorString
{
  return self->_errorString;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_errorString, 0);
  objc_storeStrong((id *)&self->_fullErrorCode, 0);
}

@end
