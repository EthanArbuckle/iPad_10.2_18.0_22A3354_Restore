@implementation PKPaymentSetupFieldBuiltInState

- (PKPaymentSetupFieldBuiltInState)initWithIdentifier:(id)a3 type:(unint64_t)a4
{
  PKPaymentSetupFieldBuiltInState *v4;
  void *v5;
  objc_super v7;

  v7.receiver = self;
  v7.super_class = (Class)PKPaymentSetupFieldBuiltInState;
  v4 = -[PKPaymentSetupField initWithIdentifier:type:](&v7, sel_initWithIdentifier_type_, CFSTR("state"), a4);
  if (v4)
  {
    PKLocalizedPaymentString(CFSTR("STATE"), 0);
    v5 = (void *)objc_claimAutoreleasedReturnValue();
    -[PKPaymentSetupField _setLocalizedDisplayName:](v4, "_setLocalizedDisplayName:", v5);

  }
  return v4;
}

- (BOOL)isBuiltIn
{
  return 1;
}

- (BOOL)supportsAddressAutofill
{
  return 1;
}

@end
