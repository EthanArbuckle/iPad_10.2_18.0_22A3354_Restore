@implementation PKSearchTransactionRewardsResult

+ (BOOL)supportsSecureCoding
{
  return 1;
}

- (unint64_t)tokenType
{
  return 8;
}

- (void)encodeWithCoder:(id)a3
{
  unint64_t rewardsValueUnit;
  id v5;

  rewardsValueUnit = self->_rewardsValueUnit;
  v5 = a3;
  objc_msgSend(v5, "encodeInteger:forKey:", rewardsValueUnit, CFSTR("rewardsValueUnit"));
  objc_msgSend(v5, "encodeObject:forKey:", self->_rewardsValue, CFSTR("rewardsValue"));

}

- (PKSearchTransactionRewardsResult)initWithCoder:(id)a3
{
  id v4;
  PKSearchTransactionRewardsResult *v5;
  uint64_t v6;
  NSDecimalNumber *rewardsValue;

  v4 = a3;
  v5 = -[PKSearchTransactionRewardsResult init](self, "init");
  if (v5)
  {
    v5->_rewardsValueUnit = objc_msgSend(v4, "decodeIntegerForKey:", CFSTR("rewardsValueUnit"));
    objc_msgSend(v4, "decodeObjectOfClass:forKey:", objc_opt_class(), CFSTR("rewardsValue"));
    v6 = objc_claimAutoreleasedReturnValue();
    rewardsValue = v5->_rewardsValue;
    v5->_rewardsValue = (NSDecimalNumber *)v6;

  }
  return v5;
}

- (NSString)description
{
  void *v3;
  void *v4;
  objc_super v6;

  v6.receiver = self;
  v6.super_class = (Class)PKSearchTransactionRewardsResult;
  -[PKSearchTransactionRewardsResult description](&v6, sel_description);
  v3 = (void *)objc_claimAutoreleasedReturnValue();
  v4 = (void *)objc_msgSend(v3, "mutableCopy");

  objc_msgSend(v4, "appendFormat:", CFSTR("rewardsValueUnit: '%lu'; "), self->_rewardsValueUnit);
  objc_msgSend(v4, "appendFormat:", CFSTR("rewardsValue: '%@'; "), self->_rewardsValue);
  objc_msgSend(v4, "appendFormat:", CFSTR(">"));
  return (NSString *)v4;
}

- (BOOL)isEqual:(id)a3
{
  _QWORD *v4;
  NSDecimalNumber *rewardsValue;
  NSDecimalNumber *v6;
  BOOL v7;

  v4 = a3;
  objc_opt_class();
  if ((objc_opt_isKindOfClass() & 1) == 0)
    goto LABEL_7;
  rewardsValue = self->_rewardsValue;
  v6 = (NSDecimalNumber *)v4[1];
  if (!rewardsValue || !v6)
  {
    if (rewardsValue == v6)
      goto LABEL_5;
LABEL_7:
    v7 = 0;
    goto LABEL_8;
  }
  if ((-[NSDecimalNumber isEqual:](rewardsValue, "isEqual:") & 1) == 0)
    goto LABEL_7;
LABEL_5:
  v7 = self->_rewardsValueUnit == v4[2];
LABEL_8:

  return v7;
}

- (unint64_t)hash
{
  id v3;
  uint64_t v4;
  unint64_t v5;

  v3 = objc_alloc_init(MEMORY[0x1E0C99DE8]);
  objc_msgSend(v3, "safelyAddObject:", self->_rewardsValue);
  v4 = PKCombinedHash(17, v3);
  v5 = self->_rewardsValueUnit - v4 + 32 * v4;

  return v5;
}

- (NSDecimalNumber)rewardsValue
{
  return self->_rewardsValue;
}

- (void)setRewardsValue:(id)a3
{
  objc_storeStrong((id *)&self->_rewardsValue, a3);
}

- (unint64_t)rewardsValueUnit
{
  return self->_rewardsValueUnit;
}

- (void)setRewardsValueUnit:(unint64_t)a3
{
  self->_rewardsValueUnit = a3;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_rewardsValue, 0);
}

@end
