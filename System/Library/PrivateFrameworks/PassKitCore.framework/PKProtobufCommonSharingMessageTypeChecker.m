@implementation PKProtobufCommonSharingMessageTypeChecker

- (BOOL)hasMessageType
{
  return self->_messageType != 0;
}

- (id)description
{
  void *v3;
  void *v4;
  void *v5;
  void *v6;
  objc_super v8;

  v3 = (void *)MEMORY[0x1E0CB3940];
  v8.receiver = self;
  v8.super_class = (Class)PKProtobufCommonSharingMessageTypeChecker;
  -[PKProtobufCommonSharingMessageTypeChecker description](&v8, sel_description);
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  -[PKProtobufCommonSharingMessageTypeChecker dictionaryRepresentation](self, "dictionaryRepresentation");
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v3, "stringWithFormat:", CFSTR("%@ %@"), v4, v5);
  v6 = (void *)objc_claimAutoreleasedReturnValue();

  return v6;
}

- (id)dictionaryRepresentation
{
  void *v3;
  void *v4;
  NSString *messageType;

  objc_msgSend(MEMORY[0x1E0C99E08], "dictionary");
  v3 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(MEMORY[0x1E0CB37E8], "numberWithUnsignedInt:", self->_version);
  v4 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v3, "setObject:forKey:", v4, CFSTR("version"));

  messageType = self->_messageType;
  if (messageType)
    objc_msgSend(v3, "setObject:forKey:", messageType, CFSTR("messageType"));
  return v3;
}

- (BOOL)readFrom:(id)a3
{
  return PKProtobufCommonSharingMessageTypeCheckerReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  id v4;

  v4 = a3;
  PBDataWriterWriteUint32Field();
  if (self->_messageType)
    PBDataWriterWriteStringField();

}

- (void)copyTo:(id)a3
{
  NSString *messageType;

  *((_DWORD *)a3 + 4) = self->_version;
  messageType = self->_messageType;
  if (messageType)
    objc_msgSend(a3, "setMessageType:", messageType);
}

- (id)copyWithZone:(_NSZone *)a3
{
  uint64_t v5;
  uint64_t v6;
  void *v7;

  v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  *(_DWORD *)(v5 + 16) = self->_version;
  v6 = -[NSString copyWithZone:](self->_messageType, "copyWithZone:", a3);
  v7 = *(void **)(v5 + 8);
  *(_QWORD *)(v5 + 8) = v6;

  return (id)v5;
}

- (BOOL)isEqual:(id)a3
{
  _QWORD *v4;
  NSString *messageType;
  char v6;

  v4 = a3;
  if (objc_msgSend(v4, "isMemberOfClass:", objc_opt_class()) && self->_version == *((_DWORD *)v4 + 4))
  {
    messageType = self->_messageType;
    if ((unint64_t)messageType | v4[1])
      v6 = -[NSString isEqual:](messageType, "isEqual:");
    else
      v6 = 1;
  }
  else
  {
    v6 = 0;
  }

  return v6;
}

- (unint64_t)hash
{
  uint64_t v2;

  v2 = 2654435761 * self->_version;
  return -[NSString hash](self->_messageType, "hash") ^ v2;
}

- (void)mergeFrom:(id)a3
{
  self->_version = *((_DWORD *)a3 + 4);
  if (*((_QWORD *)a3 + 1))
    -[PKProtobufCommonSharingMessageTypeChecker setMessageType:](self, "setMessageType:");
}

- (unsigned)version
{
  return self->_version;
}

- (void)setVersion:(unsigned int)a3
{
  self->_version = a3;
}

- (NSString)messageType
{
  return self->_messageType;
}

- (void)setMessageType:(id)a3
{
  objc_storeStrong((id *)&self->_messageType, a3);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_messageType, 0);
}

@end
