...
- (uint64_t)itk_randomize
{
  ...
  if (result >= 2)
  {
    ...
    do
    {
      ...
      result = objc_msgSend(a1, "itk_swapItemAtIndex:withItemAtIndex:", v5, arc4random_uniform(v3));
      --v4;
    }
    while (v4);
  }
...
- (void)itk_insertNonNilObject:()ITK atIndex:
{
  ...
  {
    +[ITKAssert handleFailedAssertWithCondition:functionName:simulateCrash:showAlert:format:](ITKAssert, "handleFailedAssertWithCondition:functionName:simulateCrash:showAlert:format:", "__objc_no", "-[NSMutableArray(ITK) itk_insertNonNilObject:atIndex:]", 0, 0, CFSTR("Trying to insert object in mutableArray at negative index %ld"), a4, v7);
    goto LABEL_8;
  }
  if (objc_msgSend(a1, "count") < a4)
  {
    +[ITKAssert handleFailedAssertWithCondition:functionName:simulateCrash:showAlert:format:](ITKAssert, "handleFailedAssertWithCondition:functionName:simulateCrash:showAlert:format:", "__objc_no", "-[NSMutableArray(ITK) itk_insertNonNilObject:atIndex:]", 0, 0, CFSTR("Trying to insert object in mutableArray at index (%ld) greater than count (%ld)"), a4, objc_msgSend(a1, "count"));
LABEL_8:
    v6 = v8;
    goto LABEL_9;
  }
...
- (void)itk_swapItemAtIndex:()ITK withItemAtIndex:
{
  ...
  if (objc_msgSend(a1, "count") <= a3 || objc_msgSend(a1, "count") <= a4)
  {
    ...
    +[ITKAssert handleFailedAssertWithCondition:functionName:simulateCrash:showAlert:format:](ITKAssert, "handleFailedAssertWithCondition:functionName:simulateCrash:showAlert:format:", "__objc_no", "-[NSMutableArray(ITK) itk_swapItemAtIndex:withItemAtIndex:]", 0, 0, CFSTR("Trying to swap item at index %@, with item at index %@, but array size is %ld"), v8, v7, objc_msgSend(a1, "count"));
LABEL_6:
