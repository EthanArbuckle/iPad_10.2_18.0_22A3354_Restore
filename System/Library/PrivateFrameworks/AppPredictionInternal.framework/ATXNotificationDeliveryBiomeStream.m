...
- (NSString)identifier
{
  return (NSString *)CFSTR("notificationSuggestionDelivery");
}

- (ATXNotificationDeliveryBiomeStream)init
{
  return -[ATXNotificationDeliveryBiomeStream initWithStoreConfig:](self, "initWithStoreConfig:", 0);
}

- (ATXNotificationDeliveryBiomeStream)initWithStoreConfig:(id)a3
{
  id v4;
  ATXNotificationDeliveryBiomeStream *v5;
  id v6;
  void *v7;
  uint64_t v8;
  BMStoreStream *inner;
  ...
  v5 = -[ATXNotificationDeliveryBiomeStream init](&v11, sel_init);
  if (v5)
  {
    if (v4)
    {
    ...
    v7 = v6;
    v8 = objc_msgSend(objc_alloc(MEMORY[0x1E0D02790]), "initWithPrivateStreamIdentifier:storeConfig:eventDataClass:", CFSTR("notificationSuggestionDelivery"), v6, objc_opt_class());
    inner = v5->_inner;
...
{
  -[BMStoreStream pruneWithPredicateBlock:](self->_inner, "pruneWithPredicateBlock:", &__block_literal_global_188);
}
