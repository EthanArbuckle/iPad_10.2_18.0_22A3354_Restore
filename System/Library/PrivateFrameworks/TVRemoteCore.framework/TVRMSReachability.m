...
- (void)stopNotifier
{
  ...
  if (reachabilityRef)
  {
    ...
    Current = CFRunLoopGetCurrent();
    SCNetworkReachabilityUnscheduleFromRunLoop(v4, Current, (CFStringRef)*MEMORY[0x24BDBD5A0]);
  }
}
...
- (void)dealloc
{
  ...
  -[TVRMSReachability stopNotifier](self, "stopNotifier");
  reachabilityRef = self->_reachabilityRef;
  if (reachabilityRef)
    CFRelease(reachabilityRef);
  v4.receiver = self;
  v4.super_class = (Class)TVRMSReachability;
  -[TVRMSReachability dealloc](&v4, sel_dealloc);
}
...
- (int64_t)currentReachabilityStatus
{
  ...
  if (self->_alwaysReturnLocalWiFiStatus)
    return -[TVRMSReachability localWiFiStatusForFlags:](self, "localWiFiStatusForFlags:", flags);
  return -[TVRMSReachability networkStatusForFlags:](self, "networkStatusForFlags:", flags);
}
