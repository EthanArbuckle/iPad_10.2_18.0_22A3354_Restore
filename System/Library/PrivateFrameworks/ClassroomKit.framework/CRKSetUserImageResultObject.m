@implementation CRKSetUserImageResultObject

+ (BOOL)supportsSecureCoding
{
  return 1;
}

- (CRKSetUserImageResultObject)initWithCoder:(id)a3
{
  id v4;
  CRKSetUserImageResultObject *v5;
  void *v6;
  uint64_t v7;
  NSString *imageIdentifier;
  objc_super v10;

  v4 = a3;
  v10.receiver = self;
  v10.super_class = (Class)CRKSetUserImageResultObject;
  v5 = -[CATTaskResultObject initWithCoder:](&v10, sel_initWithCoder_, v4);
  if (v5)
  {
    objc_msgSend(MEMORY[0x24BDBCF20], "setWithObjects:", objc_opt_class(), 0);
    v6 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v4, "decodeObjectOfClasses:forKey:", v6, CFSTR("imageIdentifier"));
    v7 = objc_claimAutoreleasedReturnValue();
    imageIdentifier = v5->_imageIdentifier;
    v5->_imageIdentifier = (NSString *)v7;

  }
  return v5;
}

- (void)encodeWithCoder:(id)a3
{
  id v4;
  void *v5;
  objc_super v6;

  v6.receiver = self;
  v6.super_class = (Class)CRKSetUserImageResultObject;
  v4 = a3;
  -[CATTaskResultObject encodeWithCoder:](&v6, sel_encodeWithCoder_, v4);
  -[CRKSetUserImageResultObject imageIdentifier](self, "imageIdentifier", v6.receiver, v6.super_class);
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "encodeObject:forKey:", v5, CFSTR("imageIdentifier"));

}

- (NSString)imageIdentifier
{
  return self->_imageIdentifier;
}

- (void)setImageIdentifier:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 8);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_imageIdentifier, 0);
}

@end
