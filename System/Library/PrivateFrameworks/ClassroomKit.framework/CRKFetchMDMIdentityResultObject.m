@implementation CRKFetchMDMIdentityResultObject

+ (BOOL)supportsSecureCoding
{
  return 1;
}

- (CRKFetchMDMIdentityResultObject)initWithCoder:(id)a3
{
  id v4;
  CRKFetchMDMIdentityResultObject *v5;
  void *v6;
  uint64_t v7;
  NSString *mdmIdentityCommonName;
  objc_super v10;

  v4 = a3;
  v10.receiver = self;
  v10.super_class = (Class)CRKFetchMDMIdentityResultObject;
  v5 = -[CATTaskResultObject initWithCoder:](&v10, sel_initWithCoder_, v4);
  if (v5)
  {
    objc_msgSend(MEMORY[0x24BDBCF20], "setWithObjects:", objc_opt_class(), 0);
    v6 = (void *)objc_claimAutoreleasedReturnValue();
    objc_msgSend(v4, "decodeObjectOfClasses:forKey:", v6, CFSTR("mdmIdentityCommonName"));
    v7 = objc_claimAutoreleasedReturnValue();
    mdmIdentityCommonName = v5->_mdmIdentityCommonName;
    v5->_mdmIdentityCommonName = (NSString *)v7;

  }
  return v5;
}

- (void)encodeWithCoder:(id)a3
{
  id v4;
  void *v5;
  objc_super v6;

  v6.receiver = self;
  v6.super_class = (Class)CRKFetchMDMIdentityResultObject;
  v4 = a3;
  -[CATTaskResultObject encodeWithCoder:](&v6, sel_encodeWithCoder_, v4);
  -[CRKFetchMDMIdentityResultObject mdmIdentityCommonName](self, "mdmIdentityCommonName", v6.receiver, v6.super_class);
  v5 = (void *)objc_claimAutoreleasedReturnValue();
  objc_msgSend(v4, "encodeObject:forKey:", v5, CFSTR("mdmIdentityCommonName"));

}

- (NSString)mdmIdentityCommonName
{
  return self->_mdmIdentityCommonName;
}

- (void)setMdmIdentityCommonName:(id)a3
{
  objc_storeStrong((id *)&self->_mdmIdentityCommonName, a3);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_mdmIdentityCommonName, 0);
}

@end
