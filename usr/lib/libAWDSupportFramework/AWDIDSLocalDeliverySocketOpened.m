@implementation AWDIDSLocalDeliverySocketOpened

- (void)dealloc
{
  objc_super v3;

  -[AWDIDSLocalDeliverySocketOpened setService:](self, "setService:", 0);
  -[AWDIDSLocalDeliverySocketOpened setStreamName:](self, "setStreamName:", 0);
  -[AWDIDSLocalDeliverySocketOpened setGuid:](self, "setGuid:", 0);
  v3.receiver = self;
  v3.super_class = (Class)AWDIDSLocalDeliverySocketOpened;
  -[AWDIDSLocalDeliverySocketOpened dealloc](&v3, sel_dealloc);
}

- (void)setTimestamp:(unint64_t)a3
{
  *(_BYTE *)&self->_has |= 0x10u;
  self->_timestamp = a3;
}

- (void)setHasTimestamp:(BOOL)a3
{
  char v3;

  if (a3)
    v3 = 16;
  else
    v3 = 0;
  *(_BYTE *)&self->_has = *(_BYTE *)&self->_has & 0xEF | v3;
}

- (BOOL)hasTimestamp
{
  return (*(_BYTE *)&self->_has >> 4) & 1;
}

- (BOOL)hasService
{
  return self->_service != 0;
}

- (void)setIsToDefaultPairedDevice:(unint64_t)a3
{
  *(_BYTE *)&self->_has |= 1u;
  self->_isToDefaultPairedDevice = a3;
}

- (void)setHasIsToDefaultPairedDevice:(BOOL)a3
{
  *(_BYTE *)&self->_has = *(_BYTE *)&self->_has & 0xFE | a3;
}

- (BOOL)hasIsToDefaultPairedDevice
{
  return *(_BYTE *)&self->_has & 1;
}

- (void)setOpenError:(unint64_t)a3
{
  *(_BYTE *)&self->_has |= 2u;
  self->_openError = a3;
}

- (void)setHasOpenError:(BOOL)a3
{
  char v3;

  if (a3)
    v3 = 2;
  else
    v3 = 0;
  *(_BYTE *)&self->_has = *(_BYTE *)&self->_has & 0xFD | v3;
}

- (BOOL)hasOpenError
{
  return (*(_BYTE *)&self->_has >> 1) & 1;
}

- (void)setSocketError:(unint64_t)a3
{
  *(_BYTE *)&self->_has |= 8u;
  self->_socketError = a3;
}

- (void)setHasSocketError:(BOOL)a3
{
  char v3;

  if (a3)
    v3 = 8;
  else
    v3 = 0;
  *(_BYTE *)&self->_has = *(_BYTE *)&self->_has & 0xF7 | v3;
}

- (BOOL)hasSocketError
{
  return (*(_BYTE *)&self->_has >> 3) & 1;
}

- (void)setPriority:(unint64_t)a3
{
  *(_BYTE *)&self->_has |= 4u;
  self->_priority = a3;
}

- (void)setHasPriority:(BOOL)a3
{
  char v3;

  if (a3)
    v3 = 4;
  else
    v3 = 0;
  *(_BYTE *)&self->_has = *(_BYTE *)&self->_has & 0xFB | v3;
}

- (BOOL)hasPriority
{
  return (*(_BYTE *)&self->_has >> 2) & 1;
}

- (BOOL)hasStreamName
{
  return self->_streamName != 0;
}

- (BOOL)hasGuid
{
  return self->_guid != 0;
}

- (id)description
{
  objc_super v3;

  v3.receiver = self;
  v3.super_class = (Class)AWDIDSLocalDeliverySocketOpened;
  return (id)objc_msgSend(MEMORY[0x24BDD17C8], "stringWithFormat:", CFSTR("%@ %@"), -[AWDIDSLocalDeliverySocketOpened description](&v3, sel_description), -[AWDIDSLocalDeliverySocketOpened dictionaryRepresentation](self, "dictionaryRepresentation"));
}

- (id)dictionaryRepresentation
{
  void *v3;
  NSString *service;
  char has;
  NSString *streamName;
  NSString *guid;

  v3 = (void *)objc_msgSend(MEMORY[0x24BDBCED8], "dictionary");
  if ((*(_BYTE *)&self->_has & 0x10) != 0)
    objc_msgSend(v3, "setObject:forKey:", objc_msgSend(MEMORY[0x24BDD16E0], "numberWithUnsignedLongLong:", self->_timestamp), CFSTR("timestamp"));
  service = self->_service;
  if (service)
    objc_msgSend(v3, "setObject:forKey:", service, CFSTR("service"));
  has = (char)self->_has;
  if ((has & 1) != 0)
  {
    objc_msgSend(v3, "setObject:forKey:", objc_msgSend(MEMORY[0x24BDD16E0], "numberWithUnsignedLongLong:", self->_isToDefaultPairedDevice), CFSTR("isToDefaultPairedDevice"));
    has = (char)self->_has;
    if ((has & 2) == 0)
    {
LABEL_7:
      if ((has & 8) == 0)
        goto LABEL_8;
      goto LABEL_17;
    }
  }
  else if ((*(_BYTE *)&self->_has & 2) == 0)
  {
    goto LABEL_7;
  }
  objc_msgSend(v3, "setObject:forKey:", objc_msgSend(MEMORY[0x24BDD16E0], "numberWithUnsignedLongLong:", self->_openError), CFSTR("openError"));
  has = (char)self->_has;
  if ((has & 8) == 0)
  {
LABEL_8:
    if ((has & 4) == 0)
      goto LABEL_10;
    goto LABEL_9;
  }
LABEL_17:
  objc_msgSend(v3, "setObject:forKey:", objc_msgSend(MEMORY[0x24BDD16E0], "numberWithUnsignedLongLong:", self->_socketError), CFSTR("socketError"));
  if ((*(_BYTE *)&self->_has & 4) != 0)
LABEL_9:
    objc_msgSend(v3, "setObject:forKey:", objc_msgSend(MEMORY[0x24BDD16E0], "numberWithUnsignedLongLong:", self->_priority), CFSTR("priority"));
LABEL_10:
  streamName = self->_streamName;
  if (streamName)
    objc_msgSend(v3, "setObject:forKey:", streamName, CFSTR("streamName"));
  guid = self->_guid;
  if (guid)
    objc_msgSend(v3, "setObject:forKey:", guid, CFSTR("guid"));
  return v3;
}

- (BOOL)readFrom:(id)a3
{
  return AWDIDSLocalDeliverySocketOpenedReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  char has;

  if ((*(_BYTE *)&self->_has & 0x10) != 0)
    PBDataWriterWriteUint64Field();
  if (self->_service)
    PBDataWriterWriteStringField();
  has = (char)self->_has;
  if ((has & 1) != 0)
  {
    PBDataWriterWriteUint64Field();
    has = (char)self->_has;
    if ((has & 2) == 0)
    {
LABEL_7:
      if ((has & 8) == 0)
        goto LABEL_8;
      goto LABEL_16;
    }
  }
  else if ((*(_BYTE *)&self->_has & 2) == 0)
  {
    goto LABEL_7;
  }
  PBDataWriterWriteUint64Field();
  has = (char)self->_has;
  if ((has & 8) == 0)
  {
LABEL_8:
    if ((has & 4) == 0)
      goto LABEL_10;
    goto LABEL_9;
  }
LABEL_16:
  PBDataWriterWriteUint64Field();
  if ((*(_BYTE *)&self->_has & 4) != 0)
LABEL_9:
    PBDataWriterWriteUint64Field();
LABEL_10:
  if (self->_streamName)
    PBDataWriterWriteStringField();
  if (self->_guid)
    PBDataWriterWriteStringField();
}

- (void)copyTo:(id)a3
{
  char has;

  if ((*(_BYTE *)&self->_has & 0x10) != 0)
  {
    *((_QWORD *)a3 + 5) = self->_timestamp;
    *((_BYTE *)a3 + 72) |= 0x10u;
  }
  if (self->_service)
    objc_msgSend(a3, "setService:");
  has = (char)self->_has;
  if ((has & 1) != 0)
  {
    *((_QWORD *)a3 + 1) = self->_isToDefaultPairedDevice;
    *((_BYTE *)a3 + 72) |= 1u;
    has = (char)self->_has;
    if ((has & 2) == 0)
    {
LABEL_7:
      if ((has & 8) == 0)
        goto LABEL_8;
      goto LABEL_16;
    }
  }
  else if ((*(_BYTE *)&self->_has & 2) == 0)
  {
    goto LABEL_7;
  }
  *((_QWORD *)a3 + 2) = self->_openError;
  *((_BYTE *)a3 + 72) |= 2u;
  has = (char)self->_has;
  if ((has & 8) == 0)
  {
LABEL_8:
    if ((has & 4) == 0)
      goto LABEL_10;
    goto LABEL_9;
  }
LABEL_16:
  *((_QWORD *)a3 + 4) = self->_socketError;
  *((_BYTE *)a3 + 72) |= 8u;
  if ((*(_BYTE *)&self->_has & 4) != 0)
  {
LABEL_9:
    *((_QWORD *)a3 + 3) = self->_priority;
    *((_BYTE *)a3 + 72) |= 4u;
  }
LABEL_10:
  if (self->_streamName)
    objc_msgSend(a3, "setStreamName:");
  if (self->_guid)
    objc_msgSend(a3, "setGuid:");
}

- (id)copyWithZone:(_NSZone *)a3
{
  uint64_t v5;
  uint64_t v6;
  char has;

  v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  v6 = v5;
  if ((*(_BYTE *)&self->_has & 0x10) != 0)
  {
    *(_QWORD *)(v5 + 40) = self->_timestamp;
    *(_BYTE *)(v5 + 72) |= 0x10u;
  }

  *(_QWORD *)(v6 + 56) = -[NSString copyWithZone:](self->_service, "copyWithZone:", a3);
  has = (char)self->_has;
  if ((has & 1) != 0)
  {
    *(_QWORD *)(v6 + 8) = self->_isToDefaultPairedDevice;
    *(_BYTE *)(v6 + 72) |= 1u;
    has = (char)self->_has;
    if ((has & 2) == 0)
    {
LABEL_5:
      if ((has & 8) == 0)
        goto LABEL_6;
LABEL_11:
      *(_QWORD *)(v6 + 32) = self->_socketError;
      *(_BYTE *)(v6 + 72) |= 8u;
      if ((*(_BYTE *)&self->_has & 4) == 0)
        goto LABEL_8;
      goto LABEL_7;
    }
  }
  else if ((*(_BYTE *)&self->_has & 2) == 0)
  {
    goto LABEL_5;
  }
  *(_QWORD *)(v6 + 16) = self->_openError;
  *(_BYTE *)(v6 + 72) |= 2u;
  has = (char)self->_has;
  if ((has & 8) != 0)
    goto LABEL_11;
LABEL_6:
  if ((has & 4) != 0)
  {
LABEL_7:
    *(_QWORD *)(v6 + 24) = self->_priority;
    *(_BYTE *)(v6 + 72) |= 4u;
  }
LABEL_8:

  *(_QWORD *)(v6 + 64) = -[NSString copyWithZone:](self->_streamName, "copyWithZone:", a3);
  *(_QWORD *)(v6 + 48) = -[NSString copyWithZone:](self->_guid, "copyWithZone:", a3);
  return (id)v6;
}

- (BOOL)isEqual:(id)a3
{
  int v5;
  char has;
  NSString *service;
  NSString *streamName;
  NSString *guid;

  v5 = objc_msgSend(a3, "isMemberOfClass:", objc_opt_class());
  if (v5)
  {
    has = (char)self->_has;
    if ((has & 0x10) != 0)
    {
      if ((*((_BYTE *)a3 + 72) & 0x10) == 0 || self->_timestamp != *((_QWORD *)a3 + 5))
        goto LABEL_34;
    }
    else if ((*((_BYTE *)a3 + 72) & 0x10) != 0)
    {
LABEL_34:
      LOBYTE(v5) = 0;
      return v5;
    }
    service = self->_service;
    if ((unint64_t)service | *((_QWORD *)a3 + 7))
    {
      v5 = -[NSString isEqual:](service, "isEqual:");
      if (!v5)
        return v5;
      has = (char)self->_has;
    }
    if ((has & 1) != 0)
    {
      if ((*((_BYTE *)a3 + 72) & 1) == 0 || self->_isToDefaultPairedDevice != *((_QWORD *)a3 + 1))
        goto LABEL_34;
    }
    else if ((*((_BYTE *)a3 + 72) & 1) != 0)
    {
      goto LABEL_34;
    }
    if ((has & 2) != 0)
    {
      if ((*((_BYTE *)a3 + 72) & 2) == 0 || self->_openError != *((_QWORD *)a3 + 2))
        goto LABEL_34;
    }
    else if ((*((_BYTE *)a3 + 72) & 2) != 0)
    {
      goto LABEL_34;
    }
    if ((has & 8) != 0)
    {
      if ((*((_BYTE *)a3 + 72) & 8) == 0 || self->_socketError != *((_QWORD *)a3 + 4))
        goto LABEL_34;
    }
    else if ((*((_BYTE *)a3 + 72) & 8) != 0)
    {
      goto LABEL_34;
    }
    if ((has & 4) != 0)
    {
      if ((*((_BYTE *)a3 + 72) & 4) == 0 || self->_priority != *((_QWORD *)a3 + 3))
        goto LABEL_34;
    }
    else if ((*((_BYTE *)a3 + 72) & 4) != 0)
    {
      goto LABEL_34;
    }
    streamName = self->_streamName;
    if (!((unint64_t)streamName | *((_QWORD *)a3 + 8))
      || (v5 = -[NSString isEqual:](streamName, "isEqual:")) != 0)
    {
      guid = self->_guid;
      if ((unint64_t)guid | *((_QWORD *)a3 + 6))
        LOBYTE(v5) = -[NSString isEqual:](guid, "isEqual:");
      else
        LOBYTE(v5) = 1;
    }
  }
  return v5;
}

- (unint64_t)hash
{
  unint64_t v3;
  NSUInteger v4;
  unint64_t v5;
  unint64_t v6;
  unint64_t v7;
  unint64_t v8;
  NSUInteger v9;

  if ((*(_BYTE *)&self->_has & 0x10) != 0)
    v3 = 2654435761u * self->_timestamp;
  else
    v3 = 0;
  v4 = -[NSString hash](self->_service, "hash");
  if ((*(_BYTE *)&self->_has & 1) != 0)
  {
    v5 = 2654435761u * self->_isToDefaultPairedDevice;
    if ((*(_BYTE *)&self->_has & 2) != 0)
    {
LABEL_6:
      v6 = 2654435761u * self->_openError;
      if ((*(_BYTE *)&self->_has & 8) != 0)
        goto LABEL_7;
LABEL_11:
      v7 = 0;
      if ((*(_BYTE *)&self->_has & 4) != 0)
        goto LABEL_8;
LABEL_12:
      v8 = 0;
      goto LABEL_13;
    }
  }
  else
  {
    v5 = 0;
    if ((*(_BYTE *)&self->_has & 2) != 0)
      goto LABEL_6;
  }
  v6 = 0;
  if ((*(_BYTE *)&self->_has & 8) == 0)
    goto LABEL_11;
LABEL_7:
  v7 = 2654435761u * self->_socketError;
  if ((*(_BYTE *)&self->_has & 4) == 0)
    goto LABEL_12;
LABEL_8:
  v8 = 2654435761u * self->_priority;
LABEL_13:
  v9 = v4 ^ v3 ^ v5 ^ v6 ^ v7 ^ v8 ^ -[NSString hash](self->_streamName, "hash");
  return v9 ^ -[NSString hash](self->_guid, "hash");
}

- (void)mergeFrom:(id)a3
{
  char v5;

  if ((*((_BYTE *)a3 + 72) & 0x10) != 0)
  {
    self->_timestamp = *((_QWORD *)a3 + 5);
    *(_BYTE *)&self->_has |= 0x10u;
  }
  if (*((_QWORD *)a3 + 7))
    -[AWDIDSLocalDeliverySocketOpened setService:](self, "setService:");
  v5 = *((_BYTE *)a3 + 72);
  if ((v5 & 1) != 0)
  {
    self->_isToDefaultPairedDevice = *((_QWORD *)a3 + 1);
    *(_BYTE *)&self->_has |= 1u;
    v5 = *((_BYTE *)a3 + 72);
    if ((v5 & 2) == 0)
    {
LABEL_7:
      if ((v5 & 8) == 0)
        goto LABEL_8;
      goto LABEL_16;
    }
  }
  else if ((*((_BYTE *)a3 + 72) & 2) == 0)
  {
    goto LABEL_7;
  }
  self->_openError = *((_QWORD *)a3 + 2);
  *(_BYTE *)&self->_has |= 2u;
  v5 = *((_BYTE *)a3 + 72);
  if ((v5 & 8) == 0)
  {
LABEL_8:
    if ((v5 & 4) == 0)
      goto LABEL_10;
    goto LABEL_9;
  }
LABEL_16:
  self->_socketError = *((_QWORD *)a3 + 4);
  *(_BYTE *)&self->_has |= 8u;
  if ((*((_BYTE *)a3 + 72) & 4) != 0)
  {
LABEL_9:
    self->_priority = *((_QWORD *)a3 + 3);
    *(_BYTE *)&self->_has |= 4u;
  }
LABEL_10:
  if (*((_QWORD *)a3 + 8))
    -[AWDIDSLocalDeliverySocketOpened setStreamName:](self, "setStreamName:");
  if (*((_QWORD *)a3 + 6))
    -[AWDIDSLocalDeliverySocketOpened setGuid:](self, "setGuid:");
}

- (unint64_t)timestamp
{
  return self->_timestamp;
}

- (NSString)service
{
  return self->_service;
}

- (void)setService:(id)a3
{
  objc_setProperty_nonatomic(self, a2, a3, 56);
}

- (unint64_t)isToDefaultPairedDevice
{
  return self->_isToDefaultPairedDevice;
}

- (unint64_t)openError
{
  return self->_openError;
}

- (unint64_t)socketError
{
  return self->_socketError;
}

- (unint64_t)priority
{
  return self->_priority;
}

- (NSString)streamName
{
  return self->_streamName;
}

- (void)setStreamName:(id)a3
{
  objc_setProperty_nonatomic(self, a2, a3, 64);
}

- (NSString)guid
{
  return self->_guid;
}

- (void)setGuid:(id)a3
{
  objc_setProperty_nonatomic(self, a2, a3, 48);
}

@end
