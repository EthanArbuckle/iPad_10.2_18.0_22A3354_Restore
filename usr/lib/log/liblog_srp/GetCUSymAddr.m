@implementation GetCUSymAddr

uint64_t (*___GetCUSymAddr_DataBuffer_Free_block_invoke())(_QWORD)
{
  uint64_t (*result)(_QWORD);

  if (_GetCULibHandle_sOnce != -1)
    dispatch_once(&_GetCULibHandle_sOnce, &__block_literal_global_210);
  result = (uint64_t (*)(_QWORD))_GetCULibHandle_sHandle;
  if (_GetCULibHandle_sHandle)
  {
    result = (uint64_t (*)(_QWORD))dlsym((void *)_GetCULibHandle_sHandle, "DataBuffer_Free");
    _GetCUSymAddr_DataBuffer_Free_sAddr = result;
  }
  return result;
}

void *___GetCUSymAddr_DataBuffer_Detach_block_invoke()
{
  void *result;

  if (_GetCULibHandle_sOnce != -1)
    dispatch_once(&_GetCULibHandle_sOnce, &__block_literal_global_210);
  result = (void *)_GetCULibHandle_sHandle;
  if (_GetCULibHandle_sHandle)
  {
    result = dlsym((void *)_GetCULibHandle_sHandle, "DataBuffer_Detach");
    _GetCUSymAddr_DataBuffer_Detach_sAddr = (uint64_t (*)(_QWORD, _QWORD, _QWORD))result;
  }
  return result;
}

void *___GetCUSymAddr_DataBuffer_Append_block_invoke()
{
  void *result;

  if (_GetCULibHandle_sOnce != -1)
    dispatch_once(&_GetCULibHandle_sOnce, &__block_literal_global_210);
  result = (void *)_GetCULibHandle_sHandle;
  if (_GetCULibHandle_sHandle)
  {
    result = dlsym((void *)_GetCULibHandle_sHandle, "DataBuffer_Append");
    _GetCUSymAddr_DataBuffer_Append_sAddr = (uint64_t (*)(_QWORD, _QWORD, _QWORD))result;
  }
  return result;
}

void *___GetCUSymAddr_SecondsToYMD_HMS_block_invoke()
{
  void *result;

  if (_GetCULibHandle_sOnce != -1)
    dispatch_once(&_GetCULibHandle_sOnce, &__block_literal_global_210);
  result = (void *)_GetCULibHandle_sHandle;
  if (_GetCULibHandle_sHandle)
  {
    result = dlsym((void *)_GetCULibHandle_sHandle, "SecondsToYMD_HMS");
    _GetCUSymAddr_SecondsToYMD_HMS_sAddr = (uint64_t (*)(_QWORD, _QWORD, _QWORD, _QWORD, _QWORD, _QWORD, _QWORD))result;
  }
  return result;
}

void *___GetCUSymAddr_Base64EncodeCopyEx_block_invoke()
{
  void *result;

  if (_GetCULibHandle_sOnce != -1)
    dispatch_once(&_GetCULibHandle_sOnce, &__block_literal_global_210);
  result = (void *)_GetCULibHandle_sHandle;
  if (_GetCULibHandle_sHandle)
  {
    result = dlsym((void *)_GetCULibHandle_sHandle, "Base64EncodeCopyEx");
    _GetCUSymAddr_Base64EncodeCopyEx_sAddr = (uint64_t (*)(_QWORD, _QWORD, _QWORD, _QWORD, _QWORD))result;
  }
  return result;
}

void *___GetCUSymAddr_SNPrintF_block_invoke()
{
  void *result;

  if (_GetCULibHandle_sOnce != -1)
    dispatch_once(&_GetCULibHandle_sOnce, &__block_literal_global_210);
  result = (void *)_GetCULibHandle_sHandle;
  if (_GetCULibHandle_sHandle)
  {
    result = dlsym((void *)_GetCULibHandle_sHandle, "SNPrintF");
    _GetCUSymAddr_SNPrintF_sAddr = (uint64_t (*)(_QWORD, _QWORD, const char *, ...))result;
  }
  return result;
}

uint64_t (*___GetCUSymAddr_DataBuffer_AppendF_block_invoke())(_QWORD, const char *, ...)
{
  uint64_t (*result)(_QWORD, const char *, ...);

  if (_GetCULibHandle_sOnce != -1)
    dispatch_once(&_GetCULibHandle_sOnce, &__block_literal_global_210);
  result = (uint64_t (*)(_QWORD, const char *, ...))_GetCULibHandle_sHandle;
  if (_GetCULibHandle_sHandle)
  {
    result = (uint64_t (*)(_QWORD, const char *, ...))dlsym((void *)_GetCULibHandle_sHandle, "DataBuffer_AppendF");
    _GetCUSymAddr_DataBuffer_AppendF_sAddr = result;
  }
  return result;
}

void *___GetCUSymAddr_DataBuffer_Init_block_invoke()
{
  void *result;

  if (_GetCULibHandle_sOnce != -1)
    dispatch_once(&_GetCULibHandle_sOnce, &__block_literal_global_210);
  result = (void *)_GetCULibHandle_sHandle;
  if (_GetCULibHandle_sHandle)
  {
    result = dlsym((void *)_GetCULibHandle_sHandle, "DataBuffer_Init");
    _GetCUSymAddr_DataBuffer_Init_sAddr = (uint64_t (*)(_QWORD, _QWORD, _QWORD, _QWORD))result;
  }
  return result;
}

@end
