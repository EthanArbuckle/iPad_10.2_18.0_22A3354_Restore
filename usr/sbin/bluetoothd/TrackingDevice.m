...
- (TrackingDevice)initWithDevice:(id)a3 type:(unsigned __int8)a4 timeoutValue:(unsigned __int8)a5 targetCore:(unsigned __int8)a6 rssiThreshold:(unsigned __int8)a7 rssiThresholdGone:(unsigned __int8)a8 invalidRssiHandling:(unsigned __int8)a9 rssiPrecision:(unsigned __int8)a10 rssiPrecisionGone:(unsigned __int8)a11 eventConfiguration:(unsigned __int8)a12 vseBuffering:(unsigned __int8)a13 rssiIncreaseScanThreshold:(unsigned __int8)a14 rssiIncreaseScanWindowThreshold:(unsigned __int16)a15 rssiIncreaseScanIntervalThreshold:(unsigned __int16)a16 rssiIncreaseScanTimeoutThreshold:(unsigned __int16)a17 sensorTimeoutBetweenIncreaseScan:(unsigned __int16)a18
{
  ...
  v26 = -[TrackingDevice init](&v33, "init");
  v27 = v26;
  if (v26)
  {
    objc_storeStrong((id *)&v26->_deviceUUID, a3);
    v28 = (void *)objc_claimAutoreleasedReturnValue(+[NSNumber numberWithUnsignedChar:](NSNumber, "numberWithUnsignedChar:", v22));
    v29 = objc_claimAutoreleasedReturnValue(+[NSMutableSet setWithObject:](NSMutableSet, "setWithObject:", v28));
...
- (id)description
{
  ...
  v2 = self->_currentState - 1;
  if (v2 > 2)
    v3 = CFSTR("Not Set");
  else
    v3 = off_100939438[(char)v2];
  return (id)objc_claimAutoreleasedReturnValue(+[NSString stringWithFormat:](NSString, "stringWithFormat:", CFSTR("Tracking: %@ - state %@ - timeout value: %d - types: %@ rssi:%d:%d:%d tc:%d eCfg:%d:%d"), self->_deviceUUID, v3, self->_timeoutValue, self->_appleTypes, self->_rssiThreshold, self->_rssiPrecision, self->_invalidRssiHandling, self->_targetCore, self->_eventConfiguration, self->_vseBuffering));
}
