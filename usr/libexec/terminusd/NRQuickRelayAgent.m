...
- (void)dealloc
{
  ...
  if (IsLevelEnabled)
  {
    ...
    _NRLogWithArgs(v9, 0, "%s%.30s:%-4d Dealloc: %@", ", "-[NRQuickRelayAgent dealloc]", 73, v2);
  ...
  v15.receiver = v2;
  v15.super_class = (Class)NRQuickRelayAgent;
...
- (id)description
{
  ...
  if (self)
  {
    objc_msgSend(v3, "appendFormat:", CFSTR(", %u"), self->_assertCount);
  ...
  {
    objc_msgSend(v3, "appendFormat:", CFSTR(", %u"), 0);
    v6 = "";
  }
...
- (id)copyAgentData
{
  ...
  if (IsLevelEnabled)
  {
    ...
    _NRLogWithArgs(v7, 1, "%s%.30s:%-4d Updating agent data - state: %u", ", "-[NRQuickRelayAgent copyAgentData]", 105, v2->_state.state);

  }
  return objc_msgSend(objc_alloc((Class)NSData), "initWithBytes:length:", &v2->_state, 1);
}
...
+ (id)agentType
{
  return objc_msgSend(objc_alloc((Class)NSString), "initWithFormat:", CFSTR("%s"), "QuickRelayAgent");
}
