...
- (SDHeadphoneEndCallManager)initWithBluetoothAddress:(id)a3
{
  ...
  v10.receiver = self;
  v10.super_class = (Class)SDHeadphoneEndCallManager;
  v3 = a3;
  v4 = -[SDHeadphoneEndCallManager init](&v10, "init");
  v5 = (NSString *)objc_msgSend(v3, "copy");

  bluetoothAddressString = v4->_bluetoothAddressString;
  v4->_bluetoothAddressString = v5;

  if (dword_1007B04F8 <= 30 && (dword_1007B04F8 != -1 || _LogCategory_Initialize(&dword_1007B04F8, 30)))
    LogPrintF(&dword_1007B04F8, "-[SDHeadphoneEndCallManager initWithBluetoothAddress:]", 30, "starting end call manager for device with address:%@", v4->_bluetoothAddressString);
    ...
    while (2)
    {
      for (i = 0; i != v6; i = (char *)i + 1)
      {
        ...
        if (v11)
        {
          ...
          if (dword_1007B04F8 <= 30
            && (dword_1007B04F8 != -1 || _LogCategory_Initialize(&dword_1007B04F8, 30)))
          {
            LogPrintF(&dword_1007B04F8, "-[SDHeadphoneEndCallManager isStatusUnknown]", 30, "end call manager with device:%@", self->_bluetoothDevice);
          }
          goto LABEL_14;
        }
      }
...
LABEL_14:
    ...
    || (_BYTE)v12)
  {
    if (dword_1007B04F8 <= 30
      && (dword_1007B04F8 != -1 || _LogCategory_Initialize(&dword_1007B04F8, 30)))
    {
      LogPrintF(&dword_1007B04F8, "-[SDHeadphoneEndCallManager isStatusUnknown]", 30, "Device doesn't support end call or already set before, {_bluetoothDevice: %@}\n", self->_bluetoothDevice);
    }
    v13 = 0;
  }
  else
