...
- (GTFileTransferOptions)init
{
  ...
  result = -[GTFileTransferOptions init](&v3, "init");
  if (result)
    *(_OWORD *)&result->_chunkSize = xmmword_100027A30;
  return result;
}
...
- (GTFileTransferOptions)initWithCoder:(id)a3
{
  ...
  v5 = -[GTFileTransferOptions init](&v7, "init");
  if (v5)
  {
    v5->_chunkSize = (unint64_t)objc_msgSend(v4, "decodeInt64ForKey:", CFSTR("chunkSize"));
    v5->_compressionAlgorithm = (int64_t)objc_msgSend(v4, "decodeIntegerForKey:", CFSTR("compressionAlgorithm"));
  }
