@implementation MOEventFetchOptions

- (MOEventFetchOptions)initWithDateInterval:(id)a3 ascending:(BOOL)a4 limit:(id)a5
{
  return -[MOEventFetchOptions initWithDateInterval:ascending:category:limit:](self, "initWithDateInterval:ascending:category:limit:", a3, a4, 0, a5);
}

- (MOEventFetchOptions)initWithDateInterval:(id)a3 ascending:(BOOL)a4 category:(unint64_t)a5 limit:(id)a6
{
  id v11;
  id v12;
  MOEventFetchOptions *v13;
  MOEventFetchOptions *v14;
  objc_super v16;

  v11 = a3;
  v12 = a6;
  v16.receiver = self;
  v16.super_class = (Class)MOEventFetchOptions;
  v13 = -[MOEventFetchOptions init](&v16, "init");
  v14 = v13;
  if (v13)
  {
    v13->_ascending = a4;
    v13->_category = a5;
    objc_storeStrong((id *)&v13->_dateInterval, a3);
    objc_storeStrong((id *)&v14->_limit, a6);
  }

  return v14;
}

+ (BOOL)supportsSecureCoding
{
  return 1;
}

- (void)encodeWithCoder:(id)a3
{
  _BOOL8 ascending;
  id v5;
  void *v6;
  id v7;

  ascending = self->_ascending;
  v5 = a3;
  v6 = (void *)objc_claimAutoreleasedReturnValue(+[NSNumber numberWithBool:](NSNumber, "numberWithBool:", ascending));
  objc_msgSend(v5, "encodeObject:forKey:", v6, CFSTR("ascending"));

  objc_msgSend(v5, "encodeObject:forKey:", self->_dateInterval, CFSTR("dateInterval"));
  objc_msgSend(v5, "encodeObject:forKey:", self->_limit, CFSTR("limit"));
  v7 = (id)objc_claimAutoreleasedReturnValue(+[NSNumber numberWithUnsignedInteger:](NSNumber, "numberWithUnsignedInteger:", self->_category));
  objc_msgSend(v5, "encodeObject:forKey:", v7, CFSTR("category"));

}

- (MOEventFetchOptions)initWithCoder:(id)a3
{
  id v4;
  uint64_t v5;
  id v6;
  void *v7;
  id v8;
  uint64_t v9;
  id v10;
  void *v11;
  uint64_t v12;
  id v13;
  void *v14;
  uint64_t v15;
  id v16;
  void *v17;
  MOEventFetchOptions *v18;

  v4 = a3;
  v6 = objc_msgSend(v4, "decodeObjectOfClass:forKey:", objc_opt_class(NSNumber, v5), CFSTR("ascending"));
  v7 = (void *)objc_claimAutoreleasedReturnValue(v6);
  v8 = objc_msgSend(v7, "BOOLValue");

  v10 = objc_msgSend(v4, "decodeObjectOfClass:forKey:", objc_opt_class(NSDateInterval, v9), CFSTR("dateInterval"));
  v11 = (void *)objc_claimAutoreleasedReturnValue(v10);
  v13 = objc_msgSend(v4, "decodeObjectOfClass:forKey:", objc_opt_class(NSNumber, v12), CFSTR("limit"));
  v14 = (void *)objc_claimAutoreleasedReturnValue(v13);
  v16 = objc_msgSend(v4, "decodeObjectOfClass:forKey:", objc_opt_class(NSNumber, v15), CFSTR("category"));
  v17 = (void *)objc_claimAutoreleasedReturnValue(v16);

  v18 = -[MOEventFetchOptions initWithDateInterval:ascending:category:limit:](self, "initWithDateInterval:ascending:category:limit:", v11, v8, objc_msgSend(v17, "unsignedLongValue"), v14);
  return v18;
}

- (BOOL)isEqual:(id)a3
{
  MOEventFetchOptions *v4;
  uint64_t v5;
  MOEventFetchOptions *v6;
  uint64_t v7;
  BOOL v8;

  v4 = (MOEventFetchOptions *)a3;
  v6 = v4;
  if (v4 == self)
  {
    v8 = 1;
  }
  else if (v4 && (v7 = objc_opt_class(self, v5), (objc_opt_isKindOfClass(v6, v7) & 1) != 0))
  {
    v8 = -[MOEventFetchOptions isEqualToFetchOptions:](self, "isEqualToFetchOptions:", v6);
  }
  else
  {
    v8 = 0;
  }

  return v8;
}

- (BOOL)isEqualToFetchOptions:(id)a3
{
  void *v3;
  id v5;
  int ascending;
  unsigned int v7;
  NSDateInterval *dateInterval;
  uint64_t v9;
  void *v10;
  NSDateInterval *v11;
  void *v12;
  unsigned int v13;
  void *v14;
  unsigned __int8 v15;
  void *v16;
  NSNumber *limit;
  void *v18;
  unint64_t category;
  id v20;
  unsigned int v21;
  unsigned __int8 v22;
  BOOL v23;

  v5 = a3;
  ascending = self->_ascending;
  v7 = objc_msgSend(v5, "ascending");
  dateInterval = self->_dateInterval;
  if (dateInterval)
    goto LABEL_2;
  v3 = (void *)objc_claimAutoreleasedReturnValue(objc_msgSend(v5, "dateInterval"));
  if (!v3)
  {
    v13 = 1;
    goto LABEL_11;
  }
  if (self->_dateInterval)
  {
LABEL_2:
    v9 = objc_claimAutoreleasedReturnValue(objc_msgSend(v5, "dateInterval"));
    if (v9)
    {
      v10 = (void *)v9;
      v11 = self->_dateInterval;
      v12 = (void *)objc_claimAutoreleasedReturnValue(objc_msgSend(v5, "dateInterval"));
      v13 = -[NSDateInterval isEqualToDateInterval:](v11, "isEqualToDateInterval:", v12);

      if (dateInterval)
        goto LABEL_12;
    }
    else
    {
      v13 = 0;
      if (dateInterval)
        goto LABEL_12;
    }
  }
  else
  {
    v13 = 0;
  }
LABEL_11:

LABEL_12:
  if (self->_limit)
  {
    v14 = (void *)objc_claimAutoreleasedReturnValue(objc_msgSend(v5, "limit"));
    if (v14)
    {
      v15 = 1;
    }
    else if (self->_limit)
    {
      v16 = (void *)objc_claimAutoreleasedReturnValue(objc_msgSend(v5, "limit"));
      if (v16)
      {
        limit = self->_limit;
        v18 = (void *)objc_claimAutoreleasedReturnValue(objc_msgSend(v5, "limit"));
        v15 = -[NSNumber isEqualToNumber:](limit, "isEqualToNumber:", v18);

      }
      else
      {
        v15 = 0;
      }

    }
    else
    {
      v15 = 0;
    }

  }
  else
  {
    v15 = 0;
  }
  category = self->_category;
  v20 = objc_msgSend(v5, "category");
  if (ascending == v7)
    v21 = v13;
  else
    v21 = 0;
  if ((id)category == v20)
    v22 = v15;
  else
    v22 = 0;
  if (v21 == 1)
    v23 = v22;
  else
    v23 = 0;

  return v23;
}

- (unint64_t)hash
{
  void *v3;
  unint64_t v4;
  unint64_t v5;
  unint64_t v6;
  void *v7;
  unint64_t v8;

  v3 = (void *)objc_claimAutoreleasedReturnValue(+[NSNumber numberWithBool:](NSNumber, "numberWithBool:", self->_ascending));
  v4 = (unint64_t)objc_msgSend(v3, "hash");
  v5 = (unint64_t)-[NSDateInterval hash](self->_dateInterval, "hash");
  v6 = v5 ^ (unint64_t)-[NSNumber hash](self->_limit, "hash") ^ v4;
  v7 = (void *)objc_claimAutoreleasedReturnValue(+[NSNumber numberWithUnsignedInteger:](NSNumber, "numberWithUnsignedInteger:", self->_category));
  v8 = v6 ^ (unint64_t)objc_msgSend(v7, "hash");

  return v8;
}

- (id)description
{
  const __CFString *v3;
  void *v4;
  void *v5;
  void *v6;
  void *v7;
  void *v8;

  if (self->_ascending)
    v3 = CFSTR("YES");
  else
    v3 = CFSTR("NO");
  v4 = (void *)objc_claimAutoreleasedReturnValue(-[NSDateInterval startDate](self->_dateInterval, "startDate"));
  v5 = (void *)objc_claimAutoreleasedReturnValue(objc_msgSend(v4, "stringFromDate"));
  v6 = (void *)objc_claimAutoreleasedReturnValue(-[NSDateInterval endDate](self->_dateInterval, "endDate"));
  v7 = (void *)objc_claimAutoreleasedReturnValue(objc_msgSend(v6, "stringFromDate"));
  v8 = (void *)objc_claimAutoreleasedReturnValue(+[NSString stringWithFormat:](NSString, "stringWithFormat:", CFSTR("ascending, %@,  startDate, %@, endDate, %@, category, %lu, limit, %@"), v3, v5, v7, self->_category, self->_limit));

  return v8;
}

- (BOOL)ascending
{
  return self->_ascending;
}

- (NSDateInterval)dateInterval
{
  return self->_dateInterval;
}

- (void)setDateInterval:(id)a3
{
  objc_setProperty_nonatomic_copy(self, a2, a3, 16);
}

- (NSNumber)limit
{
  return self->_limit;
}

- (unint64_t)category
{
  return self->_category;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_limit, 0);
  objc_storeStrong((id *)&self->_dateInterval, 0);
}

@end
