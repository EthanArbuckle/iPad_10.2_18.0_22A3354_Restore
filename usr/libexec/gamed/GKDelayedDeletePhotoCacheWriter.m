...
{
  id v4;
  GKDelayedDeletePhotoCacheWriter *v5;
  GKDelayedDeletePhotoCacheWriter *v6;
  objc_super v8;

  v4 = a3;
  v8.receiver = self;
  v8.super_class = (Class)GKDelayedDeletePhotoCacheWriter;
  v5 = -[GKDelayedDeletePhotoCacheWriter init](&v8, "init");
  v6 = v5;
  if (v5)
    -[GKDelayedDeletePhotoCacheWriter setConnection:](v5, "setConnection:", v4);
...
- (id)photosInsertSQL
{
  return +[NSString stringWithFormat:](NSString, "stringWithFormat:", CFSTR("INSERT INTO delete_photos(           request_id)             VALUES (%s);"),
           ":request_id");
}
...
- (id)getOldPhotosSQL
{
  return +[NSString stringWithFormat:](NSString, "stringWithFormat:", CFSTR("            SELECT request_id             FROM requests             INNER JOIN delete_photos USING (request_id)             WHERE player_gcid = %s;"),
           ":player_gcid");
}
...
- (void)writeResources:(id)a3 handler:(id)a4
{
  ...
  _QWORD v17[4];
  id v18;
  GKDelayedDeletePhotoCacheWriter *v19;

  v6 = a3;
  v7 = a4;
  if ((unint64_t)objc_msgSend(v6, "count") >= 2)
  {
    v12 = +[NSString stringWithFormat:](NSString, "stringWithFormat:", CFSTR("%@ is being asked to write photos for multiple players. This is not currently supported."), objc_opt_class(self, v8));
